...
TEST(PackBlendDrawBufferTest, MapsColortexThroughRendertargets) {
 ...
 EXPECT_EQ(glutil::colortexToDrawBufferIndex(rts, 1), 0);
 EXPECT_EQ(glutil::colortexToDrawBufferIndex(rts, 2), 1);
 EXPECT_EQ(glutil::colortexToDrawBufferIndex(rts, 0), -1);
}

TEST(PackBlendDrawBufferTest, EmptyRendertargetsUsesIdentity) {
 EXPECT_EQ(glutil::colortexToDrawBufferIndex({}, 3), 3);
 EXPECT_EQ(glutil::colortexToDrawBufferIndex({}, -1), -1);
}
...
TEST(PackBlendDrawBufferTest, MapsHighColortexIndices) {
 ...
 EXPECT_EQ(glutil::colortexToDrawBufferIndex({16, 31}, 16), 0);
 EXPECT_EQ(glutil::colortexToDrawBufferIndex({16, 31}, 31), 1);
 EXPECT_EQ(glutil::blendFactor("SRC_ALPHA_SATURATE"), 0x0308u);
}
...
TEST(PackBlendDrawBufferTest, BlendFactorNamesMatchGlEnums) {
 ...
 EXPECT_EQ(glutil::blendFactor("ZERO"), 0u);
 EXPECT_EQ(glutil::blendFactor("ONE"), 1u);
 EXPECT_EQ(glutil::blendFactor("SRC_ALPHA"), 0x0302u);
 EXPECT_EQ(glutil::blendFactor("ONE_MINUS_SRC_ALPHA"), 0x0303u);
 EXPECT_EQ(glutil::blendFactor("SRC_COLOR"), 0x0300u);
 EXPECT_EQ(glutil::blendFactor("DST_COLOR"), 0x0306u);
}
