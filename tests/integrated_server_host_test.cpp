...
TEST(IntegratedServerHost, UsesCustomPortAndInMemoryProperties) {
  ...
  ASSERT_TRUE(server.startAsync()) << (server.lastError().empty() ? "integrated server failed to start" : server.lastError());
