...
TEST(LanHostCoordinator, LocalWorldCanHostRemoteCannotAndDisconnectClearsState) {
    ...
    client.worldSession().ownedWorldStorageMut() =
        std::make_unique<net::minecraft::AlphaWorldStorage>(root, "LanWorld", true);
    client.worldSession().ownedWorldMut() =
        std::make_unique<net::minecraft::World>(client.worldSession().ownedWorldStorage(), "LanWorld", 12345, false);
