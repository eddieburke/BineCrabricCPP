...
using net::minecraft::util::concurrent::TaskPriority;
...
TEST(ChannelTest, UrgentPriorityPopsFirst) {
 Channel<int> channel;
 channel.tryPush(1, TaskPriority::Normal);
 channel.tryPush(2, TaskPriority::Urgent);
 channel.tryPush(3, TaskPriority::High);
 channel.tryPush(4, TaskPriority::Normal);
 int out = 0;
 EXPECT_TRUE(channel.tryPop(out));
 EXPECT_EQ(out, 2);
 EXPECT_TRUE(channel.tryPop(out));
...
TEST(ChannelTest, VersionStampDropsStaleEntries) {
 Channel<int> channel;
 channel.tryPush(1, 10);
 channel.tryPush(2, 20);
 EXPECT_EQ(channel.latestStamp(), 20U);
 int out = 0;
 EXPECT_TRUE(channel.tryPop(out));
 EXPECT_EQ(out, 2);
 EXPECT_FALSE(channel.tryPop(out));
