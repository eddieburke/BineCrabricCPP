...
| `billboards({brightness?, rotation_x_rad?, rotation_y_rad?, blend?, depth_test?, depth_write?, billboards = {{x, y, z, size, alpha}, ...}})` | Draws billboard sprites. Returns count. |
...
### `minecraft.texture.create(spec)`

Creates a texture from pixel data. Accepts `{width, height, pixels = {argb...}}` and returns `{id, width, height}`.

```lua
local tex = minecraft.texture.create({
  width = 16, height = 16,
  pixels = {0xFFFF0000, 0xFF00FF00, ...}
})
```

### `minecraft.texture.release(id)`

Releases a previously created texture.

### `minecraft.texture.get_pixels(pathOrId)`

Returns `{width, height, pixels = {argb...}}` for a texture path or mod texture ID.

### `minecraft.texture.update(id, spec)`

Updates a texture's pixel data. Accepts `{width, height, pixels = {argb...}}`.

```lua
minecraft.texture.update(tex.id, {
  width = 16, height = 16,
  pixels = {0xFF0000FF, ...}
})
```

### `minecraft.texture.bind(id, unit?)`

Binds a mod registry texture ID or raw OpenGL texture ID to a sampler unit. The unit defaults to `0`.

```lua
minecraft.texture.bind(tex.id, 2)
```
