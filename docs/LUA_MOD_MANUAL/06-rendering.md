...
| `billboards` | array | required | Array of billboard specs |
...
| `blend` | boolean | `false` | Alpha blending (opaque cutout by default) |
| `cull` | boolean | `true` | Back-face culling (`false` for crossed planes) |
| `depth_test` | boolean | `true` | Enable depth testing |
| `depth_write` | boolean | `true` | Enable depth buffer writes |
| `layer` | string | `"auto"` | Shader program family: `auto`, `entity`, `terrain`, `block`, `sky`, `basic` |
| `shader_entity` | string | `""` | Name for `entity.properties` → Iris `entityId` (e.g. `"lua_mod_entity"`) |
| `entity_id` | int | `0` | Raw Iris `entityId` override (not the network entity id) |
| `block_id` / `block_meta` | int | `0` | Optional `mc_Entity` for terrain/block layers |
...
Read, create, update, bind, and release textures. Resource reads are cached
for performance.

| Function | Result |
|---|---|
| `create({width, height, pixels})` | `{id, width, height}` |
| `update(id, {width, height, pixels})` | boolean |
| `release(id)` | boolean |
| `get_pixels(pathOrId)` | `{width, height, pixels}` or `nil` |
| `bind(id, unit?)` | boolean |
| `size(path)` | `{width, height}` |
| `pixel(path, x, y)` | `{r, g, b, a}` |
