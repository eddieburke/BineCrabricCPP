...
loaders. Modules therefore use `minecraft.require` or `minecraft.require_dir`.
...
`mods/lib/settings/init.lua` is the only shared library. It creates a cached
proxy per mod id. The first call to
