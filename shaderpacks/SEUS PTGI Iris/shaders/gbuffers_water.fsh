...
float GetWaves(vec3 position, in float scale) {
  ...
  // allwaves += pow(textureSmooth(noisetex, position.xz * 0.1).x, 19.0) * 150.1;
...
{
	vec3 p = worldPosition.xyz * 0.7;
	float a = 0.05;

	texNormal.x += simplex3d(p 		 + FRAME_TIME * 0.5) * a;
	texNormal.y += simplex3d(p + 2.0 + FRAME_TIME * 0.5) * a;
...
void main() {
  ...
  // gl_FragData[0] = vec4(tex.rgb, 0.2);
	...
	waterNormal = mix(waterNormal, texNormal, isIce + isStainedGlass + isSlime);

	waterNormal = mix(waterNormal, SlimeJiggleNormal(texNormal, worldPosition.xyz), isSlime);
