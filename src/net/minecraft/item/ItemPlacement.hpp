...
inline void playerLookRay(const PlayerEntity* user, float partialTicks, Vec3d& start, Vec3d& end, double reach = 5.0)
{
    ...
    const float sinPitch = MathHelper::sin(-pitch * (kPiF / 180.0f));
        ...
        start.x + static_cast<double>(sinYaw * negCosPitch) * reach,
...
{
    if (world == nullptr || block == nullptr || block->soundGroup == nullptr) {
        return;
    }
    world->playSound(static_cast<double>(x) + 0.5, static_cast<double>(y) + 0.5, static_cast<double>(z) + 0.5,
        block->soundGroup->getSound(), (block->soundGroup->getVolume() + 1.0f) * 0.5f,
        block->soundGroup->getPitch() * 0.8f);
}

inline bool placeBlockItem(ItemStack* stack, PlayerEntity* user, World* world, int blockId, int metadata,
    int x, int y, int z, int side, bool alwaysReturnTrue)
{
