...
BucketItem::BucketItem(int rawId, int fluidBlockId) : Item(rawId), fluidBlockId_(fluidBlockId) {
  setMaxCount(1);
}
    ...
    if(fluidBlockId_ == 0) {
      ...
      if(world->isAir(x, y, z) || !world->getMaterial(x, y, z).isSolid()) {
        if(world->dimension != nullptr && world->dimension->evaporatesWater &&
          ...
          world->playSound(soundX + 0.5, soundY + 0.5, soundZ + 0.5, "random.fizz", 0.5f,
...
void BucketItem::registerClass() {
  ...
  LAVA_BUCKET.setTexturePosition(12, 4)
      ->setTranslationKey("bucketLava")
      ->setCraftingReturnItem(Item::byRawId(69))
      ->setFuelTime(20000);
