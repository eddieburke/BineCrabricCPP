...
protected:
  ArmorItem(int rawId, int type, int textureIndex, int equipmentSlot)
      : Item(rawId, RegistrationMode::Deferred), type(type), equipmentSlot(equipmentSlot),
        maxProtection(PROTECTION_BY_SLOT[equipmentSlot]), textureIndex(textureIndex) {
    setMaxCount(1);
...
public:
  [[nodiscard]] int getMaxProtection() const {
    return maxProtection;
  }
  [[nodiscard]] int getTextureIndex() const {
    return textureIndex;
  }
  [[nodiscard]] int getEquipmentSlot() const {
