  ...
  if(!world->isRemote()) {
   ...
   for(int i = 0; i < 128; ++i) {
    ...
    for(int j = 0; j < i / 16; ++j) {
     ...
     if(world->getBlockId(px, py - 1, pz) != Block::GRASS_BLOCK->id ||
        world->shouldSuffocate(px, py, pz)) {
      blocked = true;
      break;
     }
    }
...
void DyeItem::registerSmeltingRecipes() {
 if(Block::CACTUS != nullptr) {
  recipe::SmeltingRecipeManager::instance().addRecipe(Block::CACTUS->id, ItemStack(Item::byRawId(95), 1, 2));
 }
}
void DyeItem::registerRecipes(recipe::CraftingRecipeManager& recipeManager) {
 ...
 recipeManager.addShapelessRecipe(
     ItemStack(Item::byRawId(95), 3, 13),
 ...
 recipeManager.addShapelessRecipe(ItemStack(Item::byRawId(95), 4, 13),
                                  {ItemStack(Item::byRawId(95), 1, 4),
                                   ItemStack(Item::byRawId(95), 1, 1),
                                   ItemStack(Item::byRawId(95), 1, 1),
                                   ItemStack(Item::byRawId(95), 1, 15)});
 if(Block::LAPIS_BLOCK != nullptr) {
  recipeManager.addShapedRecipe(
      ItemStack(Block::LAPIS_BLOCK),
      {std::string("###"), std::string("###"), std::string("###"), '#', ItemStack(Item::byRawId(95), 9, 4)});
  recipeManager.addShapedRecipe(ItemStack(Item::byRawId(95), 9, 4), {std::string("#"), '#', Block::LAPIS_BLOCK});
 }
}
