...
DoorItem::DoorItem(int rawId, block::material::Material& material) : Item(rawId), material_(&material) {
    setMaxCount(1);
}
...
bool DoorItem::useOnBlock(ItemStack* stack, PlayerEntity* user, World* world, int x, int y, int z, int side) {
    ...
    const int left =
        (world->shouldSuffocate(x - dx, y, z - dz) ? 1 : 0) + (world->shouldSuffocate(x - dx, y + 1, z - dz) ? 1 : 0);
    const int right =
        (world->shouldSuffocate(x + dx, y, z + dz) ? 1 : 0) + (world->shouldSuffocate(x + dx, y + 1, z + dz) ? 1 : 0);
    const bool doorLeft =
        world->getBlockId(x - dx, y, z - dz) == block->id || world->getBlockId(x - dx, y + 1, z - dz) == block->id;
