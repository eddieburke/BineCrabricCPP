...
public:
    explicit BlockItem(int rawId)
        : Item(rawId),
          itemId_(rawId + 256)
    {
        Block* block = itemId_ >= 0 && itemId_ < Block::BLOCK_COUNT ? Block::BLOCKS[static_cast<std::size_t>(itemId_)] : nullptr;
        if (block != nullptr) {
    ...
    bool useOnBlock(ItemStack* stack, PlayerEntity* user, World* world, int x, int y, int z, int side) override
    {
        ...
        return detail::placeBlockItem(stack, user, world, itemId_, getPlacementMetadata(stack != nullptr ? stack->getDamage() : 0),
            x, y, z, side, false);
    }
