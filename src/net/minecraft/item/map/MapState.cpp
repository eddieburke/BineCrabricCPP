...
MapState::PlayerUpdateTracker::PlayerUpdateTracker(PlayerEntity* playerIn)
    : player(playerIn)
{
    startZ.fill(0);
    endZ.fill(127);
...
std::vector<std::uint8_t> MapState::PlayerUpdateTracker::getUpdateData(MapState& mapState, const ItemStack& stack)
{
    ...
    if (--colorsUpdateInterval < 0) {
        ...
        for (std::size_t iconIndex = 0; iconIndex < mapState.icons.size(); ++iconIndex) {
            ...
            iconPacket[iconIndex * 3 + 1] =
                static_cast<std::uint8_t>(icon.type + (icon.rotation & 0x0F) * 16);
...
void MapState::update(PlayerEntity* player, const ItemStack& stack)
{
    ...
    for (auto it = updateTrackers_.begin(); it != updateTrackers_.end();) {
        ...
        std::uint8_t rotation =
            static_cast<std::uint8_t>(static_cast<double>(player->yaw) * 16.0 / 360.0 + 0.5);
        ...
        icons.push_back(MapIcon {
            0,
            static_cast<std::uint8_t>(static_cast<double>(relativeX) * 2.0 + 0.5),
            static_cast<std::uint8_t>(static_cast<double>(relativeZ) * 2.0 + 0.5),
            rotation});
        ++it;
    }
}
