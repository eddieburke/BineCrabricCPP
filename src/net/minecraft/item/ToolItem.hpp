...
protected:
  ToolItem(int rawId, int damageBoost, ToolMaterial material, Block** effectiveOn, int effectiveCount)
      : Item(rawId, RegistrationMode::Deferred), toolMaterial_(material),
        miningSpeed_(toolMaterialMiningSpeed(material)), damage_(damageBoost + toolMaterialAttackDamage(material)) {
    setMaxCount(1);
