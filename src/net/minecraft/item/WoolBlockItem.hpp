...
public:
    ...
    {
        return Block::WOOL != nullptr ? Block::WOOL->getTexture(2, WoolBlock::getBlockMeta(damage)) : BlockItem::getTextureId(damage);
    }
    ...
    [[nodiscard]] std::string getTranslationKey(const ItemStack* stack) const override
    {
        ...
        const auto index = static_cast<std::size_t>(WoolBlock::getBlockMeta(damage)) % DyeItem::names.size();
