...
map::MapState* MapItem::getSavedMapState(ItemStack& stack, World* world) {
  ...
  map::MapState* mapState =
      dynamic_cast<map::MapState*>(world->persistentStateManager.getOrCreate(typeid(map::MapState), stateId));
  if(mapState == nullptr) {
    ...
    newState->scale = 3;
    newState->dimension = world->dimension != nullptr ? static_cast<std::uint8_t>(world->dimension->id) : 0;
    newState->markDirty();
...
void MapItem::update(World* world, Entity* entity, map::MapState* mapState) {
  ...
  for(int mapX = entityMapX - sampleRadius + 1; mapX < entityMapX + sampleRadius; ++mapX) {
    ...
    for(int mapZ = entityMapZ - sampleRadius - 1; mapZ < entityMapZ + sampleRadius; ++mapZ) {
      ...
      const bool outsideCircle = offsetX * offsetX + offsetZ * offsetZ > (sampleRadius - 2) * (sampleRadius - 2);
      ...
      const std::uint8_t nextColor = static_cast<std::uint8_t>(mapColorId * 4 + shadeIndex);
...
void MapItem::onCraft(ItemStack* stack, World* world, PlayerEntity* player) {
  ...
  mapState->scale = 3;
  mapState->dimension = world->dimension != nullptr ? static_cast<std::uint8_t>(world->dimension->id) : 0;
  mapState->markDirty();
