...
struct ItemStack {
    ...
    constexpr ItemStack(int id, int count = 1, int damage = 0)
        : count(count), itemId(id), damage(damage) {}
    ...
    [[nodiscard]] int maxCount() const; // back-compat alias for getMaxCount
    ...
    [[nodiscard]] int getDamage2() const noexcept { return damage; }
    ...
    [[nodiscard]] bool sameItem(const ItemStack& other) const noexcept { return isItemEqual(other); }
    ...
    // --- copy / split (faithful) ---
    [[nodiscard]] ItemStack copy() const noexcept { return {itemId, count, damage}; }
