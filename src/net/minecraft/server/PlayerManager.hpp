...
public:
  ...
  ::net::minecraft::entity::player::ServerPlayerEntity* connectPlayer(network::ServerLoginNetworkHandler* loginNetworkHandler,
                                                                      const std::string& name);
  ::net::minecraft::entity::player::ServerPlayerEntity* respawnPlayer(::net::minecraft::entity::player::ServerPlayerEntity* player,
                                                                      int dimensionId);
  ...
  template <typename PacketT>
  void sendToAround(::net::minecraft::entity::player::PlayerEntity* player, double x, double y, double z, double range,
                    int dimensionId, const PacketT& packet) {
    const double rangeSq = range * range;
    for(::net::minecraft::entity::player::ServerPlayerEntity* serverPlayer : players) {
      if(serverPlayer == nullptr || static_cast<::net::minecraft::entity::player::PlayerEntity*>(serverPlayer) == player ||
         serverPlayer->dimensionId != dimensionId ||
         serverPlayer->networkHandler == nullptr) {
        continue;
      }
