...
net::minecraft::world::chunk::ChunkCache* chunkCacheFor(ServerWorld* world) {
 if(world == nullptr) {
  return nullptr;
 }
 return dynamic_cast<net::minecraft::world::chunk::ChunkCache*>(world->getChunkSource());
}
} // namespace
...
void PlayerManager::addPlayer(::net::minecraft::entity::player::ServerPlayerEntity* player) {
 ...
 if(net::minecraft::world::chunk::ChunkCache* cache = chunkCacheFor(serverWorld); cache != nullptr) {
...
::net::minecraft::entity::player::ServerPlayerEntity* PlayerManager::connectPlayer(
 ...
 std::string address =
     loginNetworkHandler->connection() != nullptr ? loginNetworkHandler->connection()->getAddress() : std::string{};
 ...
 return new ::net::minecraft::entity::player::ServerPlayerEntity(
     server_, overworld, trimmedName, &interactionManager);
}
::net::minecraft::entity::player::ServerPlayerEntity* PlayerManager::respawnPlayer(
 ...
 auto* respawned = new ::net::minecraft::entity::player::ServerPlayerEntity(
     server_, targetWorld, player->name, &interactionManager);
 ...
 if(spawnPos.has_value()) {
  if(const std::optional<Vec3i> validSpawn =
   ...
   respawned->setPositionAndAnglesKeepPrevAngles(static_cast<float>(validSpawn->x) + 0.5f,
                                                 ...
                                                 static_cast<float>(validSpawn->z) + 0.5f,
                                                 0.0f,
                                                 0.0f);
 ...
 if(net::minecraft::world::chunk::ChunkCache* cache = chunkCacheFor(targetWorld); cache != nullptr) {
...
void PlayerManager::changePlayerDimension(::net::minecraft::entity::player::ServerPlayerEntity* player) {
 ...
 if(player->isAlive()) {
  ...
  if(net::minecraft::world::chunk::ChunkCache* cache = chunkCacheFor(newWorld); cache != nullptr) {
...
void PlayerManager::updatePlayerAfterDimensionChange(::net::minecraft::entity::player::ServerPlayerEntity* player) {
 ...
 getChunkMap(player->dimensionId).addPlayer(player);
 if(ServerWorld* world = server_->getWorld(player->dimensionId); world != nullptr) {
  if(net::minecraft::world::chunk::ChunkCache* cache = chunkCacheFor(world); cache != nullptr) {
   cache->loadChunk(static_cast<int>(player->x) >> 4, static_cast<int>(player->z) >> 4);
  }
 }
}
...
void PlayerManager::addTransientOperator(const std::string& name) {
 ops_.insert(toLowerCopy(trimCopy(name)));
}
...
::net::minecraft::entity::player::ServerPlayerEntity* PlayerManager::getPlayer(const std::string& name) {
 for(::net::minecraft::entity::player::ServerPlayerEntity* player : players) {
  if(player != nullptr && player->name.size() == name.size() &&
     std::equal(player->name.begin(), player->name.end(), name.begin(), name.end(), [](char a, char b) {
...
void PlayerManager::updateBlockEntity(int x, int y, int z, block::entity::BlockEntity* blockEntity) {
 if(blockEntity == nullptr || blockEntity->world == nullptr || blockEntity->world->dimension == nullptr) {
  return;
 }
