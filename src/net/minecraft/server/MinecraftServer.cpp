...
MinecraftServer::MinecraftServer()
    : playerManager(this), entityTrackers{EntityTracker(this, 0), EntityTracker(this, -1)} {
}
...
bool MinecraftServer::init() {
 ...
 if(launchConfig_.has_value()) {
  ...
  properties->setProperty("mods-enabled", config.modsEnabled);
 ...
 if(!onlineMode) {
  ...
  ServerLog::LOGGER.log(LogLevel::Warning,
                        "To change this, set \"online-mode\" to \"true\" in server.properties.");
 }
...
void MinecraftServer::loadWorld(const std::filesystem::path& storageRoot,
 ...
 for(int i = 0; i < 2; ++i) {
  ...
  worldEventListeners_[static_cast<std::size_t>(i)] =
 ...
 for(int i = 0; i < 2; ++i) {
  ...
  for(int j = -radius; j <= radius && running; j += 16) {
   for(int k = -radius; k <= radius && running; k += 16) {
    ...
    if(net::minecraft::world::chunk::ChunkCache* cache = dynamic_cast<net::minecraft::world::chunk::ChunkCache*>(world->getChunkSource()); cache != nullptr) {
