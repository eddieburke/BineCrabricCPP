...
public:
  TrackedChunk(ChunkMap* owner, int chunkX, int chunkZ) : owner_(owner), chunkX_(chunkX), chunkZ_(chunkZ), chunkPos_(chunkX, chunkZ) {
  ...
  void removePlayer(ServerPlayerEntity* player) {
    ...
    if(players_.empty()) {
      ...
      if(dirtyBlockCount_ > 0) {
        const auto end = std::remove_if(owner_->chunksToUpdate_.begin(), owner_->chunksToUpdate_.end(),
                                        [&](const std::shared_ptr<TrackedChunk>& tracked) {
                                          return tracked.get() == this;
                                        });
        owner_->chunksToUpdate_.erase(end, owner_->chunksToUpdate_.end());
      }
      if(ServerWorld* world = owner_->getWorld(); world != nullptr) {
        if(auto* chunkCache = dynamic_cast<world::chunk::ServerChunkCache*>(world->getChunkSource()); chunkCache != nullptr) {
          chunkCache->isLoaded(chunkX_, chunkZ_);
        }
      }
    }
  ...
  void updateChunk() {
    ...
    if(dirtyBlockCount_ == 1) {
      ...
      const int blockZ = chunkZ_ * 16 + maxY_;
...
private:
  void sendBlockEntityUpdate(BlockEntity* blockEntity) {
...
ChunkMap::ChunkMap(MinecraftServer* server, int dimensionId, int viewRadius) : server_(server), dimensionId_(dimensionId) {
  if(viewRadius > 15) {
...
void ChunkMap::addPlayer(ServerPlayerEntity* player) {
  ...
  for(int ring = 1; ring <= viewRadius * 2; ++ring) {
    for(int side = 0; side < 2; ++side) {
      ...
      for(int step = 0; step < ring; ++step) {
        getOrCreateChunk(chunkX + (stepX += direction[0]), chunkZ + (stepZ += direction[1]), true)->addPlayer(player);
      }
    }
  }
  directionIndex %= 4;
  for(int edge = 0; edge < viewRadius * 2; ++edge) {
    getOrCreateChunk(chunkX + (stepX += kDirections[static_cast<std::size_t>(directionIndex)][0]),
                     chunkZ + (stepZ += kDirections[static_cast<std::size_t>(directionIndex)][1]), true)
        ->addPlayer(player);
  }
