...
WindowClass::WindowClass(const std::wstring& className, WNDPROC windowProc, HINSTANCE instance, HICON icon,
  ...
  windowClass.hIcon = icon != nullptr ? icon
                                      : LoadIconW(nullptr, reinterpret_cast<LPCWSTR>(kIconApplication));
  windowClass.hCursor = cursor != nullptr ? cursor
                                          : LoadCursorW(nullptr, reinterpret_cast<LPCWSTR>(kCursorArrow));
...
WindowClass::WindowClass(WindowClass&& other) noexcept
    : className_(std::move(other.className_)), atom_(other.atom_), instance_(other.instance_),
