...
constexpr const char* kColorGray = "\xC2\xA7"
                                   "7";
constexpr const char* kColorYellow = "\xC2\xA7"
                                     "e";
constexpr const char* kColorLightPurple = "\xC2\xA7"
                                          "d";
} // namespace
...
void ServerCommandHandler::executeCommand(Command& command) {
  ...
  if(startsWithIgnoreCase(line, "tp ")) {
    ...
    if(sourcePlayer->networkHandler != nullptr) {
      sourcePlayer->networkHandler->teleport(targetPlayer->x, targetPlayer->y, targetPlayer->z, targetPlayer->yaw,
                                             targetPlayer->pitch);
    }
    ...
    try {
      ...
      if(toLowerCopy(method) == "add") {
        for(std::size_t i = 0; i < std::size(server_->worlds); ++i) {
          ServerWorld* serverWorld = server_->worlds[i];
          if(serverWorld != nullptr) {
            serverWorld->synchronizeTimeAndUpdates(serverWorld->getTime() + static_cast<std::uint64_t>(amount));
          }
        }
...
void ServerCommandHandler::executeWhitelist(const std::string& commandUser, const std::string& message,
