...
ConnectionListener::ConnectionListener(MinecraftServer* server, const std::string& bindAddress, int port,
                                       bool onlineMode)
...
void ConnectionListener::tick() {
  ...
  for(std::size_t index = 0; index < playSnapshot.size();) {
    ...
    const bool disconnected = active.handler == nullptr || active.handler->disconnected || active.connection == nullptr ||
