...
void ServerPlayNetworkHandler::disconnect(const std::string& reason) {
  ...
  if(player_ != nullptr && server_ != nullptr) {
    ...
    leftPacket.chatMessage = std::string(
                                 "\xC2\xA7"
                                 "e") +
...
void ServerPlayNetworkHandler::teleport(double x, double y, double z, float yaw, float pitch) {
  ...
  packet.setMove(x, y + static_cast<double>(player_->getEyeHeight()), y, z, yaw, pitch, false);
  sendPacket(packet);
}
...
void ServerPlayNetworkHandler::onPlayerInteractBlock(const PlayerInteractBlockC2SPacket& packet) {
  ...
  if(packet.side == 255) {
    ...
    if(teleported_ &&
       player_->getSquaredDistance(static_cast<double>(packet.x) + 0.5,
                                   static_cast<double>(packet.y) + 0.5,
                                   static_cast<double>(packet.z) + 0.5) < 64.0 &&
       (spawnDistance > 16 || bypassSpawnProtection)) {
      player_->interactionManager.interactBlock(
          player_, serverWorld, itemStack, packet.x, packet.y, packet.z, packet.side);
    }
...
void ServerPlayNetworkHandler::handleInteractEntity(const PlayerInteractEntityC2SPacket& packet) {
  ...
  if(entity != nullptr && player_->canSee(entity) && player_->getSquaredDistance(*entity) < 36.0) {
    mod::EntityInteractEvent event{player_, entity, packet.isLeftClick == 1, false};
    event.sneaking = player_->isSneaking();
    event.stack = player_->inventory.getSelectedItem();
    mod::runtime::setModContext(serverWorld, false, player_);
...
void ServerPlayNetworkHandler::onClickSlot(const ClickSlotC2SPacket& packet) {
  ...
  ItemStack result =
      player_->currentScreenHandler->onSlotClick(packet.slot, packet.button, packet.holdingShift, player_);
