...
EntityTrackerEntry::EntityTrackerEntry(::net::minecraft::entity::Entity* entity, int trackedDistanceIn,
                                       int trackingFrequencyIn, bool alwaysUpdateVelocity)
...
void EntityTrackerEntry::notifyNewLocation(const std::vector<ServerPlayerEntity*>& players) {
  ...
  if(currentTrackedEntity->velocityModified) {
    sendToAround(makeVelocityPacket(currentTrackedEntity->id, currentTrackedEntity->velocityX,
                                    currentTrackedEntity->velocityY, currentTrackedEntity->velocityZ));
...
void EntityTrackerEntry::updateListener(ServerPlayerEntity* player) {
  ...
  if(dx >= static_cast<double>(-trackedDistance) && dx <= static_cast<double>(trackedDistance) &&
    ...
    if(listeners.find(player) == listeners.end()) {
      ...
      if(auto* trackedPlayer = dynamic_cast<::net::minecraft::entity::player::PlayerEntity*>(currentTrackedEntity);
...
std::unique_ptr<Packet> EntityTrackerEntry::createAddEntityPacket() const {
  ...
  if(auto* arrow = dynamic_cast<mcentity::projectile::ArrowEntity*>(currentTrackedEntity)) {
    const int ownerId =
        arrow->owner != nullptr ? dynamic_cast<mcentity::LivingEntity*>(arrow->owner)->id : currentTrackedEntity->id;
