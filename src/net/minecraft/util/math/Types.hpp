...
inline int chunkPosHashCode(int x, int z) noexcept {
    return (x < 0 ? std::numeric_limits<int>::min() : 0) | ((x & 32767) << 16) | (z < 0 ? 32768 : 0) | (z & 32767);
}
...
class JavaRandom {
   public:
    ...
    [[nodiscard]] int nextInt(int bound) const {
        ...
        do {
            bits = next(31);
            value = bits % bound;
        } while (javaIntOverflowIsNegative(static_cast<std::uint32_t>(bits - value) +
   ...
   private:
    ...
    [[nodiscard]] static constexpr int javaIntFromBits(std::uint32_t value) noexcept {
        return (value & 0x80000000U) == 0U ? static_cast<int>(value) : -1 - static_cast<int>(~value);
    }

    [[nodiscard]] static constexpr std::int64_t javaLongFromBits(std::uint64_t value) noexcept {
        return (value & 0x8000000000000000ULL) == 0ULL ? static_cast<std::int64_t>(value)
