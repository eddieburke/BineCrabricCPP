...
public:
    ...
    void create(std::vector<double>& map, double x, double z, int width, int depth, double frequencyX, double frequencyZ, double amplitude) const
    {
        int index = 0;
        for (int ix = 0; ix < width; ++ix) {
            ...
            for (int iz = 0; iz < depth; ++iz) {
                ...
                map[static_cast<std::size_t>(index++)] += 70.0
                    * (corner(gi0, dx0, dz0) + corner(gi1, dx1, dz1) + corner(gi2, dx2, dz2))
                    * amplitude;
            }
        }
    }
...
private:
    ...
    {
        static constexpr std::array<std::array<int, 3>, 12> grads {{
        ...
        t *= t;
        return t * t * (static_cast<double>(grads[static_cast<std::size_t>(gradIndex)][0]) * x
