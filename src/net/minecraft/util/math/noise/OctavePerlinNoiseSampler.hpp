...
public:
  ...
  std::vector<double>& create(std::vector<double>& map, double x, double y, double z, int width, int height,
                              int depth, double scaleX, double scaleY, double scaleZ) const {
    ...
    double frequency = 1.0;
    for(const PerlinNoiseSampler& sampler : samplers_) {
      sampler.create(map, x, y, z, width, height, depth, scaleX * frequency, scaleY * frequency,
                     scaleZ * frequency, frequency);
      frequency /= 2.0;
    }
    return map;
  }
  std::vector<double>& create(std::vector<double>& map, int x, int z, int width, int depth, double scaleX,
                              double scaleZ, double scaleUnused) const {
    (void)scaleUnused;
