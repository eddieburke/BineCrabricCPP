...
bool string(const std::string& text, std::size_t& pos, std::string* out) {
  ...
  while(pos < text.size()) {
    ...
    case 'u': {
      ...
      if(!codeUnit(text, pos, value)) return false;
      if(value >= 0xD800U && value <= 0xDBFFU) {
        if(pos + 2 > text.size() || text[pos] != '\\' || text[pos + 1] != 'u') return false;
        pos += 2;
        std::uint32_t low = 0;
        if(!codeUnit(text, pos, low) || low < 0xDC00U || low > 0xDFFFU) return false;
...
bool skipContainer(const std::string& text, std::size_t& pos, char open, char close) {
  ...
  while(pos < text.size()) {
    if(open == '{') {
      if(!string(text, pos, nullptr)) return false;
      pos = whitespace(text, pos);
      if(pos >= text.size() || text[pos++] != ':') return false;
    }
    if(!skipValue(text, pos)) return false;
    pos = whitespace(text, pos);
    if(pos >= text.size()) return false;
    if(text[pos] == close) {
      ++pos;
...
bool skipValue(const std::string& text, std::size_t& pos) {
  pos = whitespace(text, pos);
  if(pos >= text.size()) return false;
  if(text[pos] == '"') return string(text, pos, nullptr);
  if(text[pos] == '{') return skipContainer(text, pos, '{', '}');
  if(text[pos] == '[') return skipContainer(text, pos, '[', ']');
...
bool range(const std::string& text, const std::string& key, std::size_t& start, std::size_t& end) {
  ...
  while((pos = whitespace(text, pos)) < text.size() && text[pos] != '}') {
    std::string current;
    if(!string(text, pos, &current)) return false;
    pos = whitespace(text, pos);
    if(pos >= text.size() || text[pos++] != ':') return false;
    start = whitespace(text, pos);
    end = start;
    if(!skipValue(text, end)) return false;
...
std::optional<std::string> stringField(const std::string& text, const std::string& key) {
  ...
  if(!range(text, key, start, end)) return std::nullopt;
  std::string value;
...
std::optional<std::string> objectField(const std::string& text, const std::string& key) {
  std::size_t start = 0, end = 0;
  if(!range(text, key, start, end) || start >= text.size() || text[start] != '{') return std::nullopt;
  return text.substr(start, end - start);
}
std::vector<std::string> objectArrayField(const std::string& text, const std::string& key) {
  std::vector<std::string> objects;
  std::size_t start = 0, end = 0;
  if(!range(text, key, start, end) || start >= text.size() || text[start++] != '[') return objects;
  while((start = whitespace(text, start)) < end && text[start] != ']') {
    const std::size_t objectStart = start;
    if(text[start] != '{' || !skipValue(text, start)) return {};
