...
LightningEntity::LightningEntity(World* world, double xIn, double yIn, double zIn) : LightningEntity(world) {
  ...
  if(world != nullptr && world->difficulty >= 2 &&
    ...
    if(Block::FIRE != nullptr && world->getBlockId(bx, by, bz) == 0 &&
       Block::FIRE->canPlaceAt(world, bx, by, bz, 0)) {
    ...
    for(int i = 0; i < 4; ++i) {
      ...
      if(world->getBlockId(fx, fy, fz) != 0 || Block::FIRE == nullptr ||
         !Block::FIRE->canPlaceAt(world, fx, fy, fz, 0)) {
        continue;
      }
...
void LightningEntity::tick() {
  ...
  if(ambientTick < 0) {
    if(remainingActions == 0) {
      ...
      if(world->isRegionLoaded(bx, by, bz, 10) && world->getBlockId(bx, by, bz) == 0 && Block::FIRE != nullptr &&
         Block::FIRE->canPlaceAt(world, bx, by, bz, 0)) {
        world->setBlock(bx, by, bz, Block::FIRE->id);
      }
    }
  ...
  if(ambientTick >= 0) {
    ...
    const std::vector<Entity*> struck =
        world->getEntities(this, Box{x - radius, y - radius, z - radius, x + radius, y + 6.0 + radius, z + radius});
...
MC_REGISTER_ENTITY(LightningEntity)
} // namespace net::minecraft::entity
