...
class Path {
   public:
    ...
    explicit Path(std::vector<PathNode> nodesIn) : nodes(std::move(nodesIn)) {
    ...
    [[nodiscard]] bool isFinished() const {
        return currentNodeIndex >= length;
    }
    ...
    [[nodiscard]] Vec3d getNodePosition(const Entity& entity) const {
        ...
        return Vec3d{
            static_cast<double>(node.x) + offset, static_cast<double>(node.y), static_cast<double>(node.z) + offset};
    }
