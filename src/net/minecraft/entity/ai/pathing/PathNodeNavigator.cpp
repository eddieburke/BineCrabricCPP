...
Path PathNodeNavigator::findPath(Entity* startEntity, int x, int y, int z, float distance) {
  return findPath(startEntity, static_cast<double>(x) + 0.5, static_cast<double>(y) + 0.5,
                  static_cast<double>(z) + 0.5, distance);
}
Path PathNodeNavigator::findPath(Entity* startEntity, double x, double y, double z, float distance) {
  ...
  resetSearch();
  PathNode* startNode =
      getNode(MathHelper::floor(startEntity->boundingBox.minX), MathHelper::floor(startEntity->boundingBox.minY),
              MathHelper::floor(startEntity->boundingBox.minZ));
  PathNode* endNode =
      getNode(MathHelper::floor(x - static_cast<double>(startEntity->width / 2.0f)), MathHelper::floor(y),
