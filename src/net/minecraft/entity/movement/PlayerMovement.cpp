...
float groundSlipperiness(const LivingEntity& entity)
{
    ...
    const int blockId = entity.world->getBlockId(
        MathHelper::floor(entity.x),
        MathHelper::floor(entity.boundingBox.minY) - 1,
        MathHelper::floor(entity.z));
...
void applyFluidWallClimb(LivingEntity& entity, double prevY)
{
    ...
    if (!entity.getEntitiesInside(
            entity.velocityX,
            entity.velocityY + 0.6 - entity.y + prevY,
            entity.velocityZ)) {
        return;
    }
...
void travelWater(LivingEntity& entity, float sideways, float forward)
{
    const double prevY = entity.y;
    entity.moveNonSolid(sideways, forward, 0.02f);
...
void travelLava(LivingEntity& entity, float sideways, float forward)
{
    const double prevY = entity.y;
    entity.moveNonSolid(sideways, forward, 0.02f);
    entity.move(entity.velocityX, entity.velocityY, entity.velocityZ);
...
void travelGround(LivingEntity& entity, float sideways, float forward)
{
    ...
    entity.moveNonSolid(sideways, forward, acceleration);
...
void travelFlyingAir(LivingEntity& entity, float sideways, float forward)
{
    float slipperiness = groundSlipperiness(entity);
    const float acceleration = 0.16277136f / (slipperiness * slipperiness * slipperiness);
    entity.moveNonSolid(sideways, forward, entity.onGround ? 0.1f * acceleration : 0.02f);
...
void PlayerMovement::travel(LivingEntity& entity, float sideways, float forward)
{
    if (entity.world == nullptr) {
        return;
    }
    if (entity.isSubmergedInWater()) {
        travelWater(entity, sideways, forward);
    } else if (entity.isTouchingLava()) {
        travelLava(entity, sideways, forward);
    } else {
        travelGround(entity, sideways, forward);
    }
    updateWalkAnimation(entity);
}

void PlayerMovement::travelFlying(LivingEntity& entity, float sideways, float forward)
{
    if (entity.world == nullptr) {
        return;
    }
    if (entity.isSubmergedInWater()) {
        ...
        travelFlyingAir(entity, sideways, forward);
    }
    updateWalkAnimation(entity);
}
