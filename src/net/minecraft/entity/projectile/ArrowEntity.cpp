...
ArrowEntity::ArrowEntity(World* world, LivingEntity* ownerIn) : Entity(world) {
 ...
 if(owner != nullptr) {
  setPositionAndAnglesKeepPrevAngles(
      owner->x, owner->y + static_cast<double>(owner->getEyeHeight()), owner->z, owner->yaw, owner->pitch);
...
void ArrowEntity::tick() {
 Entity::tick();
 tickArrowProjectile(
     *this, owner, inAirTime, blockX, blockY, blockZ, blockId, blockMeta, inGround, life, shake, pickupAllowed);
}
void ArrowEntity::onPlayerInteraction(player::PlayerEntity* player) {
 ...
 if(inGround && pickupAllowed && shake <= 0) {
  ...
  if(player->inventory.addStack(pickupStack)) {
   world->playSound(
       this, "random.pop", 0.2f, ((random.nextFloat() - random.nextFloat()) * 0.7f + 1.0f) * 2.0f);
