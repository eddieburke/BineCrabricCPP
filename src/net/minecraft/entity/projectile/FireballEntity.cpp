...
FireballEntity::FireballEntity(World* world, double xIn, double yIn, double zIn, double velocityXIn, double velocityYIn,
                               double velocityZIn)
    : Entity(world) {
  setBoundingBoxSpacing(1.0f, 1.0f);
...
FireballEntity::FireballEntity(World* world, LivingEntity* ownerIn, double velocityXIn, double velocityYIn,
                               double velocityZIn)
    : Entity(world) {
  owner = ownerIn;
  setBoundingBoxSpacing(1.0f, 1.0f);
  ...
  if(isSubmergedInWater()) {
    if(world != nullptr) {
      for(int i = 0; i < 4; ++i) {
        ...
        world->addParticle("bubble", x - velocityX * static_cast<double>(offset),
                           y - velocityY * static_cast<double>(offset),
                           z - velocityZ * static_cast<double>(offset), velocityX, velocityY, velocityZ);
      }
    }
    drag = 0.8f;
  }
