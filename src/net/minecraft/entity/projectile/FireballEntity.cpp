...
Entity* findEntityOnPath(Entity& projectile, LivingEntity* owner, int inAirTime, const Vec3d& start, const Vec3d& end)
{
    ...
    const Box searchBox = projectile.boundingBox.stretch(end.x - start.x, end.y - start.y, end.z - start.z).expand(1.0, 1.0, 1.0);
    ...
    : Entity(world)
{
    ...
    setPosition(x, y, z);
    const double length = MathHelper::sqrt(static_cast<float>(velocityXIn * velocityXIn + velocityYIn * velocityYIn + velocityZIn * velocityZIn));
    if (length > 1.0e-4) {
...
FireballEntity::FireballEntity(World* world, LivingEntity* ownerIn, double velocityXIn, double velocityYIn, double velocityZIn)
    : Entity(world)
{
    owner = ownerIn;
    setBoundingBoxSpacing(1.0f, 1.0f);
