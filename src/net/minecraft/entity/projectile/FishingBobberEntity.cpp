...
void updateBobberRotation(FishingBobberEntity& bobber)
{
    const float horizontal = MathHelper::sqrt(
        static_cast<float>(bobber.velocityX * bobber.velocityX + bobber.velocityZ * bobber.velocityZ));
    bobber.yaw = static_cast<float>(std::atan2(bobber.velocityX, bobber.velocityZ) * 180.0 / 3.141592653589793);
    bobber.pitch = static_cast<float>(std::atan2(bobber.velocityY, static_cast<double>(horizontal)) * 180.0 / 3.141592653589793);
...
void FishingBobberEntity::tick()
{
    ...
    for (int i = 0; i < samples; ++i) {
        const double sampleMinY = boundingBox.minY + (boundingBox.maxY - boundingBox.minY) * static_cast<double>(i) / static_cast<double>(samples) - 0.125 + 0.125;
        const double sampleMaxY = boundingBox.minY + (boundingBox.maxY - boundingBox.minY) * static_cast<double>(i + 1) / static_cast<double>(samples) - 0.125 + 0.125;
            ...
            if (random.nextInt(biteDelay) == 0) {
                ...
                for (float particle = 0.0f; particle < 1.0f + width * 20.0f; particle += 1.0f) {
                    ...
                    world->addParticle(
                        ...
                        static_cast<double>(splashY) + 1.0f,
