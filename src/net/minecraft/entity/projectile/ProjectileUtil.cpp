...
namespace {
[[nodiscard]] Entity* findEntityOnPath(
    Entity& projectile, LivingEntity* owner, int inAirTime, const Vec3d& start, const Vec3d& end) {
  ...
  if(entity.isSubmergedInWater()) {
    if(entity.world != nullptr) {
      for(int i = 0; i < 4; ++i) {
        ...
        entity.world->addParticle("bubble",
...
void tickThrownProjectile(Entity& projectile,
  ...
  const Vec3d end{
      projectile.x + projectile.velocityX, projectile.y + projectile.velocityY, projectile.z + projectile.velocityZ};
  ...
  if(hit.has_value()) {
    ...
    if(!projectile.world->isRemote() && spawnChickens && projectile.random.nextInt(8) == 0) {
      ...
      for(int i = 0; i < chickenCount; ++i) {
        ...
        chicken->setPositionAndAnglesKeepPrevAngles(
            projectile.x, projectile.y, projectile.z, projectile.yaw, 0.0f);
    ...
    if(hit->entity != nullptr) {
      if(hit->entity->damage(owner, 4)) {
        projectile.world->playSound(
            &projectile, "random.drr", 1.0f, 1.2f / (projectile.random.nextFloat() * 0.2f + 0.9f));
        projectile.markDead();
      } else {
