...
[[nodiscard]] Entity* findEntityOnPath(Entity& projectile, LivingEntity* owner, int inAirTime, const Vec3d& start,
  ...
  const Box searchBox = projectile.boundingBox.stretch(end.x - start.x, end.y - start.y, end.z - start.z).expand(1.0);
  ...
  if(entity.isSubmergedInWater()) {
    if(entity.world != nullptr) {
      for(int i = 0; i < 4; ++i) {
        ...
        entity.world->addParticle("bubble", entity.x - entity.velocityX * static_cast<double>(offset),
                                  entity.y - entity.velocityY * static_cast<double>(offset),
                                  entity.z - entity.velocityZ * static_cast<double>(offset), entity.velocityX,
                                  entity.velocityY, entity.velocityZ);
      }
    }
    drag = 0.8f;
  }
...
void tickThrownProjectile(Entity& projectile, LivingEntity* owner, int& inAirTime, int& blockX, int& blockY,
                          int& blockZ, int& blockId, bool& inGround, int& removalTimer, int& shake, bool spawnChickens) {
  if(projectile.world == nullptr) {
    return;
  }
  ...
  const Vec3d end{projectile.x + projectile.velocityX, projectile.y + projectile.velocityY,
  ...
  if(hit.has_value()) {
    ...
    if(!projectile.world->isRemote() && spawnChickens && projectile.random.nextInt(8) == 0) {
      ...
      for(int i = 0; i < chickenCount; ++i) {
        ...
        chicken->setPositionAndAnglesKeepPrevAngles(projectile.x, projectile.y, projectile.z, projectile.yaw,
                                                    0.0f);
...
void tickArrowProjectile(Entity& projectile, LivingEntity*& owner, int& inAirTime, int& blockX, int& blockY,
                         int& blockZ, int& blockId, int& blockMeta, bool& inGround, int& life, int& shake,
                         bool pickupAllowed) {
  if(projectile.world == nullptr) {
    return;
  }
  if(projectile.prevPitch == 0.0f && projectile.prevYaw == 0.0f) {
    const float horizontal = MathHelper::sqrt(static_cast<float>(projectile.velocityX * projectile.velocityX +
    ...
    projectile.prevPitch = projectile.pitch = static_cast<float>(
        std::atan2(projectile.velocityY, static_cast<double>(horizontal)) * 180.0 / 3.141592653589793);
  }
  ...
  if(embeddedBlockId > 0 && embeddedBlockId < Block::BLOCK_COUNT &&
    ...
    if(shape.has_value() && projectile.x >= shape->minX && projectile.x <= shape->maxX &&
       projectile.y >= shape->minY && projectile.y <= shape->maxY && projectile.z >= shape->minZ &&
       projectile.z <= shape->maxZ) {
      inGround = true;
    }
  }
  if(shake > 0) {
  ...
  if(hit.has_value()) {
    if(hit->entity != nullptr) {
      if(hit->entity->damage(owner, 4)) {
        projectile.world->playSound(&projectile, "random.drr", 1.0f,
      ...
      const float length = MathHelper::sqrt(static_cast<float>(projectile.velocityX * projectile.velocityX +
