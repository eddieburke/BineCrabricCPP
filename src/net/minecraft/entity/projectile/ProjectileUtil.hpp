...
inline void setProjectileVelocity(Entity& entity, double dirX, double dirY, double dirZ, float speed, float divergence)
{
    ...
    const float horizontal = MathHelper::sqrt(static_cast<float>(entity.velocityX * entity.velocityX + entity.velocityZ * entity.velocityZ));
    entity.prevYaw = entity.yaw = static_cast<float>(std::atan2(entity.velocityX, entity.velocityZ) * 180.0 / 3.141592653589793);
    entity.prevPitch = entity.pitch = static_cast<float>(std::atan2(entity.velocityY, static_cast<double>(horizontal)) * 180.0 / 3.141592653589793);
}
