...
bool isHostileMobDamageSource(const Entity& source) {
 ...
 return id == "Monster" || id == "Creeper" || id == "Skeleton" || id == "Spider" || id == "Giant" ||
        id == "Zombie" || id == "Slime" || id == "Ghast" || id == "PigZombie";
}
...
PlayerEntity::PlayerEntity(World* world)
 ...
 if(world != nullptr) {
  ...
  double spawnY = static_cast<double>(spawn.y) + 1.0;
  // Integrated SP used fixed Y=65; that traps the player in cave air pockets when
  // the stored spawn Y is sea-level but the column is solid above. Match server join:
  // stand on the first solid from the top.
  if(!world->isRemote()) {
   const int validY = world->getSpawnPositionValidityY(spawn.x, spawn.z);
   if(validY > 0) {
    spawnY = static_cast<double>(validY);
   }
  }
  setPositionAndAnglesKeepPrevAngles(
      static_cast<double>(spawn.x) + 0.5, spawnY, static_cast<double>(spawn.z) + 0.5, 0.0f, 0.0f);
 }
 health = 20;
 maxHealth = 20;
 modelName = "humanoid";
...
void PlayerEntity::dropItem(ItemStack stack, bool throwRandomly) {
 ...
 if(throwRandomly) {
  ...
  entity->velocityX += MathHelper::cos(angle) * static_cast<double>(impulse);
  entity->velocityY += static_cast<double>((random.nextFloat() - random.nextFloat()) * 0.1f);
  entity->velocityZ += MathHelper::sin(angle) * static_cast<double>(impulse);
 }
 spawnItem(entity);
 increaseStat(stat::Stats::DROP.id, 1);
}
void PlayerEntity::spawnItem(ItemEntity* itemEntity) {
 if(world != nullptr && itemEntity != nullptr) {
...
void PlayerEntity::writeNbt(NbtCompound& nbt) const {
 ...
 nbt.putInt("SelectedItemSlot", std::clamp(inventory.selectedSlot, 0, 8));
...
void PlayerEntity::attack(Entity* target) {
 ...
 const bool critical = velocityY < 0.0;
 if(critical) {
  ++amount;
 }
 mod::AttackDamageEvent attackEvent{this, target, amount, critical, false};
 attackEvent.fallDistance = fallDistance;
 attackEvent.onGround = onGround;
 attackEvent.targetX = target->x;
 attackEvent.targetY = target->y + static_cast<double>(target->height) * 0.5;
 attackEvent.targetZ = target->z;
 net::minecraft::mod::runtime::luaHookAttackDamage(attackEvent);
 if(attackEvent.canceled || attackEvent.damage <= 0) {
  return;
 }
...
SleepAttemptResult PlayerEntity::trySleep(int xIn, int yIn, int zIn) {
 ...
 if(world != nullptr && world->isPosLoaded(xIn, yIn, zIn)) {
  ...
  setPosition(
      static_cast<float>(xIn) + offsetX, static_cast<float>(yIn) + 0.9375f, static_cast<float>(zIn) + offsetZ);
 } else {
...
void PlayerEntity::wakeUp(bool resetSleepTimer, bool updateSleepingPlayers, bool setSpawnPosFlag) {
 ...
 if(sleepingPos.has_value() && world != nullptr) {
  ...
  setPosition(static_cast<float>(wakePos->x) + 0.5f,
...
void PlayerEntity::updateCapeUrl() {
 if(profileCapeUrl.empty()) {
  playerCapeUrl = msauth::legacyCapeUrl(name);
 } else {
  playerCapeUrl = profileCapeUrl;
 }
...
void PlayerEntity::travel(float sideways, float forward) {
 ...
 const double startY = y;
 const double startZ = z;
 mod::PlayerTravelEvent event{this, sideways, forward};
 net::minecraft::mod::runtime::luaHookPlayerTravel(event);
 movement::PlayerMovement::travel(*this, event.sideways, event.forward, event.speedMultiplier);
...
void PlayerEntity::updateMovementStats(double dx, double dy, double dz) {
 ...
 if(isInFluid(block::material::Material::WATER)) {
  const int distance =
      static_cast<int>(std::round(MathHelper::sqrt(static_cast<float>(dx * dx + dy * dy + dz * dz)) * 100.0f));
  if(distance > 0) {
   increaseStat(stat::Stats::DIVE_ONE_CM.id, distance);
  }
 } else if(isSubmergedInWater()) {
  const int distance =
      static_cast<int>(std::round(MathHelper::sqrt(static_cast<float>(dx * dx + dz * dz)) * 100.0f));
  if(distance > 0) {
...
void PlayerEntity::increaseRidingMotionStats(double deltaX, double deltaY, double deltaZ) {
 ...
 const int distance = static_cast<int>(
     std::round(MathHelper::sqrt(static_cast<float>(deltaX * deltaX + deltaY * deltaY + deltaZ * deltaZ)) * 100.0f));
 if(distance <= 0) {
  return;
 }
 ...
 if(vehicleId == "Minecart" || dynamic_cast<entity::vehicle::MinecartEntity*>(vehicle) != nullptr) {
  ...
  if(!ridingStartPos.has_value()) {
   ...
   const double railDistance =
       MathHelper::sqrt(static_cast<float>(startDx * startDx + startDy * startDy + startDz * startDz));
