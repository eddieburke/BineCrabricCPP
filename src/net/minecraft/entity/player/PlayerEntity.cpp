...
PlayerEntity::PlayerEntity(World* world)
    : LivingEntity(world), inventory(this), playerScreenHandler(&inventory, world == nullptr || !world->isRemote()),
      currentScreenHandler(&playerScreenHandler) {
  initDataTracker();
  standingEyeHeight = 1.62f;
  if(world != nullptr) {
    const Vec3i spawn = world->getSpawnPos();
    setPositionAndAnglesKeepPrevAngles(static_cast<double>(spawn.x) + 0.5, static_cast<double>(spawn.y) + 1.0,
                                       static_cast<double>(spawn.z) + 0.5, 0.0f, 0.0f);
  }
  health = 20;
  maxHealth = 20;
  modelName = "humanoid";
...
SleepAttemptResult PlayerEntity::trySleep(int xIn, int yIn, int zIn) {
  ...
  if(world != nullptr && world->isPosLoaded(xIn, yIn, zIn)) {
    ...
    setPosition(static_cast<float>(xIn) + offsetX, static_cast<float>(yIn) + 0.9375f,
                static_cast<float>(zIn) + offsetZ);
  } else {
...
void PlayerEntity::wakeUp(bool resetSleepTimer, bool updateSleepingPlayers, bool setSpawnPosFlag) {
  ...
  if(sleepingPos.has_value() && world != nullptr) {
    ...
    setPosition(static_cast<float>(wakePos->x) + 0.5f, static_cast<float>(wakePos->y) + standingEyeHeight + 0.1f,
