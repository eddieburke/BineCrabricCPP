...
void ServerPlayerEntity::openChestScreen(int xIn, int yIn, int zIn) {
 ...
 if(world->getBlockId(xIn - 1, yIn, zIn) == chestId) {
  ownedScreenInventory_ =
      std::make_unique<DoubleInventory>("Large chest",
                                        world->getBlockEntity(xIn - 1, yIn, zIn) != nullptr
                                            ? dynamic_cast<Inventory*>(world->getBlockEntity(xIn - 1, yIn, zIn))
                                            : nullptr,
                                        chest);
