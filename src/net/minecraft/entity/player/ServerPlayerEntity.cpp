...
ServerPlayerEntity::ServerPlayerEntity(server::MinecraftServer* serverIn, World* world, const std::string& nameIn,
  ...
  if(world != nullptr) {
    ...
    setPositionAndAnglesKeepPrevAngles(static_cast<double>(spawnX) + 0.5, static_cast<double>(spawnY),
                                       static_cast<double>(spawnZ) + 0.5, 0.0f, 0.0f);
  }
  server = serverIn;
  stepHeight = 0.0f;
  name = nameIn;
...
void ServerPlayerEntity::onDisconnect() {
  ...
  if(passenger != nullptr) {
    passenger->setVehicle(passenger);
  }
  if(isSleeping()) {
...
void ServerPlayerEntity::updateInput(float sidewaysSpeedIn, float forwardSpeedIn, bool jumpingIn, bool sneakingIn,
                                     float pitchIn, float yawIn) {
