...
void ClientPlayerEntity::openChestScreen(Inventory* inventoryIn)
{
    ...
    minecraft_->setScreen(
        std::make_unique<client::gui::screen::ingame::DoubleChestScreen>(&inventory, inventoryIn));
}
...
void ClientPlayerEntity::openChestScreen(int xIn, int yIn, int zIn)
{
    ...
    minecraft_->setScreen(
        std::make_unique<client::gui::screen::ingame::DoubleChestScreen>(&inventory, chestInventory));
}
...
void ClientPlayerEntity::openFurnaceScreen(block::entity::FurnaceBlockEntity* furnaceIn)
{
    ...
    minecraft_->setScreen(
        std::make_unique<client::gui::screen::ingame::FurnaceScreen>(&inventory, furnaceIn));
}
...
void ClientPlayerEntity::openDispenserScreen(block::entity::DispenserBlockEntity* dispenserIn)
{
    ...
    minecraft_->setScreen(
        std::make_unique<client::gui::screen::ingame::DispenserScreen>(&inventory, dispenserIn));
}
...
void ClientPlayerEntity::openCraftingScreen(int xIn, int yIn, int zIn)
{
    ...
    minecraft_->setScreen(
        std::make_unique<client::gui::screen::ingame::CraftingScreen>(&inventory, xIn, yIn, zIn));
}
...
bool ClientPlayerEntity::pushOutOfBlock(double px, double py, double pz)
{
    ...
    if (shouldSuffocate(blockX, blockY, blockZ) || shouldSuffocate(blockX, blockY + 1, blockZ)) {
        const bool westFree = !shouldSuffocate(blockX - 1, blockY, blockZ) && !shouldSuffocate(blockX - 1, blockY + 1, blockZ);
        const bool eastFree = !shouldSuffocate(blockX + 1, blockY, blockZ) && !shouldSuffocate(blockX + 1, blockY + 1, blockZ);
        const bool northFree = !shouldSuffocate(blockX, blockY, blockZ - 1) && !shouldSuffocate(blockX, blockY + 1, blockZ - 1);
        const bool southFree = !shouldSuffocate(blockX, blockY, blockZ + 1) && !shouldSuffocate(blockX, blockY + 1, blockZ + 1);
        int direction = -1;
