  ...
  if(onGround && !wasOnGround) {
    ...
    if(world != nullptr) {
      for(int i = 0; i < size * 8; ++i) {
        ...
        world->addParticle("slime", x + static_cast<double>(offsetX), boundingBox.minY,
                           z + static_cast<double>(offsetZ), 0.0, 0.0, 0.0);
      }
      if(size > 2) {
        world->playSound(this, "mob.slime", getSoundVolume(),
...
void SlimeEntity::markDead() {
  ...
  if(world != nullptr && !world->isRemote() && size > 1 && health == 0) {
    for(int i = 0; i < 4; ++i) {
      ...
      child->setPositionAndAnglesKeepPrevAngles(x + static_cast<double>(offsetX), y + 0.5,
                                                z + static_cast<double>(offsetZ), random.nextFloat() * 360.0f,
                                                0.0f);
...
bool SlimeEntity::canSpawn() const {
  ...
  return (getSize() == 1 || world->difficulty > 0) && random.nextInt(10) == 0 &&
         chunk.getSlimeRandom(987234911LL).nextInt(10) == 0 && y < 16.0;
}
