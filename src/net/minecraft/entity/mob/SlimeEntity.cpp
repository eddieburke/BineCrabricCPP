...
void SlimeEntity::tickLiving() {
  ...
  if(onGround && --ticksUntilJump <= 0) {
    ...
    if(getSize() > 1 && world != nullptr) {
      world->playSound(
          this, "mob.slime", getSoundVolume(), ((random.nextFloat() - random.nextFloat()) * 0.2f + 1.0f) * 0.8f);
    }
    stretch = 1.0f;
...
void SlimeEntity::markDead() {
  ...
  if(world != nullptr && !world->isRemote() && size > 1 && health == 0) {
    for(int i = 0; i < 4; ++i) {
      ...
      child->setPositionAndAnglesKeepPrevAngles(x + static_cast<double>(offsetX),
                                                y + 0.5,
                                                z + static_cast<double>(offsetZ),
