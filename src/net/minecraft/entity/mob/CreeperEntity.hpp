...
public:
    ...
    {
        return (static_cast<float>(lastFuseTime)
                   + static_cast<float>(fuseTime - lastFuseTime) * tickDelta)
            / 28.0f;
    }

    [[nodiscard]] bool isCharged() const { return dataTracker.getByte(17) == 1; }

    [[nodiscard]] std::string getHurtSound() const override { return "mob.creeper"; }
    [[nodiscard]] std::string getDeathSound() const override { return "mob.creeperdeath"; }
...
private:
    [[nodiscard]] int getFuseSpeed() const { return dataTracker.getByte(16); }
    void setFuseSpeed(int fuseSpeed) { dataTracker.set(16, static_cast<std::int8_t>(fuseSpeed)); }
};
