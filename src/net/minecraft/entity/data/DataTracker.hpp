...
public:
    ...
    void writeUpdatedEntries(const std::vector<DataTrackerEntry>& entries)
    {
        for (const DataTrackerEntry& entry : entries) {
...
private:
    ...
    static std::int8_t normalizeValue(std::int8_t value) { return value; }
    static std::int16_t normalizeValue(std::int16_t value) { return value; }
    static std::int32_t normalizeValue(std::int32_t value) { return value; }
    static float normalizeValue(float value) { return value; }
    static std::string normalizeValue(const std::string& value) { return value; }
    static std::string normalizeValue(std::string&& value) { return std::move(value); }
    static ItemStack normalizeValue(const ItemStack& value) { return value; }
    static ItemStack normalizeValue(ItemStack&& value) { return std::move(value); }
    static Vec3i normalizeValue(const Vec3i& value) { return value; }
    static Vec3i normalizeValue(Vec3i&& value) { return std::move(value); }

    static void writeEntry(std::ostream& output, const DataTrackerEntry& entry)
    {
        const std::uint8_t header = static_cast<std::uint8_t>(((entry.getDataTypeId() << 5) | (entry.getId() & 0x1F)) & 0xFF);
