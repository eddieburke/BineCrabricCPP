...
bool SquidEntity::isSubmergedInWater() const {
  ...
  return world->updateMovementInFluid(boundingBox.expand(0.0, -0.6, 0.0), block::material::Material::WATER,
...
void SquidEntity::tickMovement() {
  ...
  if(isSubmergedInWater()) {
    ...
    tiltAngle +=
        (-static_cast<float>(std::atan2(horizontal, velocityY) * 180.0 / 3.141592653589793) - tiltAngle) * 0.1f;
  } else {
