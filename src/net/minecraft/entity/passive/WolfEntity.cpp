...
void WolfEntity::tick() {
  ...
  if(isWet()) {
    ...
    if(shakeProgress == 0.0f && world != nullptr) {
      world->playSound(
          this, "mob.wolf.shake", getSoundVolume(), (random.nextFloat() - random.nextFloat()) * 0.2f + 1.0f);
    }
    ...
    if(candidate.length == 0 && distance > 12.0f) {
      ...
      for(int i = 0; i <= 4; ++i) {
        for(int j = 0; j <= 4; ++j) {
          ...
          setPositionAndAnglesKeepPrevAngles(static_cast<double>(baseX + i) + 0.5,
                                             static_cast<double>(baseY),
                                             static_cast<double>(baseZ + j) + 0.5,
                                             yaw,
                                             pitch);
          return;
        }
