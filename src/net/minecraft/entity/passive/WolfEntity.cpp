...
void WolfEntity::attack(Entity* other, float distance)
{
    ...
    } else if (distance < 1.5f && other != nullptr
        && other->boundingBox.maxY > boundingBox.minY && other->boundingBox.minY < boundingBox.maxY) {
        attackCooldown = 20;
        ...
        if (candidate.length == 0 && distance > 12.0f) {
            ...
            for (int i = 0; i <= 4; ++i) {
                for (int j = 0; j <= 4; ++j) {
                    ...
                    setPositionAndAnglesKeepPrevAngles(static_cast<double>(baseX + i) + 0.5, static_cast<double>(baseY), static_cast<double>(baseZ + j) + 0.5, yaw, pitch);
                    return;
                }
            }
...
bool WolfEntity::damage(Entity* damageSource, int amount)
{
    ...
    if (!isTamed() && !isAngry()) {
        ...
        if (auto* arrow = dynamic_cast<projectile::ArrowEntity*>(attacker); arrow != nullptr && arrow->owner != nullptr) {
