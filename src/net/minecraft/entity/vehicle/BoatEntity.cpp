...
void BoatEntity::tick() {
  ...
  if(world->isRemote()) {
    if(clientInterpolationSteps > 0) {
      ...
      yaw =
          static_cast<float>(static_cast<double>(yaw) + yawDelta / static_cast<double>(clientInterpolationSteps));
...
void BoatEntity::updatePassengerPosition() {
  ...
  passenger->setPosition(
      x + offsetX, y + getPassengerRidingHeight() + static_cast<double>(passenger->standingEyeHeight), z + offsetZ);
}
