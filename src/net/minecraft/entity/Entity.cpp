...
void Entity::setPosition(double xIn, double yIn, double zIn) {
  ...
  boundingBox = {x - static_cast<double>(halfWidth),
...
void Entity::onCollision(Entity* otherEntity) {
  ...
  dx *= 0.05;
  dz *= 0.05;
  addVelocity(-(dx *= static_cast<double>(1.0f - pushSpeedReduction)), 0.0,
...
bool Entity::shouldRender(double distance) const {
  const double sideLength = std::max({boundingBox.maxX - boundingBox.minX, boundingBox.maxY - boundingBox.minY,
...
void Entity::setVehicle(Entity* entity) {
  ...
  if(vehicle == entity) {
    vehicle->passenger = nullptr;
    vehicle = nullptr;
    setPositionAndAnglesKeepPrevAngles(entity->x, entity->boundingBox.minY + static_cast<double>(entity->height),
                                       entity->z, yaw, pitch);
    return;
  }
  if(vehicle != nullptr) {
...
void Entity::baseTick() {
  ...
  if(checkWaterCollisions()) {
    if(!submergedInWater && !firstTick && world != nullptr) {
      ...
      world->playSound(this, "random.splash", splashVolume,
      ...
      for(int i = 0; i < 1 + static_cast<int>(width * 20.0f); ++i) {
        ...
        world->addParticle("bubble", x + static_cast<double>(offsetX), static_cast<double>(surfaceY + 1.0f),
      ...
      for(int i = 0; i < 1 + static_cast<int>(width * 20.0f); ++i) {
        const float offsetX = (random.nextFloat() * 2.0f - 1.0f) * width;
        const float offsetZ = (random.nextFloat() * 2.0f - 1.0f) * width;
        world->addParticle("splash", x + static_cast<double>(offsetX), static_cast<double>(surfaceY + 1.0f),
...
bool Entity::checkWaterCollisions() {
  ...
  return world->updateMovementInFluid(boundingBox.expand(0.0, -0.4, 0.0).contract(0.001, 0.001, 0.001),
                                      block::material::Material::WATER, this);
}
...
float Entity::getBrightnessAtEyes(float tickDelta) const {
  ...
  if(!world->isRegionLoaded(MathHelper::floor(boundingBox.minX), MathHelper::floor(boundingBox.minY),
                            MathHelper::floor(boundingBox.minZ), MathHelper::floor(boundingBox.maxX),
                            MathHelper::floor(boundingBox.maxY), MathHelper::floor(boundingBox.maxZ))) {
    return minBrightness;
  }
...
void Entity::setPositionAndAnglesAvoidEntities(double xIn, double yIn, double zIn, float yawIn, float pitchIn,
