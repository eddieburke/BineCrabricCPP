...
float Entity::getDistance(const Entity& entity) const
{
    return MathHelper::sqrt(
        static_cast<float>((x - entity.x) * (x - entity.x)
...
void Entity::onCollision(Entity* otherEntity)
{
    ...
    dx *= 0.05;
    dz *= 0.05;
    addVelocity(-(dx *= static_cast<double>(1.0f - pushSpeedReduction)), 0.0, -(dz *= static_cast<double>(1.0f - pushSpeedReduction)));
...
{
    const double sideLength = std::max({boundingBox.maxX - boundingBox.minX, boundingBox.maxY - boundingBox.minY, boundingBox.maxZ - boundingBox.minZ});
...
{
    nbt.put("Pos", toNbtList(x, y + static_cast<double>(cameraOffset), z));
...
void Entity::readNbt(const NbtCompound& nbt)
{
    ...
    if (const Nbt* motion = storage.get("Motion"); motion != nullptr && motion->isList() && motion->asList().size() >= 3) {
    ...
    if (const Nbt* rotation = storage.get("Rotation"); rotation != nullptr && rotation->isList() && rotation->asList().size() >= 2) {
...
void Entity::setVehicle(Entity* entity)
{
    ...
    if (entity == nullptr) {
        if (vehicle != nullptr) {
            setPositionAndAnglesKeepPrevAngles(vehicle->x, vehicle->boundingBox.minY + static_cast<double>(vehicle->height), vehicle->z, yaw, pitch);
    ...
    if (vehicle == entity) {
        vehicle->passenger = nullptr;
        vehicle = nullptr;
        setPositionAndAnglesKeepPrevAngles(entity->x, entity->boundingBox.minY + static_cast<double>(entity->height), entity->z, yaw, pitch);
        return;
    }
    if (vehicle != nullptr) {
        vehicle->passenger = nullptr;
...
void Entity::baseTick()
{
    ...
    if (checkWaterCollisions()) {
        if (!submergedInWater && !firstTick && world != nullptr) {
            float splashVolume = MathHelper::sqrt(
                static_cast<float>(velocityX * velocityX * 0.2 + velocityY * velocityY + velocityZ * velocityZ * 0.2))
                * 0.2f;
...
bool Entity::checkWaterCollisions()
{
    ...
    return world->updateMovementInFluid(
        boundingBox.expand(0.0, -0.4, 0.0).contract(0.001, 0.001, 0.001),
...
bool Entity::isWet() const
{
    ...
    return submergedInWater
        || world->isRaining(MathHelper::floor(x), MathHelper::floor(y), MathHelper::floor(z));
}
...
float Entity::getBrightnessAtEyes(float tickDelta) const
{
    ...
    if (!world->isRegionLoaded(
            MathHelper::floor(boundingBox.minX), MathHelper::floor(boundingBox.minY), MathHelper::floor(boundingBox.minZ),
            MathHelper::floor(boundingBox.maxX), MathHelper::floor(boundingBox.maxY), MathHelper::floor(boundingBox.maxZ))) {
        return minBrightness;
    }
