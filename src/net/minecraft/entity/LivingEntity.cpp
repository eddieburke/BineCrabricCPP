...
bool LivingEntity::damage(Entity* damageSource, int amount) {
    ...
    if (health <= 0) {
        if (updateEffects && world != nullptr) {
            ...
            if (!deathSound.empty()) {
                world->playSound(
                    this, deathSound, getSoundVolume(), (random.nextFloat() - random.nextFloat()) * 0.2f + 1.0f);
            }
        }
        ...
        if (!hurtSound.empty()) {
            world->playSound(
                this, hurtSound, getSoundVolume(), (random.nextFloat() - random.nextFloat()) * 0.2f + 1.0f);
        }
    }
    return true;
}
...
void LivingEntity::setPositionAndAnglesAvoidEntities(
...
Vec3d LivingEntity::getPosition(float tickDelta) const {
    ...
    return {prevX + (x - prevX) * static_cast<double>(tickDelta),
