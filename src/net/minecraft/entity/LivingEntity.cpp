...
void LivingEntity::baseTick() {
  ...
  if(isAlive() && world != nullptr && isInFluid(block::material::Material::WATER) && !canBreatheInWater()) {
    --air;
    if(air == -20) {
      air = 0;
      for(int i = 0; i < 8; ++i) {
        ...
        world->addParticle("bubble", x + static_cast<double>(offsetX), y + static_cast<double>(offsetY),
    ...
    if(deathTime > 20) {
      ...
      if(world != nullptr) {
        for(int i = 0; i < 20; ++i) {
          ...
          world->addParticle("explode", x + static_cast<double>(random.nextFloat() * width * 2.0f - width),
                             y + static_cast<double>(random.nextFloat() * height),
                             z + static_cast<double>(random.nextFloat() * width * 2.0f - width), particleDx,
                             particleDy, particleDz);
        }
      }
      markDead();
    }
...
bool LivingEntity::damage(Entity* damageSource, int amount) {
  ...
  if(health <= 0) {
    if(updateEffects && world != nullptr) {
      ...
      if(!deathSound.empty()) {
        world->playSound(this, deathSound, getSoundVolume(),
    ...
    if(!hurtSound.empty()) {
      world->playSound(this, hurtSound, getSoundVolume(),
                       (random.nextFloat() - random.nextFloat()) * 0.2f + 1.0f);
    }
  }
  return true;
...
void LivingEntity::animateSpawn() {
  ...
  for(int i = 0; i < 20; ++i) {
    ...
    world->addParticle("explode",
                       ...
                       particleDx, particleDy, particleDz);
  }
}
...
void LivingEntity::onLanding(float fallDistanceIn) {
  ...
  if(damageAmount > 0) {
    ...
    if(world != nullptr) {
      const int blockId = world->getBlockId(MathHelper::floor(x),
      ...
      if(blockId > 0 && blockId < Block::BLOCK_COUNT &&
        ...
        block::sounds::playLanding(world, this, Block::BLOCKS[static_cast<std::size_t>(blockId)]);
      }
    }
  }
}
...
void LivingEntity::setPositionAndAnglesAvoidEntities(double xIn, double yIn, double zIn, float yawIn, float pitchIn,
...
void LivingEntity::tickMovement() {
  if(bodyTrackingIncrements > 0) {
    ...
    pitch = static_cast<float>(static_cast<double>(pitch) + (lerpPitch - static_cast<double>(pitch)) /
                                                                static_cast<double>(bodyTrackingIncrements));
...
void LivingEntity::tickLiving() {
  ...
  if(lookTarget != nullptr) {
    ...
    if(--lookTimer <= 0 || lookTarget->dead ||
       lookTarget->getSquaredDistance(*this) > static_cast<double>(lookRange * lookRange)) {
      lookTarget = nullptr;
    }
  } else {
...
void LivingEntity::lookAt(Entity* target, float maxPitch, float maxYaw) {
  ...
  if(const auto* livingTarget = dynamic_cast<LivingEntity*>(target); livingTarget != nullptr) {
    dy = y + static_cast<double>(getEyeHeight()) -
         (livingTarget->y + static_cast<double>(livingTarget->getEyeHeight()));
  } else {
...
Vec3d LivingEntity::getPosition(float tickDelta) const {
  ...
  return {prevX + (x - prevX) * static_cast<double>(tickDelta), prevY + (y - prevY) * static_cast<double>(tickDelta),
