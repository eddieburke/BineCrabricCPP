...
void Entity::move(double dx, double dy, double dz) {
 ...
 if(stepHeight > 0.0f && canStep && (sneaking || cameraOffset < 0.05f) && (intendedDx != dx || intendedDz != dz)) {
 ...
 if(bypassesSteppingEffects() && !sneaking && vehicle == nullptr) {
  horizontalSpeed =
      static_cast<float>(static_cast<double>(horizontalSpeed) +
                         MathHelper::sqrt(static_cast<float>(movedX * movedX + movedZ * movedZ)) * 0.6f);
  const int blockX = MathHelper::floor(x);
  ...
  if(horizontalSpeed > static_cast<float>(nextStepSoundDistance) && blockId > 0 &&
   ...
   if(block != nullptr) {
    ...
    if(Block::SNOW != nullptr && world->getBlockId(blockX, blockY + 1, blockZ) == Block::SNOW->id) {
     block::sounds::playStep(world, this, Block::SNOW);
    } else if(!block->material.isFluid()) {
     block::sounds::playStep(world, this, block);
    }
 ...
 if(world->isRegionLoaded(neighborMinX, neighborMinY, neighborMinZ, neighborMaxX, neighborMaxY, neighborMaxZ)) {
