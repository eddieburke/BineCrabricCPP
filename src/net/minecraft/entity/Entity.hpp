...
public:
    ...
    [[nodiscard]] virtual std::type_index runtimeType() const { return typeid(*this); }
    ...
    void setWorld(World* worldIn) noexcept { world = worldIn; }

    [[nodiscard]] Vec3d position() const noexcept { return {x, y, z}; }
    [[nodiscard]] Vec3d velocity() const noexcept { return {velocityX, velocityY, velocityZ}; }
    [[nodiscard]] DataTracker& getDataTracker() noexcept { return dataTracker; }
    [[nodiscard]] const DataTracker& getDataTracker() const noexcept { return dataTracker; }

    virtual void initDataTracker() {}
    virtual void teleportTop();
    virtual void markDead() { dead = true; }
    ...
    virtual void setPositionAndAnglesAvoidEntities(
    ...
    [[nodiscard]] virtual float getEyeHeight() const { return 0.0f; }
    [[nodiscard]] virtual float getBrightnessAtEyes(float tickDelta) const;
    [[nodiscard]] virtual bool isSubmergedInWater() const { return submergedInWater; }
    ...
    [[nodiscard]] virtual float getTargetingMargin() const { return 0.1f; }
    [[nodiscard]] virtual bool isPushable() const { return false; }
    [[nodiscard]] virtual std::optional<Box> getBoundingBox() const { return std::nullopt; }
    [[nodiscard]] virtual std::optional<Box> getCollisionAgainstShape(Entity* other) const;
    [[nodiscard]] virtual std::string getTexture() const { return {}; }
    ...
    virtual void scheduleVelocityUpdate() { velocityModified = true; }
    ...
    [[nodiscard]] virtual float getShadowRadius() const { return height * 0.5f; }
    [[nodiscard]] virtual bool isAlive() const { return !dead; }
    ...
    [[nodiscard]] double getStandingEyeHeight() const { return standingEyeHeight; }
    [[nodiscard]] virtual double getPassengerRidingHeight() const { return static_cast<double>(height) * 0.75; }
    ...
    [[nodiscard]] virtual std::vector<ItemStack> getEquipment() const { return {}; }
