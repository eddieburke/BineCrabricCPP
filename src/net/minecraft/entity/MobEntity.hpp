...
public:
    ...
    [[nodiscard]] bool hasPath() const { return path != nullptr; }
    void setPath(std::unique_ptr<ai::pathing::Path> newPath) { path = std::move(newPath); }
    [[nodiscard]] Entity* getTarget() const { return target; }
    void setTarget(Entity* newTarget) { target = newTarget; }

    void tickLiving() override;

protected:
    [[nodiscard]] virtual bool isMovementBlocked() const { return false; }
    [[nodiscard]] virtual Entity* getTargetInRange() { return nullptr; }
