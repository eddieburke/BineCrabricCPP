...
void applyPersistedValue(ModSettingDef& setting, const std::string& value) {
 ...
 } else if(setting.kind == ModSettingDef::Options) {
  if(!setting.options.empty()) {
   auto it = std::find(setting.options.begin(), setting.options.end(), value);
   if(it != setting.options.end()) {
    setting.optionCurrent = static_cast<int>(std::distance(setting.options.begin(), it));
   } else {
    try {
     const int idx = std::stoi(value);
     if(idx >= 0 && idx < static_cast<int>(setting.options.size())) {
      setting.optionCurrent = idx;
     }
    } catch(...) {
    }
   }
...
void ModSettingsRegistry::registerSetting(const std::string& modId, const std::string& modName, ModSettingDef setting) {
 ...
 const auto existing = std::find_if(
     settings.begin(), settings.end(), [&setting](const ModSettingDef& value) { return value.key == setting.key; });
...
void ModSettingsRegistry::registerKeybind(const std::string& modId, ModKeybindDef keybind) {
 ...
 const auto existing = std::find_if(
     keybinds_.begin(), keybinds_.end(), [&keybind](const ModKeybindDef& value) { return value.id == keybind.id; });
...
std::string ModSettingsRegistry::getOptionValue(const std::string& modId, const std::string& key, const std::string& defaultValue) const {
 auto* setting = const_cast<ModSettingsRegistry*>(this)->findSetting(modId, key);
 if(setting && setting->kind == ModSettingDef::Options) {
  if(setting->optionCurrent >= 0 && setting->optionCurrent < static_cast<int>(setting->options.size())) {
   return setting->options[static_cast<std::size_t>(setting->optionCurrent)];
  }
 }
 return defaultValue;
}
...
void ModSettingsRegistry::setOptionValue(const std::string& modId, const std::string& key, const std::string& value) {
 auto* setting = findSetting(modId, key);
 if(setting && setting->kind == ModSettingDef::Options) {
  auto it = std::find(setting->options.begin(), setting->options.end(), value);
  if(it != setting->options.end()) {
...
void ModSettingsRegistry::save() {
 ...
 for(const auto& [modId, settings] : settings_) {
  for(const auto& s : settings) {
   ...
   } else if(s.kind == ModSettingDef::Options) {
    if(s.optionCurrent >= 0 && s.optionCurrent < static_cast<int>(s.options.size())) {
     out << modId << "." << s.key << "=" << s.options[static_cast<std::size_t>(s.optionCurrent)] << "\n";
    } else {
     out << modId << "." << s.key << "=" << s.optionCurrent << "\n";
    }
   } else {
...
void ModSettingsRegistry::markKeyPressed(int keyCode) {
 if(keyCode != 0) {
  pendingKeys_.insert(keyCode);
 }
}
bool ModSettingsRegistry::consumeKeybind(const std::string& id) {
 const auto* kb = findKeybind(id);
 if(kb != nullptr && kb->currentKeyCode != 0) {
  auto it = pendingKeys_.find(kb->currentKeyCode);
  if(it != pendingKeys_.end()) {
   pendingKeys_.erase(it);
   return true;
  }
 }
 return false;
