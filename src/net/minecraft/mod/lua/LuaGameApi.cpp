...
bool setPlayerCursorItem(int itemId, int count) {
  client::Minecraft* client = client::Minecraft::INSTANCE;
  if(client == nullptr || client->player == nullptr || itemId <= 0 || count <= 0) {
    return false;
  }
  client->player->inventory.setCursorStack(ItemStack(itemId, count));
  return true;
}
#else
bool worldIsNight(const World* /*world*/) {
...
bool spawnClientParticle(double /*x*/, double /*y*/, double /*z*/, double /*vx*/, double /*vy*/, double /*vz*/,
                         float /*scale*/, float /*red*/, float /*green*/, float /*blue*/, int /*maxAge*/,
                         float /*gravity*/) {
  return false;
}
bool readPlayerPosition(double& /*x*/, double& /*y*/, double& /*z*/) {
