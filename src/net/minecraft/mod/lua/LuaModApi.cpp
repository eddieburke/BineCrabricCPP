...
int luaFilesRead(lua_State* state) {
  ...
  if(const std::optional<std::filesystem::path> resolved =
     ...
     resolved.has_value()) {
    const std::vector<std::uint8_t> resourceBytes = net::minecraft::mod::runtime::readFileBytes(*resolved);
...
int luaRenderUpdateTexture(lua_State* state) {
  ...
  const auto* cached = client::Minecraft::INSTANCE->textureManager.getRasterImage(textureId);
  if(cached == nullptr) {
    api.pushboolean(state, 0);
    return 1;
  }
  ...
  api.settop(state, colorsIndex - 1);
  client::Minecraft::INSTANCE->textureManager.update(textureId, image);
  api.pushboolean(state, 1);
  return 1;
}
int luaRenderBindTexture(lua_State* state) {
  ...
  if(isNum != 0) {
    int maxUnits = 0;
    client::gl::getIntegerv(client::gl::tex::MaxTextureImageUnits, &maxUnits);
    if(unit < 0 || unit >= maxUnits) {
      api.pushboolean(state, 0);
      return 1;
    }
    int previousActive = client::gl::tex::Texture0;
    client::gl::getIntegerv(client::gl::tex::ActiveTexture, &previousActive);
    client::gl::activeTexture(client::gl::tex::Texture0 + unit);
    client::gl::bindTexture(client::gl::cap::Texture2D, textureId);
    client::gl::activeTexture(previousActive);
  } else {
    client::gl::bindTexture(client::gl::cap::Texture2D, textureId);
  }
  api.pushboolean(state, 1);
...
int luaRenderGetTexturePixels(lua_State* state) {
  ...
  if(api.type(state, 1) == kLuaTString) {
    ...
    if(::net::minecraft::mod::isMod(textureId)) {
      const int index = textureId - ::net::minecraft::mod::kModTextureBase;
      if(index >= 0 && index < static_cast<int>(::net::minecraft::mod::detail::modTextureEntries().size())) {
        const std::string path =
            ::net::minecraft::mod::detail::modTextureEntries()[static_cast<std::size_t>(index)].path;
        image = client::Minecraft::INSTANCE->textureManager.loadRasterForResource(path);
        loaded = (image.width > 0 && image.height > 0);
      }
    }
    if(!loaded) {
      if(const auto* cached = client::Minecraft::INSTANCE->textureManager.getRasterImage(textureId)) {
        image = *cached;
        loaded = true;
      }
    }
