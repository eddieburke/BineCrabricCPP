...
[[nodiscard]] std::optional<HitResult> entityRaycast(World* world, Entity* except, const Vec3d& start,
                                                     const Vec3d& end, double maxDistance) {
  Vec3d dir{end.x - start.x, end.y - start.y, end.z - start.z};
  const double length = std::sqrt(dir.x * dir.x + dir.y * dir.y + dir.z * dir.z);
  if(length < 1.0e-7 || world == nullptr) {
    return std::nullopt;
  }
  Box query = except != nullptr
                  ? except->boundingBox.stretch(dir.x, dir.y, dir.z).expand(1.0)
                  : Box{start.x - maxDistance, start.y - maxDistance, start.z - maxDistance,
                        start.x + maxDistance, start.y + maxDistance, start.z + maxDistance};
...
void pushRaycastResult(lua_State* state, const std::optional<HitResult>& hit, World* world) {
  ...
  if(hit->type == HitResultType::BLOCK) {
    ...
    setField(state, "entity_id", entity::EntityRegistry::getId(*hit->entity));
...
[[nodiscard]] bool readRaycastOrigin(lua_State* state, int tableIndex, Vec3d& origin, Vec3d& direction,
                                     double& maxDistance, bool& ignoreLiquids, bool& doBlocks, bool& doEntities) {
  LuaApi& api = luaApi();
  ...
  if(api.getfield(state, tableIndex, "direction") == kLuaTTable) {
    ...
    api.settop(state, tableIndex);
    if(api.getfield(state, tableIndex, "yaw") != kLuaTNil || api.getfield(state, tableIndex, "pitch") != kLuaTNil) {
      api.settop(state, tableIndex);
  ...
  maxDistance = luaFloatField(state, tableIndex, "max_distance",
                              luaFloatField(state, tableIndex, "reach", 0.0));
...
int luaRaycast(lua_State* state) {
  ...
  Entity* camera = client->camera != nullptr ? static_cast<Entity*>(client->camera)
                                                : static_cast<Entity*>(client->player);
  if(camera == nullptr) {
    api.pushnil(state);
    return 1;
  }
  ...
  const Vec3d end{origin.x + direction.x * maxDistance, origin.y + direction.y * maxDistance,
