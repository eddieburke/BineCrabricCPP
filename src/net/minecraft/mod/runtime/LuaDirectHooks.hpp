...
namespace net::minecraft::mod::runtime {
struct LuaHookFacts {
 enum class Kind : std::uint8_t { None, Str, Num, Bool };
 struct Entry {
  std::string_view key;
  std::string_view str;
  double num = 0.0;
 ...
 void addStr(std::string_view key, std::string_view value) {
  if(count < 8) {
   ...
   entries[count].str = value;
   entries[count].kind = Kind::Str;
   ++count;
  }
 }
 void addNum(std::string_view key, double value) {
  if(count < 8) {
   entries[count].key = key;
   entries[count].num = value;
   entries[count].kind = Kind::Num;
   ++count;
  }
 }
 void addBool(std::string_view key, bool value) {
  if(count < 8) {
   entries[count].key = key;
   entries[count].boolean = value;
   entries[count].kind = Kind::Bool;
   ++count;
  }
 }
};
[[nodiscard]] bool luaFactsReject(const ModHost::LoadedLuaMod::Callback& callback,
                                  const LuaHookFacts& facts);
