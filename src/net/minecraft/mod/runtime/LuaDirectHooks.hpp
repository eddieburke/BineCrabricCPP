...
struct WorldRenderEvent {
 ...
 float cloudBaseHeight = 128.0f;
 float renderDistance = 256.0f;
 float cloudRed = 1.0f;
 float cloudGreen = 1.0f;
 float cloudBlue = 1.0f;
...
void luaHookBlockBreak(BlockBreakEvent& event);
...
void luaHookChatSend(ChatEvent& event);
void luaHookChatReceive(ChatEvent& event);
} // namespace net::minecraft::mod::runtime
