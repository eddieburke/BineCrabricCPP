...
bool registerSoundKind(client::platform::audio::AudioEngine& audio, const std::string& id,
                       const std::filesystem::path& path, std::string kind) {
  kind = toLowerCopy(std::move(kind));
  if(kind.empty() || kind == "effect" || kind == "effects" || kind == "one_shot" || kind == "oneshot") {
    audio.registerEffect(id, path);
    return true;
  }
  if(kind == "streaming" || kind == "stream" || kind == "record") {
    audio.registerStreaming(id, path);
    return true;
  }
  if(kind == "music" || kind == "bgm") {
...
int luaRegisterSound(lua_State* state) {
  ...
  if(mod == nullptr || api.gettop(state) < 2 || api.type(state, 1) != kLuaTString || api.type(state, 2) != kLuaTString) {
