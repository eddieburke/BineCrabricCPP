...
bool readItemBox(lua_State* state, int tableIndex, ModelBox& box) {
  ...
  api.settop(state, tableIndex);
  return std::isfinite(box.minX) && std::isfinite(box.minY) && std::isfinite(box.minZ) &&
         std::isfinite(box.maxX) && std::isfinite(box.maxY) && std::isfinite(box.maxZ) && box.minX >= 0.0f &&
         box.minY >= 0.0f && box.minZ >= 0.0f && box.maxX <= 1.0f && box.maxY <= 1.0f && box.maxZ <= 1.0f &&
         box.minX < box.maxX && box.minY < box.maxY && box.minZ < box.maxZ;
}
bool parseItemModel(lua_State* state, int tableIndex, ItemRegistrationSpec& spec, int& manualDrawRef,
                    std::string& error) {
  LuaApi& api = luaApi();
