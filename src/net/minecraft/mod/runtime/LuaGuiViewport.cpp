...
bool readArgbField(lua_State* state, int tableIndex, const char* key, std::uint32_t& color) {
  LuaApi& api = luaApi();
  api.getfield(state, tableIndex, key);
  const bool found = api.type(state, -1) == kLuaTNumber;
  if(found) {
    color = luaArgb(state, -1);
  }
  pop(state, 1);
  return found;
}
void readColor(lua_State* state, int tableIndex, float& r, float& g, float& b, float& a) {
  std::uint32_t argb = 0;
  if(readArgbField(state, tableIndex, "color", argb)) {
...
LuaGuiViewportParams readViewportParams(lua_State* state, int tableIndex) {
  ...
  std::uint32_t argb = 0;
  if(readArgbField(state, tableIndex, "clear_color", argb)) {
