...
[[nodiscard]] bool computeFramebufferViewport(const LuaGuiViewportParams& params, int displayWidth, int displayHeight,
...
[[nodiscard]] bool guiMouseToFrame(int mouseX, int mouseY, int guiWidth, int guiHeight, int displayWidth,
                                   int displayHeight, double& frameX, double& frameY) {
  ...
  frameY = static_cast<double>(displayHeight) -
           (static_cast<double>(mouseY) + 0.5) * static_cast<double>(displayHeight) /
               static_cast<double>(guiHeight);
  return true;
}
...
LuaGuiViewportParams readViewportParams(lua_State* state, int tableIndex) {
  ...
  params.guiWidth =
      static_cast<int>(luaFloatField(state, tableIndex, "gui_width",
                                     client != nullptr ? static_cast<float>(client->displayWidth) : 0.0f));
  params.guiHeight =
      static_cast<int>(luaFloatField(state, tableIndex, "gui_height",
                                     client != nullptr ? static_cast<float>(client->displayHeight) : 0.0f));
  params.yawDeg = luaFloatField(state, tableIndex, "yaw_deg", 0.0f);
  params.pitchDeg = luaFloatField(state, tableIndex, "pitch_deg", 0.0f);
  params.camDist = luaFloatField(state, tableIndex, "distance",
                                 luaFloatField(state, tableIndex, "cam_dist", 2.05f));
...
int luaGuiBeginViewport(lua_State* state) {
  if(!luaGuiDrawActive() || luaApi().type(state, 1) != kLuaTTable || !beginLuaGuiViewport(readViewportParams(state, 1))) {
    return 0;
  }
  return 0;
}
...
bool computeLuaGuiViewportRay(const LuaGuiViewportParams& params, int mouseX, int mouseY, LuaGuiViewportRay& outRay) {
  ...
  double frameX = 0.0;
  double frameY = 0.0;
  if(!guiMouseToFrame(mouseX, mouseY, params.guiWidth, params.guiHeight, client->displayWidth, client->displayHeight,
                      frameX, frameY)) {
    return false;
  }
  if(frameX < static_cast<double>(vx) || frameX >= static_cast<double>(vx + vw) || frameY < static_cast<double>(vy) ||
     frameY >= static_cast<double>(vy + vh)) {
    return false;
  }
  const float camDist = clampFloat(params.camDist, 1.5f, 6.0f);
  const float fovDeg = clampFloat(params.fovDeg, 10.0f, 120.0f);
