...
class ModDrawScope {
  ...
  ~ModDrawScope() { client::render::RenderSystem::setShadow(saved_); }
...
template <typename Event, typename Push, typename Read>
void runLuaHookFiltered(LuaEventId id, Event& e, const LuaHookFacts& facts, Push push, Read read) {
  const int eventIndex = static_cast<int>(id);
  if(!hasLuaHook(eventIndex)) {
    return;
  }
  ...
  if(world != nullptr) {
    ModContextScope scope(world, eventPlayer(e));
    dispatchLuaHook(eventIndex, facts, fill, apply);
  } else {
    dispatchLuaHook(eventIndex, facts, fill, apply);
  }
}
...
bool luaFactsReject(const ModHost::LoadedLuaMod::Callback& callback, const LuaHookFacts& facts) {
  if(callback.filters.empty() || facts.count == 0) {
    return false;
  }
  for(const auto& filter : callback.filters) {
    for(int i = 0; i < facts.count; ++i) {
      const LuaHookFacts::Entry& entry = facts.entries[i];
      if(entry.kind == LuaHookFacts::Kind::None || entry.key != filter.key) {
        continue;
      }
      using Kind = ModHost::LoadedLuaMod::ScalarFilter::Kind;
      if(filter.kind == Kind::Str && entry.kind == LuaHookFacts::Kind::Str) {
        if(entry.str != filter.str) {
          return true;
        }
      } else if(filter.kind == Kind::Num && entry.kind == LuaHookFacts::Kind::Num) {
        if(entry.num != filter.num) {
          return true;
        }
      } else if(filter.kind == Kind::Bool && entry.kind == LuaHookFacts::Kind::Bool) {
        if(entry.boolean != filter.boolean) {
          return true;
        }
      }
      break;
...
void dispatchLuaHook(int eventIndex, const LuaHookFacts& facts, Fill fill, Read read) {
...
void dispatchLuaHook(int eventIndex, Fill fill, Read read) {
  static const LuaHookFacts kNoFacts{};
  dispatchLuaHook(eventIndex, kNoFacts, fill, read);
}
[[nodiscard]] bool isSupportedLuaEvent(std::string_view event) {
  return luaEventIndexOf(event) >= 0;
}
void subscribeLuaCallback(const std::shared_ptr<ModHost::LoadedLuaMod>&,
                         const ModHost::LoadedLuaMod::Callback&) {
...
void registerLifecycleListener(int order, LifecycleListener listener) {
  ...
  std::stable_sort(lifecycleListeners().begin(),
                   lifecycleListeners().end(),
                   [](const LifecycleListenerEntry& a, const LifecycleListenerEntry& b) {
...
void registerChunkStageListener(world::gen::ChunkStage stage,
                               int priority,
                               ChunkStageListener listener) {
...
void luaHookClientTick(ClientTickEvent& e) {
  LuaHookFacts facts;
  facts.addBool("before", e.before);
  facts.addBool("after_world", e.afterWorld);
  facts.addBool("paused", e.paused);
  runLuaHookFiltered(LuaEventId::ClientTick, e, facts, [](lua_State* state, ClientTickEvent& ev) { setClientTickFields(state, ev); }, kNoRead);
}
void luaHookRenderFrame(RenderFrameEvent& e) {
  runLuaHook(LuaEventId::RenderFrame,
              e,
...
void luaHookFirstPersonHand(FirstPersonHandRenderEvent& e) {
  runLuaHook(LuaEventId::FirstPersonHand,
              e,
...
void luaHookKeyPress(KeyPressEvent& e) {
  LuaHookFacts facts;
  facts.addNum("key", static_cast<double>(e.key));
  facts.addBool("pressed", e.pressed);
  runLuaHookFiltered(LuaEventId::KeyPress, e, facts,
...
void luaHookMouseButton(MouseButtonEvent& e) {
  LuaHookFacts facts;
  facts.addNum("button", static_cast<double>(e.button));
  facts.addBool("pressed", e.pressed);
  runLuaHookFiltered(LuaEventId::MouseButton, e, facts,
...
void luaHookCameraSetup(CameraSetupEvent& e) {
  runLuaHook(LuaEventId::CameraSetup,
              e,
...
void luaHookPlayerTravel(PlayerTravelEvent& e) {
  runLuaHook(LuaEventId::PlayerTravel,
              e,
...
void luaHookTickRate(TickRateEvent& e) {
  runLuaHook(LuaEventId::TickRate,
              e,
...
void luaHookWorldStart(WorldStartEvent& e) {
  runLuaHook(LuaEventId::WorldStart,
              e,
...
void luaHookWorldOpen(WorldOpenEvent& e) {
  runLuaHook(LuaEventId::WorldOpen,
              e,
...
void luaHookWorldTick(WorldTickEvent& e) {
  LuaHookFacts facts;
  facts.addBool("before", e.before);
  facts.addBool("remote", e.remote);
  runLuaHookFiltered(LuaEventId::WorldTick, e, facts,
              [](lua_State* state, WorldTickEvent& ev) {
                setFields(state, "remote", ev.remote, "before", ev.before);
              },
              kNoRead);
}
void luaHookEntityTick(EntityTickEvent& e) {
  LuaHookFacts facts;
  facts.addBool("remote", e.remote);
  runLuaHookFiltered(LuaEventId::EntityTick, e, facts,
...
void luaHookTileEntityTick(TileEntityTickEvent& e) {
  ...
  LuaHookFacts tileFacts;
  const std::string tileId = e.entity->id();
  tileFacts.addStr("id", tileId);
  dispatchLuaHook(
      static_cast<int>(LuaEventId::TileEntityTick), tileFacts,
      [&e](lua_State* state) {
        setFields(state,
                  "x",
                  e.entity->x,
...
void luaHookCreateWorld(CreateWorldEvent& e) {
  runLuaHook(LuaEventId::CreateWorld,
              e,
...
void luaHookBlockInteract(BlockInteractEvent& e) {
  LuaHookFacts facts;
  facts.addBool("right_click", e.rightClick);
  runLuaHookFiltered(LuaEventId::BlockInteract, e, facts,
...
void luaHookEntityInteract(EntityInteractEvent& e) {
  runLuaHook(LuaEventId::EntityInteract,
              e,
...
void luaHookAttackDamage(AttackDamageEvent& e) {
  runLuaHook(LuaEventId::AttackDamage,
              e,
...
void luaHookWorldColor(WorldColorEvent& e) {
  runLuaHook(LuaEventId::WorldColor,
              e,
...
void luaHookFogSettings(FogSettingsEvent& e) {
  runLuaHook(LuaEventId::FogSettings,
              e,
...
void luaHookEntityRender(EntityRenderEvent& e) {
  LuaHookFacts facts;
  facts.addStr("entity_type", e.entityType);
  facts.addBool("is_player", e.isPlayer);
  facts.addNum("entity_id", static_cast<double>(e.entityId));
  runLuaHookFiltered(LuaEventId::EntityRender, e, facts,
...
void luaHookWorldRender(WorldRenderEvent& e) {
  ...
  LuaHookFacts facts;
  facts.addStr("stage", renderStageName(e.stage));
  facts.addStr("moment", renderMomentName(e.moment));
  facts.addBool("shadow_pass", e.shadowPass);
  facts.addBool("is_overworld", luaWorldIsOverworld(e.world));
  ScopedModWorldDrawContext worldDrawScope{e.world, e.tickDelta};
  const ModDrawScope modCaps;
  dispatchLuaHook(
      static_cast<int>(LuaEventId::WorldRender), facts,
      [&e](lua_State* state) {
        setFields(state,
                  "tick_delta",
                  e.tickDelta,
        ...
        setWorldContextFields(state, e.world);
        const client::render::FrameRenderCamera& frameCamera = client::render::RenderCameraState::instance().frame();
...
void luaHookScreenUi(ScreenUiEvent& e) {
  ...
  LuaHookFacts facts;
  facts.addStr("screen_id", e.context->screenId);
  facts.addStr("region", e.context->region);
  ...
  dispatchLuaHook(
      static_cast<int>(LuaEventId::ScreenUi), facts,
      [&e](lua_State* state) {
        setFields(state,
                  "screen_id",
...
void luaHookScreenEvent(LuaScreenEvent& e) {
  ...
  LuaHookFacts facts;
  facts.addStr("screen_id", e.screen->id());
  activeLuaScreen()->mod = nullptr;
  dispatchLuaHook(
      static_cast<int>(LuaEventId::ScreenEvent), facts,
      [&e](lua_State* state) {
        setFields(state,
                  "screen_id",
                  e.screen->id(),
...
void luaHookPreEntityRender(PreEntityRenderEvent& e) {
  ...
  facts.addNum("entity_id", static_cast<double>(e.entityId));
  dispatchLuaHook(
      static_cast<int>(LuaEventId::PreEntityRender), facts,
      [&e](lua_State* state) {
        setFields(state,
                  "entity_id",
                  e.entityId,
...
#ifdef MINECRAFT_NATIVE_EXPORTS
  LuaHookFacts facts;
  facts.addStr("id", e.id);
  dispatchLuaHook(
      static_cast<int>(LuaEventId::PreTileEntityRender), facts,
      [&e](lua_State* state) {
        setFields(state,
                  "x",
                  e.x,
