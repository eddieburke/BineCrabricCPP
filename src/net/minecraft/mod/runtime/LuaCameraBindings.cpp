...
LUA_CAM_IMPL(luaCameraCreate, 2, api.pushinteger(state, -1), {
  int w = static_cast<int>(api.tointegerx(state, 1, nullptr));
  int h = static_cast<int>(api.tointegerx(state, 2, nullptr));
  int colorCount = api.gettop(state) >= 3 ? static_cast<int>(api.tointegerx(state, 3, nullptr)) : 1;
  bool useDepthTex = api.gettop(state) >= 4 ? api.toboolean(state, 4) != 0 : false;
...
LUA_CAM_IMPL(luaCameraRender, 8, api.pushboolean(state, 0), {
  if(client::Minecraft::INSTANCE->gameRenderer == nullptr) {
  ...
  api.pushboolean(state, m->renderWorldTo(handle, *client::Minecraft::INSTANCE->gameRenderer, tickDelta, x, y, z, yaw, pitch, roll, fov) ? 1 : 0);
})
...
LUA_CAM_IMPL(luaCameraRendering, 0, api.pushinteger(state, -1), {
  api.pushinteger(state, m->renderingHandle());
})
LUA_CAM_IMPL(luaCameraResize, 3, api.pushboolean(state, 0), {
  api.pushboolean(state, m->resize(static_cast<int>(api.tointegerx(state, 1, nullptr)),
...
LUA_CAM_IMPL(luaCameraCreateDisplaySize, 0, api.pushinteger(state, -1), {
  ...
  int w = client::Minecraft::INSTANCE->displayWidth;
  int h = client::Minecraft::INSTANCE->displayHeight;
  int colorCount = api.gettop(state) >= 1 ? static_cast<int>(api.tointegerx(state, 1, nullptr)) : 1;
  bool useDepthTex = api.gettop(state) >= 2 ? api.toboolean(state, 2) != 0 : false;
