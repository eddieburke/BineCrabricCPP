...
void subscribeLua(const LuaMod& mod, int ref, int priority, Push push, Read read) {
  hooks().subscribe<Event>(priority, [mod, ref, push, read](Event& e) {
    callLuaEvent(
        mod, ref, [&](lua_State* state) { push(state, e); }, [&](lua_State* state) { read(state, e); });
  });
}
...
#ifdef MINECRAFT_NATIVE_EXPORTS
  setFields(state, "chunk_x", LuaChunkContext::activeChunkX(), "chunk_z", LuaChunkContext::activeChunkZ(),
            "has_chunk", LuaChunkContext::hasActiveChunk());
#else
...
const std::unordered_map<std::string_view, LuaEventSubscriber>& luaEventSubscribers() {
  static const std::unordered_map<std::string_view, LuaEventSubscriber> kSubscribers = {
      {"client_tick", [](const LuaMod& mod, int ref, int priority) {
         subscribeLua<ClientTickEvent>(
             mod, ref, priority, [](lua_State* state, ClientTickEvent& e) { setClientTickFields(state, e); },
             kNoRead);
       }},
      {"render_targets", [](const LuaMod& mod, int ref, int priority) {
         subscribeLua<RenderTargetsEvent>(
             mod, ref, priority,
             [](lua_State* state, RenderTargetsEvent& e) { setFields(state, "tick_delta", e.tickDelta); }, kNoRead);
       }},
      {"key_press", [](const LuaMod& mod, int ref, int priority) {
      ...
      {"mouse_button", [](const LuaMod& mod, int ref, int priority) {
      ...
      {"raycast", [](const LuaMod& mod, int ref, int priority) {
         subscribeLua<RaycastEvent>(
             ...
             [](lua_State* state, RaycastEvent& e) {
               ...
               setFields(state, "has_hit", e.hasHit, "type", typeName, "hit_x", e.hitX, "hit_y", e.hitY, "hit_z",
                         e.hitZ, "block_x", e.blockX, "block_y", e.blockY, "block_z", e.blockZ, "side", e.side,
                         "block_id", e.blockId, "block_name", blockWireNameFromId(e.blockId), "item_id", e.blockId);
               if(e.entity != nullptr) {
                 const auto pos = e.entity->position();
                 setFields(state, "entity_id", entity::EntityRegistry::getId(*e.entity), "entity_raw_id",
                           entity::EntityRegistry::getRawId(*e.entity), "entity_x", pos.x, "entity_y", pos.y,
                           "entity_z", pos.z);
               }
             },
             kNoRead);
       }},
      {"fov", [](const LuaMod& mod, int ref, int priority) {
      ...
      {"camera_setup", [](const LuaMod& mod, int ref, int priority) {
         subscribeLua<CameraSetupEvent>(
             mod, ref, priority,
             [](lua_State* state, CameraSetupEvent& e) {
               setFields(state, "tick_delta", e.tickDelta, "x", e.x, "y", e.y, "z", e.z, "yaw", e.yaw, "pitch",
                         e.pitch, "roll", e.roll, "custom_view", e.customView, "hide_first_person_hand",
                         e.hideFirstPersonHand);
             },
             [](lua_State* state, CameraSetupEvent& e) {
               readFields(state, "x", e.x, "y", e.y, "z", e.z, "yaw", e.yaw, "pitch", e.pitch, "roll", e.roll,
                          "custom_view", e.customView, "hide_first_person_hand", e.hideFirstPersonHand);
             });
       }},
      {"player_travel", [](const LuaMod& mod, int ref, int priority) {
         subscribeLua<PlayerTravelEvent>(
             mod, ref, priority,
             [](lua_State* state, PlayerTravelEvent& e) {
               setFields(state, "sideways", e.sideways, "forward", e.forward, "speed_multiplier", e.speedMultiplier,
                         "has_player", e.player != nullptr);
#ifdef MINECRAFT_NATIVE_EXPORTS
             ...
             [](lua_State* state, PlayerTravelEvent& e) {
               readFields(state, "sideways", e.sideways, "forward", e.forward, "speed_multiplier",
                          e.speedMultiplier);
             });
       }},
      {"tick_rate", [](const LuaMod& mod, int ref, int priority) {
      ...
      {"world_start", [](const LuaMod& mod, int ref, int priority) {
         subscribeLua<WorldStartEvent>(
             mod, ref, priority,
             [](lua_State* state, WorldStartEvent& e) {
               setFields(state, "save_name", e.saveName != nullptr ? *e.saveName : std::string(), "new_world",
                         e.newWorld);
             },
             kNoRead);
       }},
      {"world_open", [](const LuaMod& mod, int ref, int priority) {
      ...
      {"world_tick", [](const LuaMod& mod, int ref, int priority) {
      ...
      {"tile_entity_tick", [](const LuaMod& mod, int ref, int priority) {
#ifdef MINECRAFT_NATIVE_EXPORTS
         hooks().subscribe<TileEntityTickEvent>(priority, [mod, ref](TileEntityTickEvent& e) {
           ...
           callLuaEvent(
               mod, ref,
               [&e](lua_State* state) {
                 setFields(state, "x", e.entity->x, "y", e.entity->y, "z", e.entity->z, "id", e.entity->id(),
                           "client_world", e.clientWorld, "removed", e.entity->isRemoved(), "canceled", e.canceled,
                           "world_time",
                           e.entity->world != nullptr ? static_cast<double>(e.entity->world->getTime()) : 0.0,
                           "animation_frame", tileEntityAnimationFrame(e.entity), "animation_tick",
                           static_cast<double>(tileEntityAnimTick(e.entity)), "animation_speed",
      ...
      {"create_world", [](const LuaMod& mod, int ref, int priority) {
         subscribeLua<CreateWorldEvent>(
             mod, ref, priority,
             [](lua_State* state, CreateWorldEvent& e) {
               setFields(state, "save_name", e.saveName != nullptr ? *e.saveName : std::string(), "seed", e.seed,
                         "canceled", e.canceled);
               pushStringMap(state, e.options);
               luaApi().setfield(state, -2, "options");
             },
      ...
      {"block_interact", [](const LuaMod& mod, int ref, int priority) {
         subscribeLua<BlockInteractEvent>(
             mod, ref, priority,
             [](lua_State* state, BlockInteractEvent& e) {
               setFields(state, "x", e.x, "y", e.y, "z", e.z, "block_id", getBlockIdAt(e.world, e.x, e.y, e.z),
                         "side", e.side, "right_click", e.rightClick, "canceled", e.canceled, "handled", e.handled,
                         "has_player", e.player != nullptr, "has_item", e.stack != nullptr && !e.stack->empty());
               if(e.stack != nullptr && !e.stack->empty()) {
                 setFields(state, "item_id", e.stack->itemId, "item_count", e.stack->count, "item_damage",
                           e.stack->damage, "item_max_damage", e.stack->getMaxDamage(), "item_damageable",
      ...
      {"entity_interact", [](const LuaMod& mod, int ref, int priority) {
         hooks().subscribe<EntityInteractEvent>(priority, [mod, ref](EntityInteractEvent& e) {
           ...
           callLuaEvent(
               mod, ref,
               [&e](lua_State* state) {
                 setFields(state, "attack", e.attack, "canceled", e.canceled, "handled", e.handled, "has_player",
                           e.player != nullptr, "has_target", e.target != nullptr);
               },
               [&e](lua_State* state) { readFields(state, "canceled", e.canceled, "handled", e.handled); });
         });
       }},
      {"attack_damage", [](const LuaMod& mod, int ref, int priority) {
         subscribeLua<AttackDamageEvent>(
             mod, ref, priority,
             [](lua_State* state, AttackDamageEvent& e) {
               setFields(state, "damage", e.damage, "critical", e.critical, "canceled", e.canceled, "fall_distance",
                         e.fallDistance, "on_ground", e.onGround, "target_x", e.targetX, "target_y", e.targetY,
                         "target_z", e.targetZ, "has_player", e.player != nullptr, "has_target",
                         e.target != nullptr);
             },
             [](lua_State* state, AttackDamageEvent& e) {
      ...
      {"entity_teleport", [](const LuaMod& mod, int ref, int priority) {
         hooks().subscribe<EntityTeleportEvent>(priority, [mod, ref](EntityTeleportEvent& e) {
           ...
           callLuaEvent(
               mod, ref,
               [&e](lua_State* state) {
                 setFields(state, "id", e.entity != nullptr ? e.entity->id : -1, "type",
                           e.entity != nullptr ? net::minecraft::entity::EntityRegistry::getId(*e.entity)
                                               : std::string(),
                           "from_x", e.fromX, "from_y", e.fromY, "from_z", e.fromZ, "x", e.x, "y", e.y, "z", e.z,
                           "yaw", e.yaw, "pitch", e.pitch, "canceled", e.canceled, "has_entity",
                           e.entity != nullptr, "has_player",
                           dynamic_cast<net::minecraft::entity::player::PlayerEntity*>(e.entity) != nullptr);
                 setWorldContextFields(state, e.world);
               },
               [&e](lua_State* state) {
                 readFields(state, "x", e.x, "y", e.y, "z", e.z, "yaw", e.yaw, "pitch", e.pitch, "canceled",
                            e.canceled);
               });
         });
       }},
      {"world_color", [](const LuaMod& mod, int ref, int priority) {
         subscribeLua<WorldColorEvent>(
             mod, ref, priority,
             [](lua_State* state, WorldColorEvent& e) {
               setFields(state, "partial_ticks", e.partialTicks, "r", e.color.x, "g", e.color.y, "b", e.color.z,
                         "kind", e.kind == WorldColorKind::Sky ? "sky" : "fog");
               setWorldContextFields(state, e.world);
#ifdef MINECRAFT_NATIVE_EXPORTS
               if(e.world != nullptr) {
                 setFields(state, "celestial", static_cast<double>(normalizedCelestial(e.world, e.partialTicks)),
                           "world_time", static_cast<double>(e.world->getTime() % 24000ULL), "is_night",
      ...
      {"entity_render", [](const LuaMod& mod, int ref, int priority) {
         subscribeLua<EntityRenderEvent>(
             mod, ref, priority,
             [](lua_State* state, EntityRenderEvent& e) {
               setFields(state, "entity_id", e.entityId, "entity_type", e.entityType, "is_player", e.isPlayer,
                         "tick_delta", e.tickDelta);
               luaApi().createtable(state, 0, 12);
               setFields(state, "body_yaw", e.pose.bodyYaw, "head_yaw", e.pose.headYaw, "head_pitch",
                         e.pose.headPitch, "limb_swing", e.pose.limbSwing, "limb_distance", e.pose.limbDistance,
                         "yaw", e.pose.yaw, "pitch", e.pose.pitch, "roll", e.pose.roll, "scale", e.pose.scale,
                         "offset_x", e.pose.offsetX, "offset_y", e.pose.offsetY, "offset_z", e.pose.offsetZ);
             ...
             [](lua_State* state, EntityRenderEvent& e) {
               ...
               if(luaApi().type(state, -1) == kLuaTTable) {
                 readFields(state, "body_yaw", e.pose.bodyYaw, "head_yaw", e.pose.headYaw, "head_pitch",
                            e.pose.headPitch, "limb_swing", e.pose.limbSwing, "limb_distance", e.pose.limbDistance,
                            "yaw", e.pose.yaw, "pitch", e.pose.pitch, "roll", e.pose.roll, "scale", e.pose.scale,
                            "offset_x", e.pose.offsetX, "offset_y", e.pose.offsetY, "offset_z", e.pose.offsetZ);
      ...
      {"world_render", [](const LuaMod& mod, int ref, int priority) {
         hooks().subscribe<WorldRenderEvent>(priority, [mod, ref](WorldRenderEvent& e) {
           ...
           callLuaEvent(
               mod, ref,
               [&e](lua_State* state) {
                 setFields(state, "tick_delta", e.tickDelta, "stage", renderStageName(e.stage), "moment",
                           renderMomentName(e.moment), "cancel_vanilla", e.cancelVanilla, "vanilla_stage_ran",
                           e.vanillaStageRan, "celestial_angle", static_cast<double>(e.celestialAngle),
                           "sky_yaw_deg", static_cast<double>(e.skyYawDegrees), "star_brightness",
                           static_cast<double>(e.starBrightness), "rain_strength",
                           static_cast<double>(e.rainStrength), "stars_enabled", e.starsEnabled,
                           "astronomy_enabled", e.astronomyEnabled, "astronomy_utc_millis", e.astronomyUtcMillis,
                           "observer_latitude_deg", static_cast<double>(e.observerLatitudeDegrees),
                           "observer_longitude_deg", static_cast<double>(e.observerLongitudeDegrees));
                 setWorldContextFields(state, e.world);
...
#ifdef MINECRAFT_NATIVE_EXPORTS
                 if(e.world != nullptr) {
                   setFields(state, "world_time", static_cast<double>(e.world->getTime() % 24000ULL), "celestial",
                             static_cast<double>(normalizedCelestial(e.world, e.tickDelta)), "is_night",
                             worldIsNight(e.world));
                 }
...
#endif
                 setFields(state, "camera_x", cameraX, "camera_y", cameraY, "camera_z", cameraZ, "camera_yaw",
                           static_cast<double>(frameCamera.yaw), "camera_pitch",
                           static_cast<double>(frameCamera.pitch), "camera_roll",
                           static_cast<double>(frameCamera.roll), "custom_camera", frameCamera.customView);
               },
               [&e](lua_State* state) {
                 readField(state, "cancel_vanilla", e.cancelVanilla);
                 if(e.stage == WorldRenderStage::Sky && e.moment == RenderHookMoment::Before) {
                   readFields(state, "celestial_angle", e.celestialAngle, "sky_yaw_deg", e.skyYawDegrees,
                              "astronomy_enabled", e.astronomyEnabled, "astronomy_utc_millis", e.astronomyUtcMillis,
                              "observer_latitude_deg", e.observerLatitudeDegrees, "observer_longitude_deg",
                              e.observerLongitudeDegrees);
                 }
               });
         });
       }},
      {"chunk_generation", [](const LuaMod& mod, int ref, int priority) {
         hooks().subscribe<world::gen::ChunkGenerationEvent>(priority, [mod, ref](world::gen::ChunkGenerationEvent& e) {
           ...
           callLuaEvent(
               mod, ref,
               [&e](lua_State* state) {
                 setFields(state, "stage", chunkStageName(e.stage), "moment",
                           e.moment == world::gen::HookMoment::Before ? "before" : "after", "cancel_vanilla",
                           e.cancelVanilla, "vanilla_stage_ran", e.vanillaStageRan, "world_seed",
      ...
      {"screen_region", [](const LuaMod& mod, int ref, int priority) {
         hooks().subscribe<ScreenRegionEvent>(priority, [mod, ref](ScreenRegionEvent& e) {
           ...
           callLuaEvent(
               ...
               [&e](lua_State* state) {
                 ...
                 setFields(state, "phase_name", phaseName, "screen_id", std::string(e.screenId), "region",
                           std::string(e.region), "mouse_x", e.mouseX, "mouse_y", e.mouseY, "button", e.button,
                           "scroll_delta", e.scrollDelta, "x", e.x, "y", e.y, "width", e.width, "height", e.height,
                           "handled", e.handled);
               },
               [&e](lua_State* state) {
      ...
      {"world_spawn_search", [](const LuaMod& mod, int ref, int priority) {
      ...
      {"screen_ui", [](const LuaMod& mod, int ref, int priority) {
#ifdef MINECRAFT_NATIVE_EXPORTS
         hooks().subscribe<ScreenUiEvent>(priority, [mod, ref](ScreenUiEvent& e) {
           ...
           callLuaEvent(
               mod, ref,
               [&e](lua_State* state) {
                 setFields(state, "screen_id", std::string(e.context->screenId), "region",
      ...
      {"screen_event", [](const LuaMod& mod, int ref, int priority) {
#ifdef MINECRAFT_NATIVE_EXPORTS
         hooks().subscribe<LuaScreenEvent>(priority, [mod, ref](LuaScreenEvent& e) {
           ...
           static constexpr const char* kPhaseNames[] = {
               "init", "render", "tick", "key", "mouse", "scroll", "close",
           };
           activeLuaScreen()->mod = mod.get();
           callLuaEvent(
               mod, ref,
               [&e](lua_State* state) {
                 setFields(state, "screen_id", e.screen->id(), "phase", kPhaseNames[static_cast<int>(e.phase)],
                           "width", e.screen->width(), "height", e.screen->height(), "mouse_x", e.mouseX, "mouse_y",
                           e.mouseY, "x", e.mouseX, "y", e.mouseY, "tick_delta", e.tickDelta, "key", e.keyCode,
                           "char", static_cast<int>(static_cast<unsigned char>(e.character)), "button", e.button,
                           "released", e.released, "delta", e.scrollDelta, "handled", e.handled);
               },
