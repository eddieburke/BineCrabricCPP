...
void ModHost::shutdown() {
 ...
 for(const std::shared_ptr<LoadedLuaMod>& mod : loadedLuaMods_) {
  ...
  if(api != nullptr && api->ready() && mod->state != nullptr) {
   ...
   for(const int ref : mod->eventTableRefs) {
    if(ref != kLuaNoRef) {
     api->unref(state, kLuaRegistryIndex, ref);
    }
   }
   mod->eventTableRefs.clear();
   mod->eventDepth = 0;
   ...
   if(client::Minecraft::INSTANCE != nullptr) {
    for(const int textureId : mod->ownedTextureIds) {
      const int glId =
          net::minecraft::registry::TextureRegistry::resolveGlId(textureId, client::Minecraft::INSTANCE->textureManager);
      if(glId >= 0) {
...
std::vector<std::string> ModHost::listAssets(std::string_view modId, std::string_view relativeDir) const {
 const std::filesystem::path directory = assetPath(modId, relativeDir);
