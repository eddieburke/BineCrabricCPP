...
int luaChunkSetBlock(lua_State* state) {
    ...
    const int localX = luaIntArg(state, 1);
    const int y = luaIntArg(state, 2);
    const int localZ = luaIntArg(state, 3);
    const int blockId = luaIntArg(state, 4);
...
int luaChunkFillBlock(lua_State* state) {
    ...
    int x1 = luaIntArg(state, 1);
    int y1 = luaIntArg(state, 2);
    int z1 = luaIntArg(state, 3);
    int x2 = luaIntArg(state, 4);
    int y2 = luaIntArg(state, 5);
    int z2 = luaIntArg(state, 6);
    const int blockId = luaIntArg(state, 7);
    if (x1 > x2) {
        std::swap(x1, x2);
    }
    if (y1 > y2) {
...
int luaChunkGetHeight(lua_State* state) {
    ...
    const int localX = luaIntArg(state, 1);
    const int localZ = luaIntArg(state, 2);
...
int luaWorldSetCursor(lua_State* state) {
    ...
    const int itemId = luaIntArg(state, 1);
    const int count = luaIntArg(state, 2);
...
int luaWorldSetTime(lua_State* state) {
    ...
    if (client::Minecraft::INSTANCE != nullptr && client::Minecraft::INSTANCE->world != nullptr) {
        client::Minecraft::INSTANCE->world->synchronizeTimeAndUpdates(static_cast<std::uint64_t>(tick));
