...
int luaGuiDrawText(lua_State* state) {
 ...
 const ModLuaGuiDrawScope textCaps(true);
...
int luaScreenUiAddCenteredButton(lua_State* state) {
 if(g_activeScreenUi == nullptr || g_activeScreenUi->screen == nullptr) {
  return 0;
 }
 ...
 if(api.gettop(state) < 2 + argOffset || api.type(state, 1 + argOffset) != kLuaTNumber ||
    api.type(state, 2 + argOffset) != kLuaTString) {
  return 0;
 }
 const int y = luaIntArg(state, 1 + argOffset);
 const std::string text = luaString(state, 2 + argOffset, "");
 const int ref = api.gettop(state) >= 3 + argOffset ? retainButtonCallback(state, mod, 3 + argOffset) : kLuaNoRef;
 const int labelRef =
     api.gettop(state) >= 4 + argOffset ? retainButtonCallback(state, mod, 4 + argOffset) : kLuaNoRef;
 client::gui::widget::ActionButtonWidget& button =
     g_activeScreenUi->addCenteredButton(y, text, [mod, ref]() { invokeButtonCallback(mod, ref); });
...
int luaScreenUiAddButton(lua_State* state) {
 ...
 const int ref = api.gettop(state) >= 6 + argOffset ? retainButtonCallback(state, mod, 6 + argOffset) : kLuaNoRef;
 (void)g_activeScreenUi->addButton(x, y, width, height, text, [mod, ref]() { invokeButtonCallback(mod, ref); });
 return 0;
}
int luaScreenUiAddStackedCenteredButton(lua_State* state) {
 if(g_activeScreenUi == nullptr || g_activeScreenUi->screen == nullptr ||
    g_activeScreenUi->stackedButtonY == nullptr) {
  return 0;
 }
 auto* mod = currentLuaMod(state);
 if(mod == nullptr) {
...
int luaScreenAddField(lua_State* state) {
 LuaApi& api = luaApi();
 if(g_activeLuaScreen == nullptr || !g_luaScreenInitPhase || api.type(state, 1) != kLuaTString) {
  return 0;
 }
 ...
 g_activeLuaScreen->addField(name, x, y, width, height, text, maxLen, numeric, allowSign, allowDot);
 return 0;
}
int luaScreenFieldText(lua_State* state) {
 LuaApi& api = luaApi();
 std::string text;
 if(g_activeLuaScreen != nullptr && api.type(state, 1) == kLuaTString) {
  if(client::gui::widget::TextFieldWidget* field = g_activeLuaScreen->field(luaString(state, 1, ""))) {
   text = field->getText();
  }
 }
 api.pushstring(state, text.c_str());
...
int luaScreenSetFieldsVisible(lua_State* state) {
 ...
 if(g_activeLuaScreen != nullptr) {
  g_activeLuaScreen->setFieldsVisible(api.toboolean(state, 1) != 0);
 }
 return 0;
}
int luaScreenAddButton(lua_State* state) {
 LuaApi& api = luaApi();
 if(g_activeLuaScreen == nullptr || !g_luaScreenInitPhase || api.gettop(state) < 5 ||
 ...
 const int ref = api.gettop(state) >= 6 ? retainButtonCallback(state, mod, 6) : kLuaNoRef;
