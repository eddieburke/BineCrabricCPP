...
void applyPoseHook(const PoseHookRegistration& hook, const net::minecraft::LivingEntity& entity, float tickDelta,
...
int luaRegisterGlobalPoseHook(lua_State* state) {
  ...
  if(mod == nullptr || api.gettop(state) < 2 || api.type(state, 1) != kLuaTString || api.type(state, 2) != kLuaTFunction) {
...
int luaRegisterLocalPoseHook(lua_State* state) {
  LuaApi& api = luaApi();
  ModHost::LoadedLuaMod* mod = currentLuaMod(state);
  if(mod == nullptr || api.gettop(state) < 2 || api.type(state, 1) != kLuaTNumber || api.type(state, 2) != kLuaTFunction) {
    api.pushboolean(state, 0);
    return 1;
  }
  std::shared_ptr<ModHost::LoadedLuaMod> owner;
...
int luaEntitiesList(lua_State* state) {
  ...
  const std::string filter = api.gettop(state) >= 1 && api.type(state, 1) == kLuaTString
                                 ? luaString(state, 1, "")
                                 : std::string();
  int count = 0;
  for(net::minecraft::entity::Entity* e : world->entities()) {
    ...
    if(type == "Item") {
      ...
      if(modTex) {
        ...
        setField(state, "texture_path", net::minecraft::client::render::item::ItemModelRenderer::spriteAtlasPath(stack));
...
void applyRegisteredPoseHooks(const net::minecraft::LivingEntity& entity, float tickDelta,
                              net::minecraft::mod::EntityRenderPose& pose) {
