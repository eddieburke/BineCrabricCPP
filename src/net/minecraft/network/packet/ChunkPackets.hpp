...
public:
    ...
    ChunkStatusUpdateS2CPacket() { worldPacket = false; }
    ...
    void apply(NetworkHandler& networkHandler) const override { networkHandler.onChunkStatusUpdate(*this); }
...
public:
    ...
    ChunkDataS2CPacket() { worldPacket = true; }
    ...
    void read(std::istream& input) override
    {
        ...
        const std::vector<std::uint8_t> compressed = packetio::readBytes(input, static_cast<std::size_t>(chunkDataSize));
    ...
    void apply(NetworkHandler& networkHandler) const override { networkHandler.handleChunkData(*this); }
    [[nodiscard]] std::size_t size() const override { return static_cast<std::size_t>(17 + chunkDataSize); }
};
...
public:
    ...
    ChunkDeltaUpdateS2CPacket() { worldPacket = true; }
    ...
    void apply(NetworkHandler& networkHandler) const override { networkHandler.onChunkDeltaUpdate(*this); }
    [[nodiscard]] std::size_t size() const override { return static_cast<std::size_t>(10 + entryCount * 4); }
};
