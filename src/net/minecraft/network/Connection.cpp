...
void Connection::configureAcceptedSocket(SOCKET socket) {
  server::network::ServerSocket::configureAcceptedSocket(socket);
}
...
void SocketOutputStreamBuf::sendAll(const char* data, std::size_t length) {
  while(length > 0) {
    const int chunk = ::send(socket_, data,
...
Connection::Connection(SOCKET socket, std::string name, NetworkHandler& networkHandler)
    : socket_(socket), name_(std::move(name)), inputBuf_(socket_), input_(&inputBuf_), outputBuf_(socket_),
      output_(&outputBuf_) {
  ensureWinsock();
  setSocketOptions();
