...
std::string appendHandshakeMetadata(const std::string& serverId,
                                    bool nativeCppMods,
                                    bool luaModsEnabled,
 ...
 for(const auto& [modId, url] : downloadUrls) {
  if(mod::lua::isSafeModId(modId) && !url.empty()) {
 ...
 while(cursor <= query.size()) {
  ...
  if(!pair.empty()) {
   ...
   const std::string value =
       equals == std::string::npos ? std::string{} : percentDecode(pair.substr(equals + 1));
   if(key == "kind") {
    out.nativeCppMods = value == "cpp";
   } else if(key == "lua") {
    out.luaModsEnabled = value == "1" || mod::lua::toLowerCopy(value) == "true";
   } else if(key == "mods") {
    out.requiredMods = mod::runtime::WorldRequiredMods::splitCsv(value);
   } else if(key.rfind("dl.", 0) == 0) {
    const std::string modId = key.substr(3);
    if(mod::lua::isSafeModId(modId) && !value.empty()) {
