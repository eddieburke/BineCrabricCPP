...
// LabPBR (PBR) companion texture pipeline for the beta 1.7.3 flat-atlas model.
//
// Port of Iris 26.1's net.irisshaders.iris.pbr package (PBRTextureManager +
// PBRAtlasTexture + AtlasPBRLoader + SimplePBRLoader + ChannelMipmapGenerator)
// fitted to the beta asset model: the Java original stitches per-sprite
// `textures/<sprite>_n.png` / `_s.png` overlays into dedicated atlas textures;
// beta has no per-block texture files, so the companion images themselves ARE
// the atlases (/terrain_n.png, /gui/items_n.png, or per-mod <path>_n.png),
// grid-compatible with the diffuse atlas when dimensions match.
//
...
class PbrTextures {
 public:
  ...
  enum class Type { Normal, Specular };
  ...
  // Port of ChannelMipmapGenerator.blend for the beta pipeline. Colors are
  // ARGB-packed like RasterImage.argb. Normal maps blend all channels
  // linearly; specular maps blend R linearly and G/B/A discretely (LabPBR).
  // When `labPbr` is false the specular channels blend linearly too — Java's
  // PBRSpriteContents falls back to LINEAR_MIPMAP_GENERATOR when no texture
  // format is active.
  ...
  // Port of AbstractMipmapGenerator.generateMipLevels' per-level downsample
  // using the channel rules above. Operates on RGBA byte buffers (same layout
  // as TextureManager::load). `dst` may alias `src` (in-place halving, as in
  // TextureManager::uploadStaticMipmapLevels): every dst pixel is read before
  // it is written, and its taps belong to dst pixels processed earlier.
  ...
  static void pbrDownsampleMip(const std::uint8_t* src, int srcWidth,
                               std::uint8_t* dst, int dstWidth, int dstHeight,
                               Type type, bool labPbr) noexcept;

  // Port of ImageManipulationUtil.scaleNearestNeighbor / scaleBilinear,
  // used when a companion atlas does not match the diffuse atlas dimensions.
  // Java picks nearest-neighbor when the target is an integer multiple of the
  // source, otherwise bilinear (AtlasPBRLoader.createPBRSprite).
  ...
  // C++ analog of PBRTextureManager.getOrLoadHolder: cached per-diffuse-id
  // holder. Loading is synchronous (no one-frame deferral), uploading the
  // companion into its TextureManager-owned GL id with LabPBR mip levels and
  // the PBRAtlasTexture sampler parameters (clamp-to-edge, nearest mip).
  // `labPbr` selects the specular mip blend rules (Java TextureFormatLoader).
