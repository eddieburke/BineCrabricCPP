...
void WorldRenderer::createColumn(int sectionX, int sectionZ) {
  ...
  for(int sectionY = 0; sectionY < kChunkSectionCountY; ++sectionY) {
    ...
    auto builder = std::make_unique<chunk::ChunkBuilder>(
        world, globalBlockEntities, sectionX * kChunkSectionSize, sectionY * kChunkSectionSize,
        sectionZ * kChunkSectionSize, kChunkSectionSize, &regionManager_);
...
bool WorldRenderer::startMeshJob(chunk::ChunkBuilder* chunk, bool nearLane, int priority,
...
bool WorldRenderer::compileChunks(net::minecraft::LivingEntity& /*camera*/, bool force) {
  ...
  const float gridAreaScale =
      static_cast<float>(renderRadiusChunks_ * renderRadiusChunks_) /
      static_cast<float>(kBaselineRadius * kBaselineRadius);
  const std::size_t workerCount = meshScheduler_.workerCount();
  const std::size_t backlog =
      dirtyChunks_.size() + pendingMeshUploads_.size() + meshScheduler_.pendingJobs();
  const bool loadingBacklog = backlog > 512u;
  const int minUploadsPerFrame = loadingBacklog
                                     ? std::clamp(static_cast<int>(workerCount * 2), 4, 16)
  ...
  if(inFlight < targetInFlight && pendingMeshUploads_.size() < targetInFlight * 2u) {
    ...
    const int minCapturesPerFrame =
        force ? static_cast<int>(workerCount * 2u)
    ...
    const auto canCapture = [&] {
      return inFlight < targetInFlight && captureBudget.hasRemaining(captures);
    };
