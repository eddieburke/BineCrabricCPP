...
struct OutlineTextureOffScope {
 OutlineTextureOffScope() { RenderSystem::disableTexture(); }
 ~OutlineTextureOffScope() { RenderSystem::enableTexture(); }
...
void WorldRenderer::drainPendingColumns() {
 ...
 while(!pendingColumns_.empty() && budget.hasRemaining(static_cast<int>(inspected))) {
  ...
  if(world->getChunkSource() != nullptr && world->getChunkSource()->isChunkLoaded(col.x, col.z)) {
...
int WorldRenderer::renderChunksVbo(int layer, double /*tickDelta*/, double interpX, double interpY, double interpZ, bool skipBuildDrawLists) {
...
bool WorldRenderer::compileChunks(net::minecraft::LivingEntity& /*camera*/, bool force) {
 ...
 if(inFlight < targetInFlight && pendingMeshUploads_.size() < targetInFlight * 2u) {
  ...
  const net::minecraft::util::concurrent::FrameBudget captureBudget =
      net::minecraft::util::concurrent::FrameBudget::fromMs((force || loadingBacklog) ? 6 : 2, minCapturesPerFrame);
  int captures = 0;
...
void WorldRenderer::renderMiningProgress(net::minecraft::PlayerEntity* entity,
 ...
 const int stage = std::clamp(static_cast<int>(miningProgress * 10.0f), 0, 9);
 const float alpha = 0.15f + 0.60f * (static_cast<float>(stage) / 9.0f);
