...
void RenderType::restoreRenderState(const RenderSystem::StateShadow& saved, gl::ShaderProgram* savedProgram) const {
 if(saved.texture2D) {
  RenderSystem::enableTexture();
 } else {
  RenderSystem::disableTexture();
 ...
 if(saved.viewportValid) {
  RenderSystem::viewport(saved.viewport[0], saved.viewport[1], saved.viewport[2], saved.viewport[3]);
 }
 {
...
RenderPassScope::RenderPassScope(const RenderType& type)
    : type_(&type), saved_(RenderSystem::getShadow()), savedProgram_(gl::engine_pipeline::getActiveProgram()) {
