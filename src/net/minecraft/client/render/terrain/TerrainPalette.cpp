...
void build() {
  g_table.fill(0);
  g_sideTable.fill(0);
  if(!net::minecraft::registry::Registry::isBootstrapped()) {
    return;
  }
...
const std::array<std::uint32_t, 256>& TerrainPalette::table() {
  if(!g_built) {
    build();
  }
  return g_table;
}
const std::array<std::uint32_t, 256>& TerrainPalette::sideTable() {
  if(!g_built) {
    build();
  }
  return g_sideTable;
}
std::uint64_t TerrainPalette::generation() noexcept {
  return g_generation;
}
void TerrainPalette::invalidate() {
  g_built = false;
  ++g_generation;
}
} // namespace net::minecraft::client::render::terrain
