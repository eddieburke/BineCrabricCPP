...
void inferColortexFormatsFromLayouts(ShaderPackDefinition& pack, const std::string& source);
...
void inferColortexFormatsFromLayouts(ShaderPackDefinition& pack, const std::string& source) {
 auto upgrade = [&](int index, const std::string& format) {
  ...
  ShaderTarget& target = pack.targets["colortex" + std::to_string(index)];
 ...
 // out uvec4 colortexN / out f16vec4 colortexN (gbuffers RENDERTARGETS writers)
 for(std::size_t search = 0;;) {
  ...
  const int index = std::atoi(source.c_str() + numStart);
  const std::size_t window = marker > 80 ? marker - 80 : 0;
  const std::string before = lowercase(source.substr(window, marker - window));
  if(before.find("out") == std::string::npos && before.find("uniform") == std::string::npos &&
     before.find("usampler") == std::string::npos && before.find("isampler") == std::string::npos) {
   search = marker + 8;
   continue;
  }
  if(before.find("uvec4") != std::string::npos || before.find("uvec3") != std::string::npos ||
     before.find("usampler2d") != std::string::npos)
   upgrade(index, "RGBA32UI");
  else if(before.find("isampler2d") != std::string::npos)
   upgrade(index, "RGBA32I");
  else if(before.find("f16vec4") != std::string::npos || before.find("float16_t") != std::string::npos)
