...
bool rendersAsBlockModel(const ItemStack& stack) {
 ...
 if(block != nullptr && net::minecraft::mod::lua::blockRegistrationSpecForId(block->id) != nullptr) {
  return true;
 }
...
net::minecraft::block::TerrainAtlasUv spriteUv(const ItemStack& stack) {
 ...
 const net::minecraft::client::render::ResolvedTexture uv = net::minecraft::client::render::resolveBlockTextureUv(sprite);
 return {uv.uMin, uv.uMax, uv.vMin, uv.vMax};
}
