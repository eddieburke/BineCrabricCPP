...
void ItemRenderer::renderGuiItem(client::font::TextRenderer& textRenderer, client::texture::TextureManager& textureManager,
...
{
    const int barPixels = static_cast<int>(std::lround(
        13.0 - static_cast<double>(stack.getDamage2()) * 13.0 / static_cast<double>(stack.getMaxDamage())));
    const int barColorAmount = static_cast<int>(std::lround(
        255.0 - static_cast<double>(stack.getDamage2()) * 255.0 / static_cast<double>(stack.getMaxDamage())));
...
void ItemRenderer::drawTexture(int x, int y, int u, int v, int width, int height)
{
    ...
    Tessellator& tessellator = Tessellator::INSTANCE;
    tessellator.startQuads();
    tessellator.vertex(x + 0, y + height, depth, static_cast<float>(u + 0) * texel, static_cast<float>(v + height) * texel);
    tessellator.vertex(x + width, y + height, depth, static_cast<float>(u + width) * texel,
        static_cast<float>(v + height) * texel);
    tessellator.vertex(x + width, y + 0, depth, static_cast<float>(u + width) * texel, static_cast<float>(v + 0) * texel);
