...
constexpr int kGlTexture2D = 3553;
    ...
    const net::minecraft::map::MapState& mapState)
{
    ...
    gl::GL11::glBindTexture(kGlTexture2D, texture_);
    ...
    tessellator.vertex(static_cast<float>(originX) + inset, static_cast<float>(originY + 128) - inset, -0.01f, 0.0f,
        1.0f);
    tessellator.vertex(static_cast<float>(originX + 128) - inset, static_cast<float>(originY + 128) - inset, -0.01f,
        1.0f, 1.0f);
    tessellator.vertex(static_cast<float>(originX + 128) - inset, static_cast<float>(originY) + inset, -0.01f, 1.0f,
        0.0f);
    ...
    for (const net::minecraft::map::MapState::MapIcon& mapIcon : mapState.icons) {
        ...
        gl::GL11::glTranslatef(static_cast<float>(originX) + static_cast<float>(mapIcon.x) / 2.0f + 64.0f,
            static_cast<float>(originY) + static_cast<float>(mapIcon.z) / 2.0f + 64.0f, -0.02f);
        gl::GL11::glRotatef(static_cast<float>(mapIcon.rotation * 360) / 16.0f, 0.0f, 0.0f, 1.0f);
        gl::GL11::glScalef(4.0f, 4.0f, 3.0f);
        gl::GL11::glTranslatef(-0.125f, 0.125f, 0.0f);
        ...
        tessellator.startQuads();
        tessellator.vertex(-1.0f, 1.0f, 0.0f, u0, v0);
        tessellator.vertex(1.0f, 1.0f, 0.0f, u1, v0);
        tessellator.vertex(1.0f, -1.0f, 0.0f, u1, v1);
        tessellator.vertex(-1.0f, -1.0f, 0.0f, u0, v1);
        tessellator.draw();
        gl::GL11::glPopMatrix();
    }
