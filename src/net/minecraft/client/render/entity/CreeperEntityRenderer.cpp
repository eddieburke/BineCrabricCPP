...
bool CreeperEntityRenderer::bindTexture(const net::minecraft::LivingEntity& entity, int layer, float tickDelta) {
 ...
 if(layer == 1) {
  ...
  // NOTE: the charged-creeper aura used a GL_TEXTURE matrix to scroll the
  // overlay; this port has no texture matrix (0x170B is misrouted onto the
...
bool CreeperEntityRenderer::bindDecorationTexture(const net::minecraft::LivingEntity& /*entity*/,
                                                  int /*layer*/,
                                                  float /*tickDelta*/) {
 return false;
}
