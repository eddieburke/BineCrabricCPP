...
public:
  ...
  void init(net::minecraft::World* world,
  ...
  [[nodiscard]] net::minecraft::World* world() const noexcept {
    return world_;
  }
  ...
  static void addPendingRenderer(std::type_index key, std::function<std::unique_ptr<EntityRenderer>()> factory);
  float yaw_ = 0.0f;
  float pitch_ = 0.0f;
  double x_ = 0.0;
  double y_ = 0.0;
