...
{
    delete bipedModel;
    delete blockRenderManager;
}

void EntityRenderer::render(const net::minecraft::Entity& entity, double x, double y, double z, float yaw, float tickDelta)
{
    (void)entity;
    (void)x;
    (void)y;
...
void EntityRenderer::postRender(const net::minecraft::Entity& entity, double x, double y, double z, float /*yaw*/, float tickDelta)
{
    if (dispatcher == nullptr) {
        return;
    }
    const auto& options = dispatcher->options();
    if (options.fancyGraphics && shadowRadius > 0.0f) {
...
void EntityRenderer::renderOnFire(const net::minecraft::Entity& entity, double dx, double dy, double dz, float tickDelta)
{
    (void)tickDelta;
...
void EntityRenderer::renderShadow(const net::minecraft::Entity& entity, double dx, double dy, double dz, float yaw, float tickDelta)
{
    ...
    for (int i = minX; i <= maxX; ++i) {
        for (int j = minY; j <= maxY; ++j) {
            for (int k = minZ; k <= maxZ; ++k) {
                ...
                renderShadowOnBlock(*block, dx, dy + static_cast<double>(entity.getShadowRadius()), dz, i, j, k, yaw,
                    shadowSize, cx, cy + static_cast<double>(entity.getShadowRadius()), cz);
            }
        }
    }
    tessellator.draw();
...
void EntityRenderer::renderShadowOnBlock(net::minecraft::block::Block& block, double dx, double dy, double dz, int x, int y, int z,
    float yaw, float shadowSize, double cx, double cy, double cz)
{
    if (!block.isFullCube()) {
        return;
    }
    ...
    const double yPlane = static_cast<double>(y) + block.minY + cy + 0.015625;
