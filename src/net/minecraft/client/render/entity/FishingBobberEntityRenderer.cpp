    ...
    float tickDelta)
{
    ...
    if (dispatcher->options().thirdPerson) {
        ...
        rodTipX = owner.prevX + (owner.x - owner.prevX) * static_cast<double>(tickDelta) - cosYaw * 0.35 - sinYaw * 0.85;
        rodTipY = owner.prevY + (owner.y - owner.prevY) * static_cast<double>(tickDelta) - 0.45;
        rodTipZ = owner.prevZ + (owner.z - owner.prevZ) * static_cast<double>(tickDelta) - sinYaw * 0.35 + cosYaw * 0.85;
    }
    ...
    for (int i = 0; i <= segments; ++i) {
        ...
        tessellator.vertex(x + deltaX * static_cast<double>(t), y + deltaY * static_cast<double>(t * t + t) * 0.5 + 0.25,
