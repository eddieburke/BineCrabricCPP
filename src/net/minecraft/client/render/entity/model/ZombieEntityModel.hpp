...
public:
    ...
    void setAngles(float limbAngle, float limbDistance, float animationProgress, float headYaw, float headPitch, float scale) override
    {
        ...
        const float f = net::minecraft::util::math::MathHelper::sin(handSwingProgress * kPi);
        const float f2 = net::minecraft::util::math::MathHelper::sin((1.0f - (1.0f - handSwingProgress) * (1.0f - handSwingProgress)) * kPi);
        rightArm.roll = 0.0f;
        leftArm.roll = 0.0f;
        ...
        rightArm.pitch -= f * 1.2f - f2 * 0.4f;
        leftArm.pitch -= f * 1.2f - f2 * 0.4f;
