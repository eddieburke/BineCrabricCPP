...
void WolfEntityModel::render(float limbAngle, float limbDistance, float animationProgress, float headYaw, float headPitch, float scale)
{
    EntityModel::render(limbAngle, limbDistance, animationProgress, headYaw, headPitch, scale);
...
void WolfEntityModel::animateModel(::net::minecraft::entity::LivingEntity& entity, float limbAngle, float limbDistance, float tickDelta)
{
    ...
    tail.yaw = wolfEntity->isAngry() ? 0.0f : net::minecraft::util::math::MathHelper::cos(limbAngle * 0.6662f) * 1.4f * limbDistance;
    if (wolfEntity->isInSittingPose()) {
        ...
        leftHindLeg.pitch = net::minecraft::util::math::MathHelper::cos(limbAngle * 0.6662f + kPi) * 1.4f * limbDistance;
...
void WolfEntityModel::setAngles(float limbAngle, float limbDistance, float animationProgress, float headYaw, float headPitch, float scale)
{
