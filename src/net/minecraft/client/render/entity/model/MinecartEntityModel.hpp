...
public:
  ...
  MinecartEntityModel() {
    ...
    parts[0].addCuboid(static_cast<float>(-bodyWidth / 2), static_cast<float>(-bodyLength / 2), -1.0f, bodyWidth,
                       bodyLength, 2, 0.0f);
    parts[0].setPivot(0.0f, static_cast<float>(bodyPivotY), 0.0f);
    parts[5].addCuboid(static_cast<float>(-bodyWidth / 2 + 1), static_cast<float>(-bodyLength / 2 + 1), -1.0f,
                       bodyWidth - 2, bodyLength - 2, 1, 0.0f);
