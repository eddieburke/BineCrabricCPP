...
public:
  ...
  virtual void render(const net::minecraft::Entity& entity, double x, double y, double z, float yaw,
                      float tickDelta) = 0;
  virtual void setDispatcher(EntityRenderDispatcher* dispatcherIn);
  virtual void postRender(const net::minecraft::Entity& entity, double x, double y, double z, float yaw,
                          float tickDelta);
...
private:
  ...
  void renderShadow(const net::minecraft::Entity& entity, double dx, double dy, double dz, float yaw,
                    float tickDelta);
  [[nodiscard]] net::minecraft::World* getWorld() const;
  void renderShadowOnBlock(net::minecraft::block::Block& block, double dx, double dy, double dz, int x, int y, int z,
