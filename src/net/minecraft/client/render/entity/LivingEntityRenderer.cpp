...
LivingEntityRenderer::LivingEntityRenderer(model::EntityModel* modelIn, float shadowRadiusIn)
    : model(modelIn)
{
...
void LivingEntityRenderer::render(const net::minecraft::Entity& entity, double x, double y, double z, float yaw, float tickDelta)
{
    ...
    gl::GL11::glEnable(32826); // GL_RESCALE_NORMAL
    gl::GL11::glScalef(-1.0f, -1.0f, 1.0f);
    applyScale(*living, tickDelta);
    gl::GL11::glTranslatef(0.0f, -24.0f * scaleUnit - 0.0078125f, 0.0f);
    float limbAngle = living->lastWalkAnimationSpeed + (living->walkAnimationSpeed - living->lastWalkAnimationSpeed) * tickDelta;
    ...
    if (((overlay >> 24) & 0xFF) > 0 || living->hurtTime > 0 || living->deathTime > 0) {
        ...
        gl::GL11::glDepthFunc(515);
        gl::GL11::glDisable(gl::GL11::GL_BLEND);
        gl::GL11::glEnable(gl::GL11::GL_ALPHA_TEST);
        gl::GL11::glEnable(gl::GL11::GL_TEXTURE_2D);
    }
    gl::GL11::glDisable(32826); // GL_RESCALE_NORMAL
...
void LivingEntityRenderer::applyHandSwingRotation(const net::minecraft::LivingEntity& entity, float /*headBob*/, float bodyYaw,
    float tickDelta)
{
...
int LivingEntityRenderer::getOverlayColor(const net::minecraft::LivingEntity& entity, float brightness, float tickDelta) const
{
    (void)entity;
    (void)brightness;
    (void)tickDelta;
