...
namespace {
std::string entityTypeName(const net::minecraft::entity::LivingEntity& entity) {
...
void LivingEntityRenderer::render(const net::minecraft::Entity& entity, double x, double y, double z, float /*yaw*/,
  ...
  gl::MatrixGuard entityMatrix;
  const gl::preset::EntityPassThrough passCaps;
  ...
  if(net::minecraft::mod::hooks().hasListeners<net::minecraft::mod::EntityRenderEvent>()) {
  ...
  if(poseScale != 1.0f) {
    gl::scalef(poseScale, poseScale, poseScale);
  }
  gl::translatef(0.0f, -24.0f * scaleUnit - 0.0078125f, 0.0f);
  ...
  if(((overlay >> 24) & 0xFF) > 0 || living->hurtTime > 0 || living->deathTime > 0) {
    const gl::preset::EntityHurtOverlay hurtCaps;
    if(living->hurtTime > 0 || living->deathTime > 0) {
      gl::color4f(brightness, 0.0f, 0.0f, 0.4f);
      model->render(limbDistance, limbAngle, headBob, headYawRel, headPitch, scaleUnit);
      for(int layer = 0; layer < 4; ++layer) {
    ...
    gl::depthFunc(gl::compare::Lequal);
    gl::setCap(gl::cap::Blend, false);
    gl::setCap(gl::cap::AlphaTest, true);
...
void LivingEntityRenderer::applyHandSwingRotation(const net::minecraft::LivingEntity& entity, float /*headBob*/,
  ...
  gl::rotatef(180.0f - bodyYaw, 0.0f, 1.0f, 0.0f);
  if(entity.deathTime > 0) {
    ...
    gl::rotatef(deathAnim * getDeathYaw(entity), 0.0f, 0.0f, 1.0f);
  }
}
