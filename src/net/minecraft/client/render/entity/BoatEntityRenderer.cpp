...
void BoatEntityRenderer::render(const net::minecraft::Entity& entity, double x, double y, double z, float yaw, float tickDelta)
{
    const auto* boat = dynamic_cast<const casts::BoatEntity*>(&entity);
    ...
    gl::GL11::glRotatef(180.0f - yaw, 0.0f, 1.0f, 0.0f);
    ...
    if (wobbleTicks > 0.0f) {
        gl::GL11::glRotatef(
            MathHelper::sin(wobbleTicks) * wobbleTicks * wobbleStrength / 10.0f * static_cast<float>(boat->damageWobbleSide),
            1.0f, 0.0f, 0.0f);
    }
    bindTexture("/terrain.png");
    constexpr float scalePass = 0.75f;
    gl::GL11::glScalef(scalePass, scalePass, scalePass);
    gl::GL11::glScalef(1.0f / scalePass, 1.0f / scalePass, 1.0f / scalePass);
