...
void PrecipitationRenderer::renderPrecipitation(const AtmosphereContext& ctx, float tickDelta) {
  ...
  for(int x = centerX - radius; x <= centerX + radius; ++x) {
    for(int z = centerZ - radius; z <= centerZ + radius; ++z) {
      ...
      tessellator.vertex(x + 1, high, static_cast<double>(z) + 0.5, 1.0f * texScale + uOff,
                         static_cast<float>(high) * texScale / 4.0f + scroll * texScale + vOff);
      tessellator.vertex(x + 1, highCap, static_cast<double>(z) + 0.5, 1.0f * texScale + uOff,
                         static_cast<float>(highCap) * texScale / 4.0f + scroll * texScale + vOff);
      tessellator.vertex(x + 0, highCap, static_cast<double>(z) + 0.5, 0.0f * texScale + uOff,
                         static_cast<float>(highCap) * texScale / 4.0f + scroll * texScale + vOff);
      tessellator.vertex(static_cast<double>(x) + 0.5, high, z + 0, 0.0f * texScale + uOff,
                         static_cast<float>(high) * texScale / 4.0f + scroll * texScale + vOff);
      tessellator.vertex(static_cast<double>(x) + 0.5, high, z + 1, 1.0f * texScale + uOff,
                         static_cast<float>(high) * texScale / 4.0f + scroll * texScale + vOff);
      tessellator.vertex(static_cast<double>(x) + 0.5, highCap, z + 1, 1.0f * texScale + uOff,
                         static_cast<float>(highCap) * texScale / 4.0f + scroll * texScale + vOff);
      tessellator.vertex(static_cast<double>(x) + 0.5, highCap, z + 0, 0.0f * texScale + uOff,
                         static_cast<float>(highCap) * texScale / 4.0f + scroll * texScale + vOff);
    }
  }
  tessellator.draw();
  ...
  for(int x = centerX - radius; x <= centerX + radius; ++x) {
    for(int z = centerZ - radius; z <= centerZ + radius; ++z) {
      ...
      tessellator.vertex(x + 1, low, static_cast<double>(z) + 0.5, 1.0f * texScale,
                         static_cast<float>(low) * texScale / 4.0f + anim * texScale);
      tessellator.vertex(x + 1, high, static_cast<double>(z) + 0.5, 1.0f * texScale,
                         static_cast<float>(high) * texScale / 4.0f + anim * texScale);
      tessellator.vertex(x + 0, high, static_cast<double>(z) + 0.5, 0.0f * texScale,
                         static_cast<float>(high) * texScale / 4.0f + anim * texScale);
      tessellator.vertex(static_cast<double>(x) + 0.5, low, z + 0, 0.0f * texScale,
                         static_cast<float>(low) * texScale / 4.0f + anim * texScale);
      tessellator.vertex(static_cast<double>(x) + 0.5, low, z + 1, 1.0f * texScale,
                         static_cast<float>(low) * texScale / 4.0f + anim * texScale);
      tessellator.vertex(static_cast<double>(x) + 0.5, high, z + 1, 1.0f * texScale,
                         static_cast<float>(high) * texScale / 4.0f + anim * texScale);
      tessellator.vertex(static_cast<double>(x) + 0.5, high, z + 0, 0.0f * texScale,
                         static_cast<float>(high) * texScale / 4.0f + anim * texScale);
    }
  }
  tessellator.draw();
