...
void PrecipitationRenderer::renderPrecipitation(const AtmosphereContext& ctx, float tickDelta)
{
    ...
    gl::GL11::glBindTexture(
        gl::GL11::GL_TEXTURE_2D, ctx.textureManager->getTextureId("/environment/snow.png"));
    ...
    for (int x = centerX - radius; x <= centerX + radius; ++x) {
        for (int z = centerZ - radius; z <= centerZ + radius; ++z) {
            ...
            random_.setSeed(
                static_cast<std::uint64_t>(x * x * 3121 + x * 45238971 + z * z * 418711 + z * 13761));
            ...
            const float uOff =
                random_.nextFloat() + phase * 0.01f * static_cast<float>(random_.nextGaussian());
            const float vOff =
                random_.nextFloat() + phase * static_cast<float>(random_.nextGaussian()) * 0.001f;

            const double dx =
                static_cast<double>(static_cast<float>(x) + 0.5f) - ctx.livingCamera->x;
            const double dz =
                static_cast<double>(static_cast<float>(z) + 0.5f) - ctx.livingCamera->z;
            const float dist =
                MathHelper::sqrt(static_cast<float>(dx * dx + dz * dz)) / static_cast<float>(radius);

            tessellator.startQuads();
            const float brightness = world->getLightBrightness(x, low, z);
            gl::GL11::glColor4f(brightness, brightness, brightness,
                ((1.0f - dist * dist) * 0.3f + 0.5f) * rain);
            tessellator.translate(-interpX, -interpY, -interpZ);
            constexpr float texScale = 1.0f;
            tessellator.vertex(x + 0, high, static_cast<double>(z) + 0.5,
                0.0f * texScale + uOff, static_cast<float>(high) * texScale / 4.0f + scroll * texScale + vOff);
            tessellator.vertex(x + 1, high, static_cast<double>(z) + 0.5,
                1.0f * texScale + uOff, static_cast<float>(high) * texScale / 4.0f + scroll * texScale + vOff);
            tessellator.vertex(x + 1, highCap, static_cast<double>(z) + 0.5,
                1.0f * texScale + uOff, static_cast<float>(highCap) * texScale / 4.0f + scroll * texScale + vOff);
            tessellator.vertex(x + 0, highCap, static_cast<double>(z) + 0.5,
                0.0f * texScale + uOff, static_cast<float>(highCap) * texScale / 4.0f + scroll * texScale + vOff);
            tessellator.vertex(static_cast<double>(x) + 0.5, high, z + 0,
                0.0f * texScale + uOff, static_cast<float>(high) * texScale / 4.0f + scroll * texScale + vOff);
            tessellator.vertex(static_cast<double>(x) + 0.5, high, z + 1,
                1.0f * texScale + uOff, static_cast<float>(high) * texScale / 4.0f + scroll * texScale + vOff);
            tessellator.vertex(static_cast<double>(x) + 0.5, highCap, z + 1,
                1.0f * texScale + uOff, static_cast<float>(highCap) * texScale / 4.0f + scroll * texScale + vOff);
            tessellator.vertex(static_cast<double>(x) + 0.5, highCap, z + 0,
                0.0f * texScale + uOff, static_cast<float>(highCap) * texScale / 4.0f + scroll * texScale + vOff);
    ...
    gl::GL11::glBindTexture(
        gl::GL11::GL_TEXTURE_2D, ctx.textureManager->getTextureId("/environment/rain.png"));
    ...
    for (int x = centerX - radius; x <= centerX + radius; ++x) {
        for (int z = centerZ - radius; z <= centerZ + radius; ++z) {
            ...
            const float anim =
                (static_cast<float>(
                     (ctx.atmosphereTicks + x * x * 3121 + x * 45238971 + z * z * 418711
                         + z * 13761)
                     & 0x1F)
                    + tickDelta)
            ...
            gl::GL11::glColor4f(brightness, brightness, brightness,
                ((1.0f - dist * dist) * 0.5f + 0.5f) * rain);
            tessellator.translate(-interpX, -interpY, -interpZ);
            constexpr float texScale = 1.0f;
            tessellator.vertex(x + 0, low, static_cast<double>(z) + 0.5,
                0.0f * texScale, static_cast<float>(low) * texScale / 4.0f + anim * texScale);
            tessellator.vertex(x + 1, low, static_cast<double>(z) + 0.5,
                1.0f * texScale, static_cast<float>(low) * texScale / 4.0f + anim * texScale);
            tessellator.vertex(x + 1, high, static_cast<double>(z) + 0.5,
                1.0f * texScale, static_cast<float>(high) * texScale / 4.0f + anim * texScale);
            tessellator.vertex(x + 0, high, static_cast<double>(z) + 0.5,
                0.0f * texScale, static_cast<float>(high) * texScale / 4.0f + anim * texScale);
            tessellator.vertex(static_cast<double>(x) + 0.5, low, z + 0,
                0.0f * texScale, static_cast<float>(low) * texScale / 4.0f + anim * texScale);
            tessellator.vertex(static_cast<double>(x) + 0.5, low, z + 1,
                1.0f * texScale, static_cast<float>(low) * texScale / 4.0f + anim * texScale);
            tessellator.vertex(static_cast<double>(x) + 0.5, high, z + 1,
                1.0f * texScale, static_cast<float>(high) * texScale / 4.0f + anim * texScale);
            tessellator.vertex(static_cast<double>(x) + 0.5, high, z + 0,
