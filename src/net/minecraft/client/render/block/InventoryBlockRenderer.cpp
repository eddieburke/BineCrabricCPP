...
void InventoryBlockRenderer::render(net::minecraft::block::Block& block, int metadata, float brightness) {
  ...
  if(renderType == BlockRenderType::FULL_CUBE || renderType == BlockRenderType::PISTON) {
    ...
    for(int i = 0; i < 4; ++i) {
      ...
      if(i == 0) {
        ctx_.setRenderBounds(0.5f - postHalfWidth, 0.0f, 0.0f, 0.5f + postHalfWidth, 1.0f,
                             postHalfWidth * 2.0f);
      }
      if(i == 1) {
        ctx_.setRenderBounds(0.5f - postHalfWidth, 0.0f, 1.0f - postHalfWidth * 2.0f, 0.5f + postHalfWidth,
                             1.0f, 1.0f);
      }
      postHalfWidth = 0.0625f;
      if(i == 2) {
        ctx_.setRenderBounds(0.5f - postHalfWidth, 1.0f - postHalfWidth * 3.0f, -postHalfWidth * 2.0f,
                             0.5f + postHalfWidth, 1.0f - postHalfWidth, 1.0f + postHalfWidth * 2.0f);
      }
      if(i == 3) {
