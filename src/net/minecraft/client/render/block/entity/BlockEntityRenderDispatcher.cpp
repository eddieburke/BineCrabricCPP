...
BlockEntityRenderDispatcher::BlockEntityRenderDispatcher()
{
    ...
    registerRenderer<net::minecraft::block::entity::MobSpawnerBlockEntity>(std::make_unique<MobSpawnerBlockEntityRenderer>());
    registerRenderer<net::minecraft::block::entity::PistonBlockEntity>(std::make_unique<PistonBlockEntityRenderer>());
}

void BlockEntityRenderDispatcher::prepare(net::minecraft::World* worldIn, ::net::minecraft::client::texture::TextureManager* textureManagerIn,
...
void BlockEntityRenderDispatcher::render(const net::minecraft::block::entity::BlockEntity& blockEntity, float tickDelta)
{
    if (blockEntity.distanceFrom(cameraX, cameraY, cameraZ) < 4096.0) {
        net::minecraft::WorldBlockViewAdapter view(world);
        const float brightness = view.getLightBrightness(blockEntity.x, blockEntity.y, blockEntity.z);
