...
void SignBlockEntityRenderer::render(const net::minecraft::block::entity::BlockEntity& blockEntity, double x, double y, double z, float tickDelta)
{
    (void)tickDelta;
    ...
    if (block == net::minecraft::block::Block::SIGN) {
        gl::GL11::glTranslatef(static_cast<float>(x) + 0.5f, static_cast<float>(y) + 0.75f * scale, static_cast<float>(z) + 0.5f);
    ...
    for (int i = 0; i < 4; ++i) {
        ...
        textRenderer->draw(line, -textRenderer->getWidth(line) / 2, i * 10 - static_cast<int>(sign->texts.size()) * 5, color);
    }
