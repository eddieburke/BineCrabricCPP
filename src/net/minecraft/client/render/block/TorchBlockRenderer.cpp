...
bool TorchBlockRenderer::render(net::minecraft::block::Block& block, int x, int y, int z) {
    ...
    if (blockMeta == 1) {
        renderTiltedTorch(
            block, static_cast<double>(x) - wallInset, static_cast<double>(y) + yOffset, z, -tiltAmount, 0.0);
    } else if (blockMeta == 2) {
        renderTiltedTorch(
            block, static_cast<double>(x) + wallInset, static_cast<double>(y) + yOffset, z, tiltAmount, 0.0);
    } else if (blockMeta == 3) {
        renderTiltedTorch(
            block, x, static_cast<double>(y) + yOffset, static_cast<double>(z) - wallInset, 0.0, -tiltAmount);
    } else if (blockMeta == 4) {
        renderTiltedTorch(
            block, x, static_cast<double>(y) + yOffset, static_cast<double>(z) + wallInset, 0.0, tiltAmount);
    } else {
...
void TorchBlockRenderer::renderTiltedTorch(
    ...
    tessellator.vertex(x + xTilt * (1.0 - capHeight) - stickHalf,
                       y + capHeight,
                       z + zTilt * (1.0 - capHeight) - stickHalf,
                       capUMin,
                       capUMax);
    tessellator.vertex(x + xTilt * (1.0 - capHeight) - stickHalf,
                       y + capHeight,
