...
bool RedstoneDustBlockRenderer::render(net::minecraft::block::Block& block, int x, int y, int z)
{
    ...
    bool connectSouth = net::minecraft::block::RedstoneWireBlock::shouldConnectTo(ctx_.blockView, x + 1, y, z, 3)
        || (!ctx_.blockView->shouldSuffocate(x + 1, y, z)
            && net::minecraft::block::RedstoneWireBlock::shouldConnectTo(ctx_.blockView, x + 1, y - 1, z, -1));
    ...
    if (!ctx_.blockView->shouldSuffocate(x, y + 1, z)) {
        if (ctx_.blockView->shouldSuffocate(x - 1, y, z) && net::minecraft::block::RedstoneWireBlock::shouldConnectTo(ctx_.blockView, x - 1, y + 1, z, -1)) {
            connectNorth = true;
        }
        if (ctx_.blockView->shouldSuffocate(x + 1, y, z) && net::minecraft::block::RedstoneWireBlock::shouldConnectTo(ctx_.blockView, x + 1, y + 1, z, -1)) {
            connectSouth = true;
        }
        if (ctx_.blockView->shouldSuffocate(x, y, z - 1) && net::minecraft::block::RedstoneWireBlock::shouldConnectTo(ctx_.blockView, x, y + 1, z - 1, -1)) {
            connectEast = true;
        }
        if (ctx_.blockView->shouldSuffocate(x, y, z + 1) && net::minecraft::block::RedstoneWireBlock::shouldConnectTo(ctx_.blockView, x, y + 1, z + 1, -1)) {
            connectWest = true;
