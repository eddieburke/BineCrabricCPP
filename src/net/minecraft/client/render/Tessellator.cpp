...
void Tessellator::startQuads() { start(kGlQuads); }
...
void Tessellator::color(const float r, const float g, const float b) { color(clamp255(r), clamp255(g), clamp255(b)); }
...
void Tessellator::color(const int r, const int g, const int b) { color(r, g, b, 255); }
void Tessellator::color(const int r, const int g, const int b, const int a) {
  if(colorDisabled_) return;
  hasColor_ = true;
...
void Tessellator::color(const int rgb) { color((rgb >> 16) & 0xFF, (rgb >> 8) & 0xFF, rgb & 0xFF); }
void Tessellator::color(const int rgb, const int a) { color((rgb >> 16) & 0xFF, (rgb >> 8) & 0xFF, rgb & 0xFF, a); }
void Tessellator::disableColor() { colorDisabled_ = true; }
void Tessellator::normal(const float x, const float y, const float z) {
  hasNormals_ = true;
  currentNormal_ = static_cast<std::int32_t>(static_cast<std::uint8_t>(static_cast<std::int8_t>(x * 128.0f))) |
...
void Tessellator::vertex(const double x, const double y, const double z) {
  ...
  pushVertex(vx);
  if(vertices_.size() >= flushThreshold_ && vertices_.size() % 4 == 0) flush();
}
...
void Tessellator::pushVertex(const TessellatorVertex& v) { vertices_.push_back(v); }
void Tessellator::flush() {
  if(captureOnly_) return;
...
bool TessellatorMesh::uploadToGpu() {
  gl::GLCore::init();
  if(!gl::GLCore::vboSupported || vertices.empty()) return false;
  freeGpuBuffer();
  gl::GLCore::genBuffers(1, &vbo_);
  if(!vbo_) return false;
  gl::GLCore::bindBuffer(0x8892, vbo_);
  gl::GLCore::bufferData(0x8892, static_cast<intptr_t>(vertices.size() * sizeof(TessellatorVertex)), vertices.data(),
                         0x88E4);
  gl::GLCore::bindBuffer(0x8892, 0);
  return true;
}
