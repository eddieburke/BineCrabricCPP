...
TessellatorMesh::TessellatorMesh(const TessellatorMesh& other)
    : vertices(other.vertices), mode(other.mode),
      hasTexture(other.hasTexture), hasColor(other.hasColor), hasNormals(other.hasNormals), vbo_(0) {}
...
TessellatorMesh::TessellatorMesh(TessellatorMesh&& other) noexcept
    : vertices(std::move(other.vertices)), mode(other.mode),
      hasTexture(other.hasTexture), hasColor(other.hasColor), hasNormals(other.hasNormals), vbo_(other.vbo_) {
 other.vbo_ = 0;
}
...
void Tessellator::expandQuadToTriangles() {
 ...
 if(buf.size() < 3 * sizeof(TessellatorVertex)) return;
...
void Tessellator::color(int r, int g, int b, int a) {
 ...
 hasColor_ = true;
 currentColor_ = (static_cast<std::uint32_t>(a & 0xFF) << 24U) |
...
void Tessellator::normal(float x, float y, float z) {
 hasNormals_ = true;
 currentNormal_ = static_cast<std::int32_t>(static_cast<std::uint8_t>(static_cast<std::int8_t>(x * 127.0f))) |
...
void Tessellator::vertex(double x, double y, double z) {
 ...
 if(hasTexture_) vProxy.tex(u_, v_);
 if(hasColor_) vProxy.color(currentColor_);
 if(hasNormals_) {
