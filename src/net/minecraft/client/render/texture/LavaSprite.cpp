...
LavaSprite::LavaSprite() : LiquidSprite(detail::blockTextureId(10, 14 * 16 + 13)) {
}
void LavaSprite::tick() {
  for(int x = 0; x < 16; ++x) {
    for(int y = 0; y < 16; ++y) {
      float sum = 0.0f;
      ...
      next[static_cast<std::size_t>(x + y * 16)] =
          sum / 10.0f + (heat[static_cast<std::size_t>(((x + 0) & 0xF) + ((y + 0) & 0xF) * 16)] +
  ...
  for(int pixelIndex = 0; pixelIndex < 256; ++pixelIndex) {
    float brightness = current[static_cast<std::size_t>(pixelIndex)] * 2.0f;
