...
NetherPortalSprite::NetherPortalSprite() : DynamicTexture(detail::blockTextureId(90, 14)) {
  ...
  for(int frameIndex = 0; frameIndex < 32; ++frameIndex) {
    for(int texU = 0; texU < 16; ++texU) {
      for(int texV = 0; texV < 16; ++texV) {
        ...
        frames[static_cast<std::size_t>(frameIndex)][static_cast<std::size_t>(pixelIndex) * 4 + 0] =
            static_cast<std::uint8_t>(green);
        frames[static_cast<std::size_t>(frameIndex)][static_cast<std::size_t>(pixelIndex) * 4 + 1] =
            static_cast<std::uint8_t>(red);
        frames[static_cast<std::size_t>(frameIndex)][static_cast<std::size_t>(pixelIndex) * 4 + 2] =
            static_cast<std::uint8_t>(blue);
        frames[static_cast<std::size_t>(frameIndex)][static_cast<std::size_t>(pixelIndex) * 4 + 3] =
