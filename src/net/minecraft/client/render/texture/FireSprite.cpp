...
FireSprite::FireSprite(int index) : DynamicTexture(detail::blockTextureId(51, 31) + index * 16) {
}
...
void FireSprite::tick() {
    for (int x = 0; x < 16; ++x) {
        for (int y = 0; y < 20; ++y) {
            ...
            float heatSum =
                current[static_cast<std::size_t>(x + (y + 1) % 20 * 16)] * static_cast<float>(neighborCount);
            ...
            next[static_cast<std::size_t>(x + y * 16)] =
                static_cast<float>(detail::mathRandom() * detail::mathRandom() * detail::mathRandom() * 4.0 +
