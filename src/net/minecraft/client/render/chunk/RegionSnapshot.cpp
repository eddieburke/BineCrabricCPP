...
void copyNibbleColumn(std::uint8_t* dst, std::size_t dstColumn, int halfSpan,
                      const std::vector<std::uint8_t>& src, std::size_t srcByteBase, bool atomicRead) {
...
void copyChunkBand(std::vector<std::uint8_t>& blocks, std::vector<std::uint8_t>& meta, std::vector<std::uint8_t>& skyLight,
                   std::vector<std::uint8_t>& blockLight, const Chunk& chunk, int minY, int ySpan, int halfSpan,
                   std::size_t blockBytes, std::size_t nibbleBytes) {
  ...
  for(int column = 0; column < 16 * 16; ++column) {
    ...
    std::memcpy(blocks.data() + dstColumn * static_cast<std::size_t>(ySpan), chunk.blocks.data() + srcBlockBase,
...
RegionSnapshot::RegionSnapshot(std::span<const SourceChunk> sourceChunks, int ambientDarkness,
                               ...
                               std::unique_ptr<net::minecraft::BiomeSource> biomeSource, int minBlockX, int minBlockY,
                               int minBlockZ, int maxBlockX, int maxBlockY, int maxBlockZ) {
  chunkX_ = minBlockX >> 4;
  chunkZ_ = minBlockZ >> 4;
  ...
  for(const SourceChunk& source : sourceChunks) {
    ...
    copy.present = true;
    copyChunkBand(copy.blocks, copy.meta, copy.skyLight, copy.blockLight, chunk, minY_, ySpan_, halfSpan, blockBytes,
                  nibbleBytes);
  }
