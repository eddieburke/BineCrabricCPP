...
std::uint64_t computeVisibilityBits(const RegionSnapshot& snapshot, int minX, int minY, int minZ) {
 ...
 for(int start = 0; start < 4096; ++start) {
  ...
  while(!stack.empty()) {
   ...
   const int cy = cell >> 8;
   if(cx == 0) faceMask |= 1U << 0;
   if(cx == 15) faceMask |= 1U << 1;
   if(cy == 0) faceMask |= 1U << 2;
   if(cy == 15) faceMask |= 1U << 3;
   if(cz == 0) faceMask |= 1U << 4;
   if(cz == 15) faceMask |= 1U << 5;
   ...
   if(cx > 0) tryVisit(cell - 1);
   if(cx < 15) tryVisit(cell + 1);
   if(cy > 0) tryVisit(cell - 256);
   if(cy < 15) tryVisit(cell + 256);
   if(cz > 0) tryVisit(cell - 16);
   if(cz < 15) tryVisit(cell + 16);
  }
  for(int a = 0; a < 6; ++a) {
   if((faceMask & (1U << a)) == 0) {
    continue;
