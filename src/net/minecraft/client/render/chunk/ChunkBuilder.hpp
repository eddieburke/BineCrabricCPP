...
public:
  ChunkBuilder(World* world, std::vector<::net::minecraft::block::entity::BlockEntity*>& blockEntityUpdateList, int x,
               int y, int z, int size, ChunkRegionManager* regionManager)
      : world(world), regionManager_(regionManager), x(x), y(y), z(z),
    ...
    cullingBox = net::minecraft::Box(
        static_cast<double>(this->x) - padding, static_cast<double>(this->y) - padding,
        static_cast<double>(this->z) - padding, static_cast<double>(this->x + sizeX) + padding,
        static_cast<double>(this->y + sizeY) + padding, static_cast<double>(this->z + sizeZ) + padding);
    dirty = false;
  }
