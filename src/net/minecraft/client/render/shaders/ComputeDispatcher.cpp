...
bool dispatch(PackInstance& pack,
 ...
 profiler.record(debug::RenderMetric::ComputeDispatches);
 profiler.record(debug::RenderMetric::ComputeWorkgroups,
                 static_cast<std::uint64_t>(groups[0]) * groups[1] * groups[2]);
 ...
 if(useIndirect) {
  ...
  profiler.record(debug::RenderMetric::MemoryBarriers);
