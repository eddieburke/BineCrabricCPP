...
// The addon's contract macros: every clrwl_* program sees HAS_COLORWHEEL and the
// CLRWL_RENDER_STAGE_* ids. Pack sources branch on HAS_COLORWHEEL and pass the
// stage ids to clrwl_setRenderStage; both live here so the engine's own preamble
// stays independent of the addon.
void insertContractMacros(std::string& source) {
 insertOnce(source, "_CLRWL_MERGE_MACROS",
            GlslSnippets::get("colorwheel_macros_prefix") + GlslSnippets::get("colorwheel_stage_defines"));
}
} // namespace
