...
bool normalizeDefault(PackSetting& setting) {
 ...
 setting.defaultValue =
     setting.type == SettingType::Int ? std::to_string(static_cast<int>(std::lround(value))) : std::to_string(value);
 return true;
}
} // namespace
bool PackManifest::parse(const std::string& text, PackManifest& out, std::string& error) {
 ...
 if(settings.isArray()) {
  for(std::size_t i = 0; i < settings.size(); ++i) {
   ...
   if(!validIdentifier(setting.key) ||
      std::any_of(out.settings.begin(), out.settings.end(), [&](const PackSetting& current) {
       return current.key == setting.key;
      })) {
    continue;
   }
 ...
 if(passes.isArray()) {
  ...
  for(std::size_t i = 0; i < passes.size(); ++i) {
   ...
   } else if(pass.type == "entities") {
    if(programSpec.stage != "entities") {
     error = "pass " + pass.name + " must use an entities program";
     return false;
    }
    if(programSpec.vertex.empty() || programSpec.fragment.empty()) {
     error = "pass " + pass.name + " entities program has incomplete sources";
     return false;
    }
