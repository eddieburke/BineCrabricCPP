...
class UnitAllocator {
 ...
 bool bind(gl::ShaderProgram& program,
...
void ShaderPackManager::indexPasses(Pack& pack) {
 for(std::size_t i = 0; i < pack.manifest.passes.size(); ++i) {
  ...
  if(pass.type == "post" && !pass.program.empty()) {
   ...
   pack.terrainPass = i;
  } else if(pass.type == "entities" && !pass.program.empty() && !pack.entitiesPass.has_value()) {
   pack.entitiesPass = i;
  }
 }
...
gl::ShaderProgram* ShaderPackManager::programFromPack(Pack& pack, const std::string& key) {
 ...
 if(key == "gbuffers_entities" && pack.entitiesPass.has_value()) {
  return compileProgram(pack, pack.manifest.passes[*pack.entitiesPass].program);
 }
...
bool ShaderPackManager::renderPostProcess(int textureId,
 ...
 if(world != nullptr) {
  ...
  const Vec3d start{camera.eyeX, camera.eyeY, camera.eyeZ};
  const double rayLength = std::min(static_cast<double>(farPlane), 200.0);
  const Vec3d end{start.x + static_cast<double>(sun.directionX) * rayLength,
                  start.y + static_cast<double>(sun.directionY) * rayLength,
                  start.z + static_cast<double>(sun.directionZ) * rayLength};
  sunVisibility = world->raycast(start, end, true).has_value() ? 0.0f : 1.0f;
 }
 ...
 for(std::size_t passIndex : pack->postPasses) {
  ...
  if(vertex.empty() || fragment.empty()) {
   logOnce(*pack,
           "pass '" + pass.name + "' missing shader source (" + spec.vertex + ", " + spec.fragment + ")");
   continue;
  }
  ...
  if(!toScreen) {
   ...
   const render::ColorFormat format = declared == pack->manifest.targets.end()
                                          ? render::ColorFormat::Rgba8
                                          : parseFormat(declared->second.format);
  ...
  program->set3f("uCameraWorld",
                 static_cast<float>(camera.eyeX),
                 static_cast<float>(camera.eyeY),
  ...
  if(world != nullptr) {
   ...
   const float svy =
       sun.directionX * camera.viewUpX + sun.directionY * camera.viewUpY + sun.directionZ * camera.viewUpZ;
  ...
  if(target != nullptr) {
   ...
   if(isLast) {
    ...
    logOnce(*pack,
            "final pass '" + pass.name + "' writes to '" + pass.output +
