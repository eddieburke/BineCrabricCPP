...
struct EvalContext {
 const ShaderUniformValues* frame = nullptr;
...
bool matrixNamed(const std::string& name, const ShaderUniformValues& frame, const float*& out) {
...
bool lookupBuiltin(const std::string& name, const ShaderUniformValues& frame, CustomUniformValue& out) {
...
CustomUniformValue callFunction(const std::string& name, const std::vector<CustomUniformValue>& args, EvalContext& ctx) {
 ...
 if(name == "min" && need(2)) {
  if(isVector(args[0].type) || isVector(args[1].type)) {
   const CustomUniformType type = isVector(args[0].type) ? args[0].type : args[1].type;
   float out[4]{};
   for(int i = 0; i < vecLen(type); ++i) out[i] = std::min(componentOf(args[0], i), componentOf(args[1], i));
   return makeVec(type, out);
  }
  if(args[0].type == CustomUniformType::Int && args[1].type == CustomUniformType::Int)
   return makeInt(std::min(args[0].asInt(), args[1].asInt()));
  return makeFloat(std::min(f0(0), f0(1)));
 }
 if(name == "max" && need(2)) {
  if(isVector(args[0].type) || isVector(args[1].type)) {
   const CustomUniformType type = isVector(args[0].type) ? args[0].type : args[1].type;
   float out[4]{};
   for(int i = 0; i < vecLen(type); ++i) out[i] = std::max(componentOf(args[0], i), componentOf(args[1], i));
   return makeVec(type, out);
  }
  if(args[0].type == CustomUniformType::Int && args[1].type == CustomUniformType::Int)
   return makeInt(std::max(args[0].asInt(), args[1].asInt()));
  return makeFloat(std::max(f0(0), f0(1)));
 }
...
void CustomUniformRuntime::evaluate(const ShaderUniformValues& frame) {
 values_.clear();
