...
void GameRenderer::applyFog(int mode) {
    ...
    if (living != nullptr && living->isInFluid(::net::minecraft::block::material::Material::WATER)) {
        ...
        if (mode < 0) {
            ...
            gl::fogf(gl::fog::Start, resolved.renderDistanceBlocks * (resolved.customFog ? resolved.fogStart : 0.25f));
            gl::fogf(gl::fog::End, resolved.renderDistanceBlocks * (resolved.customFog ? resolved.fogEnd : 1.0f));
        }
