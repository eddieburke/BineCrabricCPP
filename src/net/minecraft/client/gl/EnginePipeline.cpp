...
void submit(const RenderPass& pass) {
 ...
 if(pass.buffer != 0) {
  ...
  configureAttribs(
      g_streamVbo, 0, pass.stride, pass.hasTexture, pass.hasColor, pass.hasNormals, pass.hasLightmap);
  ::glDrawArrays(static_cast<GLenum>(pass.glMode), 0, static_cast<GLsizei>(pass.vertexCount));
 }
}
...
void configureAttribs(unsigned buffer,
 ...
 const bool lightmapInStride = hasLightmap && static_cast<std::size_t>(stride) >= kOffLight + 2 * sizeof(float);
 ...
 if(!cached) {
  ...
  g_attribCache =
      AttribCache{buffer, baseOffset, stride, hasTexture, hasColor, hasNormals, lightmapInStride, buffer != 0};
 }
 if(!hasNormals) {
