...
public:
  ...
  Chunk& loadChunk(int chunkX, int chunkZ) override {
    ...
    if(slot != nullptr) {
      // Replacing frees the old chunk; fence it off from the async lighting worker (non-blocking try-pin) before it is destroyed.
...
private:
  // The lighting worker pins chunks it touches via the shared render-pin. Mark the chunk evicting (so the worker stops touching it) and spin only until any in-flight pin is dropped at the worker's next box boundary, which is bounded and rare. This replaces the engine's old blocking unregister wait.
