...
public:
    ...
    explicit Quad(const std::array<Vertex, 4>& verticesIn)
        : vertices(verticesIn.begin(), verticesIn.end()),
          verticesCount(4)
    {
    }

    Quad(std::vector<Vertex> verticesIn, int u1, int v1, int u2, int v2)
        : Quad(std::move(verticesIn))
    {
        remap(u1, v1, u2, v2);
    }

    Quad(const std::array<Vertex, 4>& verticesIn, int u1, int v1, int u2, int v2)
        : Quad(verticesIn)
    {
        remap(u1, v1, u2, v2);
    }
