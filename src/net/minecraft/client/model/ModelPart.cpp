...
ModelPart::ModelPart(const ModelPart& other)
    : textureU(other.textureU), textureV(other.textureV), mirror(other.mirror), visible(other.visible),
      hidden(other.hidden), pitch(other.pitch), yaw(other.yaw), roll(other.roll), pivotX(other.pivotX),
      pivotY(other.pivotY), pivotZ(other.pivotZ), faces_(other.faces_), children_(other.children_) {
}
...
void ModelPart::addCuboid(float x, float y, float z, int sizeX, int sizeY, int sizeZ, float dilation) {
  ...
  std::array<Quad, 6> newFaces{
      ...
      Quad(std::array<Vertex, 4>{vertex6, vertex5, vertex, vertex2}, textureU + sizeZ, textureV + 0,
           textureU + sizeZ + sizeX, textureV + sizeZ),
      ...
      Quad(std::array<Vertex, 4>{vertex2, vertex, vertex4, vertex3}, textureU + sizeZ, textureV + sizeZ,
           textureU + sizeZ + sizeX, textureV + sizeZ + sizeY),
      Quad(std::array<Vertex, 4>{vertex5, vertex6, vertex7, vertex8}, textureU + sizeZ + sizeX + sizeZ,
           textureV + sizeZ, textureU + sizeZ + sizeX + sizeZ + sizeX, textureV + sizeZ + sizeY),
  };
  if(mirror) {
...
void ModelPart::renderFaces(float scale) const {
  if(faces_.empty()) {
    return;
  }
