...
void uploadDynamicMipmapLevels(int sprite, std::vector<std::uint8_t>& imageBuffer) {
    for (int level = 1; level <= 4; ++level) {
        ...
        gl::texSubImage2D(gl::cap::Texture2D,
                          level,
...
RasterImage TextureManager::loadRasterForResource(const std::string& resourcePath) {
    ...
    if (const std::vector<std::uint8_t> modBytes = mod::runtime::host().readResource(normalized); !modBytes.empty()) {
        return loadRasterFromBytes(modBytes);
    }
...
void TextureManager::downloadSkinImage(const std::string& url) {
    ...
    downloadImage(url, &skinProcessor, msauth::shouldUseBetacraftProxyForUrl(url));
}
...
bool TextureManager::resourceExists(const std::string& path) const {
    const TexturePathSpec spec = parseTexturePath(path);
    const std::string normalized = normalizeResourcePath(spec.resourcePath);
    if (const std::vector<std::uint8_t> modBytes = mod::runtime::host().readResource(normalized); !modBytes.empty()) {
        return true;
    }
    if (texturePacks_ != nullptr && texturePacks_->selected != nullptr) {
        return !texturePacks_->selected->getResource(normalized).empty();
    }
...
bool shouldTickDynamicTexture(const option::GameOptions* options,
    ...
    if (dynamic_cast<const ::net::minecraft::client::render::texture::WaterSprite*>(texture) != nullptr ||
        ...
        return options->animatedWater == 0;
    }
