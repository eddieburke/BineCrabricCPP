...
TextRenderer::TextRenderer(option::GameOptions& options, const std::string& fontPath,
...
std::unique_ptr<TextRenderer> TextRenderer::create(option::GameOptions& options,
                                                   texture::TextureManager& textureManager, const std::string& fontPath) {
  texture::RasterImage fontImage =
      texture::TextureManager::loadRasterFromFile(texture::TextureManager::resolveResourcePath(fontPath));
  if(fontImage.width > 0 && fontImage.height > 0) {
...
void TextRenderer::appendGlyphQuad(render::Tessellator& tessellator, int glyph, float penX, float r, float g, float b,
                                   float a) const {
  const int u = (glyph % 16) * 8;
  const int v = (glyph / 16) * 8;
  constexpr float size = 7.99f;
  tessellator.color(r, g, b, a);
  tessellator.vertex(static_cast<double>(penX), static_cast<double>(size), 0.0, static_cast<double>(u) / 128.0,
                     (static_cast<double>(v) + size) / 128.0);
  tessellator.vertex(static_cast<double>(penX) + size, static_cast<double>(size), 0.0,
                     (static_cast<double>(u) + size) / 128.0, (static_cast<double>(v) + size) / 128.0);
  tessellator.vertex(static_cast<double>(penX) + size, 0.0, 0.0, (static_cast<double>(u) + size) / 128.0,
                     static_cast<double>(v) / 128.0);
  tessellator.vertex(static_cast<double>(penX), 0.0, 0.0, static_cast<double>(u) / 128.0,
                     static_cast<double>(v) / 128.0);
}
