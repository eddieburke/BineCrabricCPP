...
void LoginScreen::init() {
  ...
  if(phase_ == Phase::CreateProfile) {
    ...
    if(textRenderer() != nullptr && profileNameField_ == nullptr) {
      profileNameField_ = std::make_unique<widget::TextFieldWidget>(this, textRenderer(), width() / 2 - 100, top + 36,
                                                                    200, 20, "");
...
void LoginScreen::beginSignIn() {
  ...
  workerThread_ = std::thread([this, clientId]() {
    ...
    msauth::DeviceCodeRequestResult codeRequest =
        msauth::requestDeviceCode(clientId, &cancelRequested_, progress);
    {
