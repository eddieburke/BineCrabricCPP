...
protected:
  ...
  void renderEntry(int index, int x, int y, int, render::Tessellator&) override {
    ...
    minecraft_.textRenderer->drawWithShadow(value, x + 2 + 213 - minecraft_.textRenderer->getWidth(value), y + 1,
                                            color);
  }
...
public:
  AbstractStatsListWidget(StatsScreen& screen, Minecraft& minecraft, int width, int height, stat::PlayerStats* stats,
                          std::array<StatColumn, 3> columns)
      : EntryListWidget(minecraft, width, height, 32, height - 64, 20), screen_(screen), stats_(stats),
        columns_(columns) {
    setRenderSelectionHighlight(false);
    setHeader(true, 20);
  }
...
protected:
  ...
  void renderHeader(int x, int y, render::Tessellator& tessellator) override {
    ...
    for(std::size_t i = 0; i < columns_.size(); ++i) {
      ...
      screen_.renderIcon(x + 97 + static_cast<int>(i) * 50 + (selected ? 1 : 0), y + (selected ? 2 : 1),
                         columns_[i].iconU, 18);
    }
  }
...
void StatsScreen::createButtons() {
  addActionButton(width() / 2 + 4, layout::listFooterRow2Y(height()), 150, layout::kDefaultButtonHeight,
                  resource::language::I18n::getTranslation("gui.done"), [this] {
                    if(parentFactory_) {
                      navigateTo(parentFactory_);
                    }
                  });
  addActionButton(layout::listFooterLeftX(width()), layout::listFooterRow1Y(height()), 100,
                  layout::kDefaultButtonHeight, resource::language::I18n::getTranslation("stat.generalButton"),
                  [this] { selectedStatsList_ = generalStats_.get(); });
  auto& blocksButton =
      addActionButton(width() / 2 - 46, layout::listFooterRow1Y(height()), 100, layout::kDefaultButtonHeight,
                      resource::language::I18n::getTranslation("stat.blocksButton"),
                      [this] { selectedStatsList_ = blockStats_.get(); });
  auto& itemsButton =
      addActionButton(width() / 2 + 62, layout::listFooterRow1Y(height()), 100, layout::kDefaultButtonHeight,
