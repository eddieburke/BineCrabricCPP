...
void GameMenuScreen::init() {
  ...
  addCenteredActionButton(layout::gameMenuButtonY(h, layout::kGameMenuQuitOffset), quitText,
                          [this] { saveAndQuit(); });
  addCenteredActionButton(layout::gameMenuButtonY(h, layout::kGameMenuBackOffset), "Back to game",
                          [this] { resumeGame(); });
  addCenteredActionButton(layout::gameMenuButtonY(h, layout::kGameMenuOptionsOffset), "Options...",
                          [this] { openOptions(); });
  addActionButton(layout::centerBtnX(w), layout::gameMenuButtonY(h, layout::kGameMenuSplitOffset),
                  layout::kSplitButtonWidth, layout::kDefaultButtonHeight, "Achievements",
                  [this] { openAchievements(); });
  addActionButton(layout::centerBtnX(w) + layout::kSplitButtonWidth + 4,
                  layout::gameMenuButtonY(h, layout::kGameMenuSplitOffset), layout::kSplitButtonWidth,
                  layout::kDefaultButtonHeight, "Stats", [this] { openStats(); });
  if(minecraft() != nullptr && minecraft()->world != nullptr) {
    if(minecraft()->lanHostCoordinator().isHostedWorld(minecraft()->world)) {
      addCenteredActionButton(layout::gameMenuButtonY(h, layout::kGameMenuLanOffset), "LAN Info...",
                              [this] { openLanInfo(); });
    } else if(minecraft()->lanHostCoordinator().canOpenLan()) {
      addCenteredActionButton(layout::gameMenuButtonY(h, layout::kGameMenuLanOffset), "Open to LAN...",
                              [this] { openLanMenu(); });
    }
  }
}
