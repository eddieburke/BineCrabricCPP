...
void GameMenuScreen::init() {
    ...
    if (minecraft() != nullptr && minecraft()->world != nullptr) {
        if (minecraft()->lanHostCoordinator().isHostedWorld(minecraft()->world)) {
            addCenteredActionButton(
                layout::gameMenuButtonY(h, layout::kGameMenuLanOffset), "LAN Info...", [this] { openLanInfo(); });
        } else if (minecraft()->lanHostCoordinator().canOpenLan()) {
            addCenteredActionButton(
                layout::gameMenuButtonY(h, layout::kGameMenuLanOffset), "Open to LAN...", [this] { openLanMenu(); });
        }
    }
}
