...
EditWorldScreen::EditWorldScreen(screen::ScreenFactory parentFactory, std::string worldSaveName)
    : parentFactory_(std::move(parentFactory)),
      worldSaveName_(std::move(worldSaveName))
{
    if (!parentFactory_) {
...
void EditWorldScreen::init()
{
    ...
    if (minecraft() != nullptr) {
        if (WorldStorageSource* storage = minecraft()->getWorldStorageSource()) {
            if (const std::optional<WorldProperties> props = storage->getWorldProperties(worldSaveName_); props.has_value()) {
                initialName = props->getName();
            }
        }
    }
    if (textRenderer() != nullptr) {
        levelNameField_ = std::make_unique<widget::TextFieldWidget>(
            this, textRenderer(), width() / 2 - 100, 60, 200, 20, initialName);
