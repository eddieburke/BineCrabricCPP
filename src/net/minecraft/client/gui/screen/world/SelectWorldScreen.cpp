...
public:
  WorldListWidget(SelectWorldScreen& owner, Minecraft& minecraft, int width, int height)
      : EntryListWidget(minecraft, width, height, 32, height - 64, 36), owner_(owner) {
  }
...
SelectWorldScreen::SelectWorldScreen(screen::ScreenFactory parentFactory) : parentFactory_(std::move(parentFactory)) {
  if(!parentFactory_) {
...
void SelectWorldScreen::getSaves() {
  ...
  std::sort(saves_.begin(), saves_.end(),
            [](const WorldSaveInfo& a, const WorldSaveInfo& b) { return a.compareTo(b) < 0; });
  selectedWorldId_ = -1;
}
void SelectWorldScreen::addButtons() {
  playSelectedWorldButton_ = &addActionButton(
      layout::listFooterLeftX(width()), layout::listFooterRow1Y(height()), 150, layout::kDefaultButtonHeight,
      resource::language::I18n::getTranslation("selectWorld.select"), [this] { selectWorld(selectedWorldId_); });
  renameWorldButton_ = &addActionButton(
      layout::listFooterLeftX(width()), layout::listFooterRow2Y(height()), 70, layout::kDefaultButtonHeight,
      resource::language::I18n::getTranslation("selectWorld.rename"), [this] {
  ...
  deleteWorldButton_ = &addActionButton(
      width() / 2 - 74, layout::listFooterRow2Y(height()), 70, layout::kDefaultButtonHeight,
      resource::language::I18n::getTranslation("selectWorld.delete"), [this] { confirmDeleteWorld(); });
  addActionButton(layout::listFooterRightX(width()), layout::listFooterRow1Y(height()), 150,
                  layout::kDefaultButtonHeight, resource::language::I18n::getTranslation("selectWorld.create"),
                  [this] {
                    navigateTo(std::make_unique<CreateWorldScreen>(
                        [parent = parentFactory_]() { return std::make_unique<SelectWorldScreen>(parent); }));
                  });
  addActionButton(layout::listFooterRightX(width()), layout::listFooterRow2Y(height()), 150,
                  layout::kDefaultButtonHeight, resource::language::I18n::getTranslation("gui.cancel"),
...
void SelectWorldScreen::confirmDeleteWorld() {
  ...
  const std::string warning =
      "'" + worldName + "' " + resource::language::I18n::getTranslation("selectWorld.deleteWarning");
  const std::string deleteButton = resource::language::I18n::getTranslation("selectWorld.deleteButton");
  ...
  navigateTo(std::make_unique<screen::ConfirmScreen>(
      ...
      question, warning, deleteButton, cancelButton));
}
