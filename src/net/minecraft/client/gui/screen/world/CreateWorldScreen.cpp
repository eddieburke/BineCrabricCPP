...
void openCreateWorldHostScreen(const std::unordered_map<std::string, std::string>& fields) {
 ...
 client::Minecraft::INSTANCE->setScreen(
     std::make_unique<CreateWorldScreen>(screen::ScreenFactory{}, std::move(worldName), std::move(seedText)));
}
...
CreateWorldScreen::CreateWorldScreen(screen::ScreenFactory parentFactory,
...
void CreateWorldScreen::init() {
 ...
 addActionButton(
     layout::centerBtnX(width()), nextButtonY, resource::language::I18n::getTranslation("gui.cancel"), [this] {
...
void CreateWorldScreen::createWorld() {
 ...
 net::minecraft::mod::runtime::luaHookCreateWorld(createEvent);
 ...
 const std::string displayName = worldNameField_ != nullptr ? worldNameField_->getText() : worldSaveName_;
