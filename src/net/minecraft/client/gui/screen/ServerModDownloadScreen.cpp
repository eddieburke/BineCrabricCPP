...
void ServerModDownloadScreen::init() {
  buttons_.clear();
  addActionButton(layout::listFooterLeftX(width()), layout::dialogFooterY(height()), layout::kConfirmButtonWidth,
                  layout::kDefaultButtonHeight, "Accept", [this] { accept(); });
  addActionButton(layout::listFooterRightX(width()), layout::dialogFooterY(height()), layout::kConfirmButtonWidth,
...
void ServerModDownloadScreen::render(int mouseX, int mouseY, float tickDelta) {
  ...
  if(textRenderer() != nullptr) {
    ...
    drawCenteredTextWithShadow(*textRenderer(), "This native C++ server can send missing mods for download.",
                               width() / 2, height() / 2 - 52, 0xD0D0D0);
    int y = height() / 2 - 28;
    ...
    if(status_.empty()) {
      drawCenteredTextWithShadow(*textRenderer(), "Accept to download into AppData and install before login.",
                                 width() / 2, height() / 2 + 32, 0xA0A0A0);
    } else {
...
void ServerModDownloadScreen::accept() {
  ...
  if(!handler_->downloadPendingMods(error)) {
    minecraft()->setScreen(std::make_unique<DisconnectedScreen>("connect.failed", "disconnect.genericReason",
