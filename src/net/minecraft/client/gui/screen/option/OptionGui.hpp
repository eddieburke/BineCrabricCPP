...
public:
  ...
  void intCycle(int x, int y, const char* key, const char* title, std::initializer_list<const char*> labels,
  ...
  void i18nCycle(int x, int y, const char* key, const char* title, std::initializer_list<const char*> i18nKeys,
                 int client_option::GameOptions::* member,
                 bool (*isEnabled)(const client_option::GameOptions&) = nullptr) {
    const std::optional<std::size_t> idx = client_option::OptionRegistry::indexOf(key);
    if(!idx) {
  ...
  void slider(int x, int y, const char* key, const char* title,
    ...
    widget::SliderWidget& btn =
        screen_.addButton<widget::SliderWidget>(widget::ActionButtonWidget::kNoId, x, y, *idx, minecraft_,
                                                format(options_), options_.getFloat(key), std::move(format));
    if(isEnabled != nullptr) {
      btn.active = isEnabled(options_);
    }
  }
  void mappedSlider(int x, int y, const char* key, std::function<float(const client_option::GameOptions&)> getValue,
                    std::function<std::string(const client_option::GameOptions&)> format,
                    bool (*isEnabled)(const client_option::GameOptions&) = nullptr) {
    const std::optional<std::size_t> idx = client_option::OptionRegistry::indexOf(key);
    if(!idx) {
    ...
    widget::SliderWidget& btn =
        screen_.addButton<widget::SliderWidget>(widget::ActionButtonWidget::kNoId, x, y, *idx, minecraft_,
                                                format(options_), getValue(options_), std::move(format));
    if(isEnabled != nullptr) {
      btn.active = isEnabled(options_);
    }
  }
