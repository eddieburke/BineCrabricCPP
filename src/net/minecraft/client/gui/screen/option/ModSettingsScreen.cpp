...
class ModSliderWidget : public widget::ButtonWidget {
 ...
 [[nodiscard]] bool isMouseOver(int mouseX, int mouseY) const override {
  ...
  auto* self = const_cast<ModSliderWidget*>(this);
  self->updateValue(mouseX);
  self->dragging_ = true;
  return true;
 }
 ...
 void updateValue(int mouseX) {
  const float position = std::clamp(
      static_cast<float>(mouseX - (x + 4)) / static_cast<float>(std::max(1, width - 8)), 0.0f, 1.0f);
...
ModSettingsScreen::ModSettingsScreen(ParentFactory parentFactory, bool openModPages)
    : parentFactory_(std::move(parentFactory)), openModPages_(openModPages) {
}
ModSettingsScreen::ParentFactory ModSettingsScreen::modPagesFactory() const {
 const ParentFactory parent = parentFactory_;
 return [parent] { return std::make_unique<ModSettingsScreen>(parent, true); };
}
void ModSettingsScreen::init() {
 page_ = openModPages_ ? Page::ModPages : Page::Settings;
 selectedKeybindIndex_ = -1;
 scrollOffset_ = 0;
 rebuildLayout();
}
void ModSettingsScreen::rebuildLayout() {
 title_ = page_ == Page::Settings ? "Mod Settings" : "Mod Pages";
 buttons_.clear();
 settingWidgets_.clear();
 keybindWidgets_.clear();
 sectionHeaders_.clear();
 injectedWidgets_.clear();
   ...
   for(std::size_t i = 0; i < settings.size(); ++i) {
    ...
    if(setting->kind == net::minecraft::mod::ModSettingDef::Slider) {
     ...
     addButton<ModSliderWidget>(
         ...
         [modId, key] {
          const auto* current = net::minecraft::mod::ModSettingsRegistry::instance().findSetting(modId, key);
     ...
     addActionButton(x,
                     ...
                     [this, modId, key = setting->key, widgetIndex] {
                      ...
                      if(widgetIndex >= 0 && widgetIndex < static_cast<int>(buttons_.size()) &&
                       ...
                       buttons_[static_cast<std::size_t>(widgetIndex)]->text = formatToggleLabel(*current);
                      }
                     });
    }
 ...
 if(page_ == Page::Settings && !allKeybinds.empty()) {
 ...
 if(page_ == Page::ModPages) {
  if(injectedWidgetCount > 0) {
   sectionHeaders_.push_back({"Installed Mod Pages", 0});
   contentY = kSectionLabelHeight;
  }
  for(std::size_t i = 0; i < injectedWidgetCount; ++i) {
   ...
   injectedWidgets_.push_back({widgetIndex, widgetY});
  }
  contentY += static_cast<int>((injectedWidgetCount + 1) / 2) * layout::kRowSpacing;
  contentHeight_ = std::max(0, contentY);
 }
 const int doneY = height() - kDoneYInset;
 listTop_ = kListTop;
 listBottom_ = std::max(listTop_ + kButtonHeight, doneY - kSectionGap);
 if(page_ == Page::Settings && injectedWidgetCount > 0) {
  addActionButton(layout::optionsGridX(width(), 0), doneY, kButtonWidth, kButtonHeight, "Mod Pages...", [this] {
   page_ = Page::ModPages;
   selectedKeybindIndex_ = -1;
   scrollOffset_ = 0;
   pendingPageRebuild_ = true;
  });
 } else if(page_ == Page::ModPages) {
  addActionButton(layout::optionsGridX(width(), 0), doneY, kButtonWidth, kButtonHeight, "Back", [this] {
   page_ = Page::Settings;
   selectedKeybindIndex_ = -1;
   scrollOffset_ = 0;
   pendingPageRebuild_ = true;
 ...
 const int doneX = (page_ == Page::ModPages || injectedWidgetCount > 0)
                       ? layout::optionsGridX(width(), 1)
                       : layout::centerBtnX(width());
 addActionButton(doneX,
                 doneY,
                 kButtonWidth,
                 kButtonHeight,
...
void ModSettingsScreen::updateListLayout() {
 ...
 for(const auto& injectedWidget : injectedWidgets_) {
  if(injectedWidget.widgetIndex < 0 || injectedWidget.widgetIndex >= static_cast<int>(buttons_.size())) {
   continue;
  }
  auto& button = buttons_[static_cast<std::size_t>(injectedWidget.widgetIndex)];
  if(button == nullptr) {
   continue;
  }
  button->y = listTop_ + injectedWidget.contentY - scrollOffset_;
  button->visible = button->y >= listTop_ && button->y + button->height <= listBottom_;
 }
}
...
void ModSettingsScreen::render(int mouseX, int mouseY, float tickDelta) {
 ...
 if(textRenderer() != nullptr) {
  ...
  drawCenteredTextWithShadow(*textRenderer(), page_ == Page::Settings ? "Click to change" : "Additional settings",
                             width() / 2, 25, 0xA0A0A0);
...
void ModSettingsScreen::mouseReleased(int mouseX, int mouseY, int button) {
 ...
 if(pendingPageRebuild_) {
  pendingPageRebuild_ = false;
  rebuildLayout();
 }
}
