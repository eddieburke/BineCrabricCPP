...
void KeybindsScreen::init() {
  ...
  for(int i = 0; i < client_option::GameOptions::kKeybindCount; ++i) {
    const int index = i;
    addActionButton(listX + (i % 2) * 160, height() / 6 + layout::kRowSpacing * (i / 2), 70, 20,
                    gameOptions_->getKeybindKey(i), [this, index] { selectKeybind(index); });
  }
...
void KeybindsScreen::render(int mouseX, int mouseY, float tickDelta) {
  ...
  if(textRenderer() != nullptr) {
    ...
    if(gameOptions_ != nullptr) {
      const int listX = controlsListX();
      for(int i = 0; i < client_option::GameOptions::kKeybindCount; ++i) {
        drawTextWithShadow(*textRenderer(), gameOptions_->getKeybindName(i), listX + (i % 2) * 160 + 70 + 6,
                           height() / 6 + layout::kRowSpacing * (i / 2) + 7, 0xFFFFFFFF);
      }
    }
  }
