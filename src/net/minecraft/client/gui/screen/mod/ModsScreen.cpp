...
protected:
  ...
  void renderEntry(int index, int x, int y, int height, render::Tessellator& tessellator) override {
    ...
    owner_.drawTextWithShadow(*owner_.textRenderer(), detailLine(mod), x, y + 25, mod.error.empty() ? 0x808080 : 0xFFB0B0);
  }
...
void ModsScreen::init() {
  ...
  addActionButton(layout::listFooterRightX(width()), height() - 76, layout::kConfirmButtonWidth,
                  layout::kDefaultButtonHeight, "Reload List",
                  [this] { refreshMods(); });
  addActionButton(layout::listFooterLeftX(width()), height() - 52, layout::kConfirmButtonWidth,
                  layout::kDefaultButtonHeight, "Open mods folder",
                  [this] { openModsFolder(); });
  addActionButton(layout::listFooterRightX(width()), height() - 52, layout::kConfirmButtonWidth,
                  layout::kDefaultButtonHeight, "Texture Packs...",
                  [this] { openTexturePacks(); });
  addCenteredActionButton(height() - 28, layout::kConfirmButtonWidth, layout::kDefaultButtonHeight,
...
void ModsScreen::render(int mouseX, int mouseY, float tickDelta) {
  ...
  if(textRenderer() != nullptr) {
    ...
    drawCenteredTextWithShadow(*textRenderer(), "Zip and folder mods. Changes apply on restart.",
                               width() / 2, 28, 0xA0A0A0);
