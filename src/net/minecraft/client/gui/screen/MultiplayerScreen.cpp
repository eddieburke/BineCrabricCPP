...
void MultiplayerScreen::init() {
    ...
    modsToggle_ = &addActionButton(
        ...
        height() / 4 + 84,
        resource::language::I18n::getTranslation(
            "multiplayer.mods",
            resource::language::I18n::getTranslation(
                minecraft() != nullptr && minecraft()->options.modsEnabled ? "options.on" : "options.off")),
        [this] { toggleMods(); });
...
void MultiplayerScreen::toggleMods() {
    if (minecraft() != nullptr) {
        ...
        if (modsToggle_ != nullptr) {
            modsToggle_->text = resource::language::I18n::getTranslation(
                "multiplayer.mods",
                resource::language::I18n::getTranslation(minecraft()->options.modsEnabled ? "options.on"
                                                                                          : "options.off"));
        }
    }
}
