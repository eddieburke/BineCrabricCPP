...
void TitleScreen::init()
{
    ...
    addCenteredActionButton(layout::menuRowY(h, 0),
        resource::language::I18n::getTranslation("menu.singleplayer"),
        [this, returnToTitle] {
            navigateTo(std::make_unique<world::SelectWorldScreen>(returnToTitle));
        });

    multiplayerButton_ = &addCenteredActionButton(layout::menuRowY(h, 1),
    ...
    addCenteredActionButton(layout::menuRowY(h, 2),
        ...
        [this, returnToTitle] {
            navigateTo(std::make_unique<pack::PackScreen>(returnToTitle));
        });
...
void TitleScreen::render(int mouseX, int mouseY, float tickDelta)
{
    ...
    const std::int64_t nowMillis = std::chrono::duration_cast<std::chrono::milliseconds>(
        std::chrono::system_clock::now().time_since_epoch()).count();
    const float wave = net::minecraft::util::math::MathHelper::abs(net::minecraft::util::math::MathHelper::sin(
        static_cast<float>(nowMillis % 1000) / 1000.0f * 3.14159265f * 2.0f) * 0.1f);
