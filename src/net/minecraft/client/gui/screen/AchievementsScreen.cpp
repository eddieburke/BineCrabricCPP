...
AchievementsScreen::AchievementsScreen(stat::PlayerStats* stats) : stats_(stats) {
  ...
  scaledMouseDx_ = scrollX_ =
      static_cast<double>(achievement::Achievements::OPEN_INVENTORY.column * 24 - centerX / 2 - 12);
  mouseX_ = scrollX_;
...
void AchievementsScreen::init() {
  buttons_.clear();
  addActionButton(
      width_ / 2 + 24, height_ / 2 + 74, 80, 20, resource::language::I18n::getTranslation("gui.done"), [this] {
...
void AchievementsScreen::renderIcons(int mouseX, int mouseY, float tickDelta) {
  ...
  const int playerBlockX =
      useWorldTerrain ? net::minecraft::util::math::MathHelper::floor(static_cast<float>(player->x)) : 0;
  const int playerBlockZ =
      useWorldTerrain ? net::minecraft::util::math::MathHelper::floor(static_cast<float>(player->z)) : 0;
  ...
  }
  {
    ...
    for(const achievement::AchievementDef& achievement : achievement::Achievements::ALL) {
      ...
      const achievement::AchievementDef* parent =
          achievement::Achievements::getByStatId(achievement.parentStatId());
      if(parent == nullptr) {
        continue;
      }
      ...
      const int lineColor =
          unlocked ? 0xFF6F6968 : (parentUnlocked ? (0xFF00FF00 | (pulseAlpha << 24)) : 0xFF000000);
      ...
      if(stats_->hasParentAchievement(hovered->statId())) {
        ...
        if(stats_->hasStat(hovered->statId())) {
          textRenderer()->drawWithShadow(resource::language::I18n::getTranslation("achievement.taken"),
                                         tooltipX,
                                         tooltipY + bodyHeight + 4,
                                         0xFF90F7DF);
        }
      } else {
        const achievement::AchievementDef* parent =
            achievement::Achievements::getByStatId(hovered->parentStatId());
