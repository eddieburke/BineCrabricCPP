...
void ConnectScreen::cancelConnection() {
  ...
  const bool lanHandoff = minecraft()->lanHostCoordinator().isAwaitingLoopback();
...
void ConnectScreen::tick() {
  ...
  if(!connectError.empty()) {
    if(minecraft()->lanHostCoordinator().isAwaitingLoopback()) {
      const std::string portText = std::to_string(minecraft()->lanHostCoordinator().requestedPort());
      minecraft()->lanHostCoordinator().onConnectCanceledOrFailed(connectError);
