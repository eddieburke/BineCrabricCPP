    ...
    const entity::player::PlayerEntity& player)
{
    ...
    if (!minecraft->options.ofFastDebugInfo) {
...
void InGameHud::renderRecordOverlay(font::TextRenderer& textRenderer, float tickDelta, int scaledWidth, int scaledHeight)
{
...
void InGameHud::renderChat(font::TextRenderer& textRenderer, bool chatOpen, int scaledWidth, int scaledHeight)
{
    ...
    for (int i = 0; i < static_cast<int>(messages.size()) && i < maxLines; ++i) {
        ...
        const int y = -i * 9;
        fill(2, y - 1, 322, y + 8, static_cast<std::uint32_t>(alpha / 2) << 24U);
        render::platform::GuiGlState::enableStandardBlend();
        drawTextWithShadow(textRenderer, messages[static_cast<std::size_t>(i)].text, 2, y, kColorWhite + (alpha << 24));
    }
    gl::GL11::glPopMatrix();
    render::platform::GuiGlState::endAlphaText();
}
...
void InGameHud::render(float tickDelta, bool screenOpen, int mouseX, int mouseY)
{
    ...
    const util::UiScale scale = util::uiScale(
        minecraft->options,
        util::uiFramebufferWidth(minecraft->options, minecraft->displayWidth),
    ...
    const float portalStrength = player.lastScreenDistortion
        + (player.screenDistortion - player.lastScreenDistortion) * tickDelta;
    ...
    zOffset = -90.0f;
    drawTexture(scaledWidth / 2 - 91, scaledHeight - 22, 0, 0, 182, 22);
    drawTexture(scaledWidth / 2 - 91 - 1 + player.inventory.selectedSlot * 20, scaledHeight - 22 - 1, 0, 22, 24, 22);

    gl::GL11::glBindTexture(gl::GL11::GL_TEXTURE_2D, minecraft->textureManager.getTextureId("/gui/icons.png"));
    render::platform::GuiGlState::beginCrosshairBlend();
    drawTexture(scaledWidth / 2 - 7, scaledHeight / 2 - 7, 0, 0, 16, 16);
    render::platform::GuiGlState::endCrosshairBlend();
    ...
    if (minecraft->interactionManager != nullptr && minecraft->interactionManager->canBeRendered()) {
        ...
        for (int heart = 0; heart < 10; ++heart) {
            ...
            if (armor > 0) {
                ...
                if (heart * 2 + 1 < armor) {
                    drawTexture(armorX, y, 34, 9, 9, 9);
                } else if (heart * 2 + 1 == armor) {
                    drawTexture(armorX, y, 25, 9, 9, 9);
                } else if (heart * 2 + 1 > armor) {
                    drawTexture(armorX, y, 16, 9, 9, 9);
                }
            }
            ...
            drawTexture(heartX, y, 16 + blink * 9, 0, 9, 9);
            if (blinkHearts) {
                if (heart * 2 + 1 < lastHealth) {
                    drawTexture(heartX, y, 70, 0, 9, 9);
                } else if (heart * 2 + 1 == lastHealth) {
                    drawTexture(heartX, y, 79, 0, 9, 9);
                }
            }
            if (heart * 2 + 1 < health) {
                drawTexture(heartX, y, 52, 0, 9, 9);
            } else if (heart * 2 + 1 == health) {
                drawTexture(heartX, y, 61, 0, 9, 9);
            }
        }
        ...
        if (player.isInFluid(block::material::Material::WATER)) {
            ...
            const int remaining = static_cast<int>(std::ceil(static_cast<double>(player.air) * 10.0 / 300.0)) - depleted;
            for (int bubble = 0; bubble < depleted + remaining; ++bubble) {
                ...
                if (bubble < depleted) {
                    drawTexture(bubbleX, bubbleY, 16, 18, 9, 9);
                } else {
                    drawTexture(bubbleX, bubbleY, 25, 18, 9, 9);
                }
            }
        }
    }
    ...
    {
        render::platform::ScopedLitHotbar litHotbar;
    ...
    if (player.getSleepTimer() > 0) {
        render::platform::GuiGlState::beginSleepFade();
        ...
        render::platform::GuiGlState::endSleepFade();
    }
