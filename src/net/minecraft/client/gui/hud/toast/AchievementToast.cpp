...
void AchievementToast::renderOverlay()
{
    ...
    render::platform::StereoUiFrame ui(client_->options, client_->displayWidth, client_->displayHeight);
    ui.forEachEye([&](const render::platform::StereoUiEyeContext& ctx) {
            ctx.setupHudProjection();
            const int x = ctx.scaledWidth - 160;
