...
EntryListWidget::EntryListWidget(Minecraft& minecraft, int width, int height, int top, int bottom, int itemHeight)
    : minecraft_(minecraft), width_(width), height_(height), top_(top), bottom_(bottom), left_(0), right_(width),
      listLeft_(width / 2 - 110), listRight_(width / 2 + 110), entryX_(width / 2 - 92 - 16), itemHeight_(itemHeight) {
}
...
void EntryListWidget::renderBackgroundStrip(render::Tessellator& tessellator, int yStart, int yEnd, int colorTop,
                                            int colorBottom) {
  constexpr float tile = 32.0f;
  draw::appendVerticalGradientTexturedQuad(tessellator, 0, yStart, width_, yEnd, 0.0f, static_cast<float>(yStart) / tile,
                                           static_cast<float>(width_) / tile, static_cast<float>(yEnd) / tile, 0x404040,
                                           colorTop, 0x404040, colorBottom);
}
void EntryListWidget::render(int mouseX, int mouseY, float tickDelta) {
  ...
  draw::coloredTexturedQuad(tessellator, left_, top_, right_, bottom_, static_cast<float>(left_) / tile,
                            static_cast<float>(top_ + scroll) / tile, static_cast<float>(right_) / tile,
                            static_cast<float>(bottom_ + scroll) / tile, 0x202020);
