...
void EntryListWidget::renderBackgroundStrip(int yStart, int yEnd, int colorTop, int colorBottom)
{
...
void EntryListWidget::render(int mouseX, int mouseY, float tickDelta)
{
        ...
        if (mostYStart_ == -1.0f) {
            ...
            if (mouseY >= top_ && mouseY <= bottom_) {
                ...
                if (mouseX >= listLeft && mouseX <= listRight && index >= 0 && relativeY >= 0
                    ...
                    const bool doubleClick = index == lastClickedPos_
    ...
    for (int index = 0; index < entryCount; ++index) {
        ...
        if (renderSelectionHighlight_ && isSelectedEntry(index)) {
    ...
    renderBackgroundStrip(0, top_, 255, 255);
    renderBackgroundStrip(bottom_, height_, 255, 255);
