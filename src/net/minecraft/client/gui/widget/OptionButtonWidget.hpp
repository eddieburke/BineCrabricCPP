...
public:
  ...
  OptionButtonWidget(
      int id, int x, int y, std::optional<std::size_t> registryIndex, std::string text, TextFormat format)
      : ButtonWidget(id, x, y, 150, 20, std::move(text)), registryIndex_(registryIndex), format_(std::move(format)) {
  }
