...
void appendVerticalGradientQuad(render::Tessellator& tessellator, int x1, int y1, int x2, int y2, int topRgb,
                                int topAlpha, int bottomRgb, int bottomAlpha, float z) {
...
void appendTexturedQuad(render::Tessellator& tessellator, int x1, int y1, int x2, int y2, float u1, float v1, float u2,
                        float v2, float z) {
  tessellator.vertex(static_cast<double>(x1), static_cast<double>(y2), static_cast<double>(z),
...
void appendAtlasQuad(render::Tessellator& tessellator, int x, int y, int u, int v, int width, int height, float z) {
  appendTexturedQuad(tessellator, x, y, x + width, y + height, static_cast<float>(u) * kAtlasTexel,
                     static_cast<float>(v) * kAtlasTexel, static_cast<float>(u + width) * kAtlasTexel,
                     static_cast<float>(v + height) * kAtlasTexel, z);
}
void appendTiledPanel(render::Tessellator& tessellator, int x1, int y1, int x2, int y2, float scrollV, int rgb,
                      float z) {
  constexpr float tile = 32.0f;
  tessellator.color(rgb);
  tessellator.vertex(static_cast<double>(x1), static_cast<double>(y2), static_cast<double>(z), 0.0,
                     static_cast<double>(y2 + scrollV) / tile);
  tessellator.vertex(static_cast<double>(x2), static_cast<double>(y2), static_cast<double>(z),
                     static_cast<double>(x2) / tile, static_cast<double>(y2 + scrollV) / tile);
  tessellator.vertex(static_cast<double>(x2), static_cast<double>(y1), static_cast<double>(z),
                     static_cast<double>(x2) / tile, static_cast<double>(y1 + scrollV) / tile);
  tessellator.vertex(static_cast<double>(x1), static_cast<double>(y1), static_cast<double>(z), 0.0,
...
void verticalGradientQuad(render::Tessellator& tessellator, int x1, int y1, int x2, int y2, int topRgb, int topAlpha,
                          int bottomRgb, int bottomAlpha, float z) {
  tessellator.startQuads();
  appendVerticalGradientQuad(tessellator, x1, y1, x2, y2, topRgb, topAlpha, bottomRgb, bottomAlpha, z);
  tessellator.draw();
}
void texturedQuad(render::Tessellator& tessellator, int x1, int y1, int x2, int y2, float u1, float v1, float u2,
                  float v2, float z) {
  tessellator.startQuads();
  appendTexturedQuad(tessellator, x1, y1, x2, y2, u1, v1, u2, v2, z);
  tessellator.draw();
}
void coloredTexturedQuad(render::Tessellator& tessellator, int x1, int y1, int x2, int y2, float u1, float v1, float u2,
                         float v2, int rgb, int alpha, float z) {
  tessellator.startQuads();
  tessellator.color(rgb, alpha);
  appendTexturedQuad(tessellator, x1, y1, x2, y2, u1, v1, u2, v2, z);
  tessellator.draw();
}
void verticalGradientTexturedQuad(render::Tessellator& tessellator, int x1, int y1, int x2, int y2, float u1, float v1,
                                  float u2, float v2, int topRgb, int topAlpha, int bottomRgb, int bottomAlpha,
                                  float z) {
  tessellator.startQuads();
  appendVerticalGradientTexturedQuad(tessellator, x1, y1, x2, y2, u1, v1, u2, v2, topRgb, topAlpha, bottomRgb,
                                     bottomAlpha, z);
  tessellator.draw();
}
void appendVerticalGradientTexturedQuad(render::Tessellator& tessellator, int x1, int y1, int x2, int y2, float u1,
                                        float v1, float u2, float v2, int topRgb, int topAlpha, int bottomRgb,
                                        int bottomAlpha, float z) {
  tessellator.color(bottomRgb, bottomAlpha);
  tessellator.vertex(static_cast<double>(x1), static_cast<double>(y2), static_cast<double>(z),
                     static_cast<double>(u1), static_cast<double>(v2));
  tessellator.vertex(static_cast<double>(x2), static_cast<double>(y2), static_cast<double>(z),
