...
public:
    OtherPlayerEntity(World* world, std::string name)
        : PlayerEntity(world)
    {
        ...
        stepHeight = 0.0f;
        if (!this->name.empty()) {
            skinUrl = "http://s3.amazonaws.com/MinecraftSkins/" + this->name + ".png";
        }
        noClip = true;
        sleepOffsetY = 0.25f;
    ...
    bool damage(Entity* /*damageSource*/, int /*amount*/) override { return true; }

    void setPositionAndAnglesAvoidEntities(double x, double y, double z, float yaw, float pitch, int interpolationSteps) override
    {
        targetX_ = x;
        targetY_ = y;
        targetZ_ = z;
    ...
    [[nodiscard]] float getShadowRadius() const override { return 0.0f; }
    ...
    void tickMovement() override
    {
        ...
        if (interpolationSteps_ > 0) {
            ...
            pitch = static_cast<float>(
                static_cast<double>(pitch) + (targetPitch_ - static_cast<double>(pitch)) / static_cast<double>(interpolationSteps_));
            --interpolationSteps_;
