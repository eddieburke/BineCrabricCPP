...
public:
    explicit Timer(float ticksPerSecond)
        : tps(ticksPerSecond),
          prevTickTime_(currentTimeMillis()),
          prevCorrectionTime_(nanoTimeMillis())
    {
    }
        ...
        if (tickElapsed > 1000 || tickElapsed < 0) {
            ...
            if (cooldownTickTime_ > 1000) {
                ...
                if (correctionElapsed != 0) {
                    const double measuredCorrection = static_cast<double>(cooldownTickTime_) / static_cast<double>(correctionElapsed);
