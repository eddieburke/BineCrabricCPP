...
void relightSkylightForPreparedArea(World& world, int centerX, int centerZ, int radiusBlocks) {
  for(int dx = -radiusBlocks; dx <= radiusBlocks; dx += 16) {
    for(int dz = -radiusBlocks; dz <= radiusBlocks; dz += 16) {
      if(Chunk* chunk = world.getChunkIfLoaded(centerX + dx, centerZ + dz); chunk != nullptr && !chunk->isEmpty()) {
...
void WorldSession::setWorld(Minecraft& client, World* worldIn, const std::string& message, PlayerEntity* existingPlayer,
  ...
  const bool suppressLocalSave =
      suppressNextRemoteHandoffSave_ && client.world != nullptr && !client.world->isRemote() && worldIn != nullptr &&
      worldIn->isRemote();
  ...
  if(worldIn != nullptr) {
    ...
    const bool hasSavedPlayer =
        !worldIn->isRemote() && worldIn->getProperties().getPlayerNbt() != nullptr;
    if(!worldIn->isRemote()) {
