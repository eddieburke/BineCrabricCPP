...
bool InteractionManager::interactBlock(PlayerEntity* player, World* world, ItemStack* item, int x, int y, int z,
                                       int side) {
...
PlayerEntity* InteractionManager::createPlayer(World* world) {
  ...
  return new entity::player::ClientPlayerEntity(minecraft, world, minecraft->session, minecraft->options,
                                                dimensionId);
}
