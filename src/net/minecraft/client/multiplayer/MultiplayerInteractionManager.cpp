...
void MultiplayerInteractionManager::processBlockBreakingAction(int x, int y, int z, int side) {
  ...
  if(x == breakingPosX && y == breakingPosY && z == breakingPosZ) {
    ...
    if(std::fmod(breakingSoundDelayTicks, 4.0f) == 0.0f && block != nullptr) {
      block::sounds::playMining(world, static_cast<double>(x) + 0.5, static_cast<double>(y) + 0.5,
...
bool MultiplayerInteractionManager::interactBlock(PlayerEntity* player, World* world, ItemStack* item, int x, int y,
                                                  int z, int side) {
  updateSelectedSlot();
...
ItemStack* MultiplayerInteractionManager::clickSlot(int syncId, int slotId, int button, bool shift,
                                                    PlayerEntity* player) {
