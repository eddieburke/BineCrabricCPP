...
MultiplayerInteractionManager::MultiplayerInteractionManager(Minecraft* minecraft,
                                                             multiplayer::ClientNetworkHandler* networkHandler)
    : InteractionManager(minecraft), networkHandler(networkHandler) {
}
...
void MultiplayerInteractionManager::attackBlock(int x, int y, int z, int direction) {
 ...
 if(!breakingBlock || x != breakingPosX || y != breakingPosY || z != breakingPosZ) {
  if(breakingBlock && networkHandler != nullptr) {
   PlayerActionC2SPacket abortPacket;
   abortPacket.action = 1;
   abortPacket.x = breakingPosX;
   abortPacket.y = breakingPosY;
   abortPacket.z = breakingPosZ;
   abortPacket.direction = direction;
   networkHandler->sendPacket(abortPacket);
  }
...
void MultiplayerInteractionManager::cancelBlockBreaking() {
 if(breakingBlock) {
  if(networkHandler != nullptr) {
   PlayerActionC2SPacket packet;
   packet.action = 1;
   packet.x = breakingPosX;
   packet.y = breakingPosY;
   packet.z = breakingPosZ;
   packet.direction = 0;
   networkHandler->sendPacket(packet);
  }
  breakingBlock = false;
...
void MultiplayerInteractionManager::update(float partialTick) {
 ...
 if(blockBreakingProgress <= 0.0f) {
  ...
  const float progress =
      lastBlockBreakingProgress + (blockBreakingProgress - lastBlockBreakingProgress) * partialTick;
...
PlayerEntity* MultiplayerInteractionManager::createPlayer(World* world) {
 return new multiplayer::MultiplayerClientPlayerEntity(minecraft, world, minecraft->session, networkHandler);
}
...
ItemStack* MultiplayerInteractionManager::clickSlot(
    int syncId, int slotId, int button, bool shift, PlayerEntity* player) {
