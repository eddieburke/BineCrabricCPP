...
void ClientNetworkHandler::onHandshake(const HandshakePacket& packet) {
  ...
  if(!metadata.requiredMods.empty()) {
    ...
    sendPacket(ModListPacket(mod::runtime::WorldRequiredMods::joinCsv(activeClientMods())));
  }
  beginPendingLogin(metadata.serverId);
}
...
bool ClientNetworkHandler::downloadPendingMods(std::string& error) {
  ...
  for(const std::string& modId : pendingMissingMods_) {
    ...
    request.maxResponseBytes = static_cast<std::size_t>(mod::runtime::kMaxModArchiveBytes);
    const http::HttpResponse response = http::httpRequest(request);
    if(!response.ok() || response.body.empty()) {
      error = "Failed to download required mod " + modId;
      return false;
    }
    const std::filesystem::path tempPath = tempDir / (mod::runtime::sanitizeName(modId) + ".zip.tmp");
    if(!mod::runtime::writeFileBytes(tempPath, response.body)) {
    ...
    if(!mod::runtime::parseManifestJson(std::string(manifestBytes.begin(), manifestBytes.end()), pkg, tempPath,
                                        mod::runtime::ModPackageSource::Zip, "mod.json: ") ||
       pkg.id != modId) {
      error = "Downloaded file does not match required mod " + modId;
      return false;
    }
    const std::filesystem::path finalPath = modsDir / (mod::runtime::sanitizeName(modId) + ".zip");
    if(!mod::runtime::writeFileBytes(finalPath, response.body)) {
