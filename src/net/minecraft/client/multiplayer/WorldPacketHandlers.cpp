...
void ClientNetworkHandler::onWorldTimeUpdate(const WorldTimeUpdateS2CPacket& packet) {
 ...
 const long long current = static_cast<long long>(clientWorld->time());
 const long long target = static_cast<long long>(packet.time);
 if(std::abs(target - current) > 20 || target > current) {
  clientWorld->setTime(static_cast<std::uint64_t>(target));
...
void ClientNetworkHandler::handleChunkData(const ChunkDataS2CPacket& packet) {
 ...
 clientWorld->clearBlockResets(packet.x,
                               packet.y,
                               packet.z,
                               packet.x + packet.sizeX - 1,
                               packet.y + packet.sizeY - 1,
                               packet.z + packet.sizeZ - 1);
...
void ClientNetworkHandler::onWorldEvent(const WorldEventS2CPacket& packet) {
 ...
 case 2000: {
  ...
  for(int i = 0; i < 10; ++i) {
   ...
   const double px =
       baseX + static_cast<double>(offsetX) * 0.01 + (random.nextDouble() - 0.5) * offsetZ * 0.5;
   const double py = baseY + (random.nextDouble() - 0.5) * 0.5;
   const double pz =
       baseZ + static_cast<double>(offsetZ) * 0.01 + (random.nextDouble() - 0.5) * offsetX * 0.5;
