...
void ClientNetworkHandler::onEntitySpawn(const EntitySpawnS2CPacket& packet) {
  ...
  case 61:
    ...
    break;
  case 63:
    entity = new entity::projectile::FireballEntity(clientWorld,
                                                    x,
                                                    y,
                                                    z,
  ...
  case 70:
    entity =
        new entity::FallingBlockEntity(clientWorld, x, y, z, Block::SAND != nullptr ? Block::SAND->id : 12);
    break;
  case 71:
    entity =
...
void ClientNetworkHandler::onEntityTrackerUpdate(const EntityTrackerUpdateS2CPacket& packet) {
  ...
  for(const int key : entity->getDataTracker().writeUpdatedEntries(packet.trackedValues)) {
    entity->onTrackedDataUpdated(key);
  }
}
...
void ClientNetworkHandler::onLivingEntitySpawn(const LivingEntitySpawnS2CPacket& packet) {
  ...
  if(!packet.trackedValues.empty()) {
    for(const int key : livingEntity->getDataTracker().writeUpdatedEntries(packet.trackedValues)) {
      livingEntity->onTrackedDataUpdated(key);
    }
  }
}
