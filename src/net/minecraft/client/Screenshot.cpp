...
std::string Screenshot::take(const std::filesystem::path& gameDir, int width, int height)
{
    try {
        ...
        int suffix = 1;
        do {
            outFile = screenshotDir / (baseName + (suffix == 1 ? std::string{} : "_" + std::to_string(suffix)) + ".png");
            ++suffix;
        ...
        const Gdiplus::Bitmap bitmap(width, height, width * 4, PixelFormat32bppARGB, reinterpret_cast<BYTE*>(argbPixels.data()));
        CLSID pngClsid {};
        ...
        const Gdiplus::Status status = const_cast<Gdiplus::Bitmap&>(bitmap).Save(outFile.wstring().c_str(), &pngClsid, nullptr);
        Gdiplus::GdiplusShutdown(token);
