...
struct OptionSpec {
    ...
    enum class Kind { Slider, Toggle, Cycle, Hidden } kind = Kind::Toggle;
...
{
    options.ofAoLevel = value;
    options.ao = value > 0.0f;
}

inline float getRenderScaleSlider(const GameOptions& options)
{
    return (options.ofRenderScale - 1.0f) / 4.0f;
}

inline void setRenderScaleSlider(GameOptions& options, float value)
{
    options.ofRenderScale = 1.0f + value * 4.0f;
}

inline float getEntityDistanceSlider(const GameOptions& options)
{
    return (options.ofEntityDistanceScale - 0.25f) / 3.75f;
}

inline void setEntityDistanceSlider(GameOptions& options, float value)
{
    options.ofEntityDistanceScale = 0.25f + value * 3.75f;
}

inline void loadStereoMode(GameOptions& options, std::string_view value)
{
    try {
        options.stereoMode = std::clamp(std::stoi(std::string(value)), 0, 2);
    } catch (...) {}
}

inline bool alwaysResize(const GameOptions&) { return true; }

inline OptionSpec makeSlider(const char* key, int saveOrder, ApplyFlags intFx, ApplyFlags floatFx,
    ...
    bool (*isEnabled)(const GameOptions&) = nullptr)
{
    return {key, saveOrder, OptionSpec::Kind::Slider, intFx, floatFx,
        getF, setF, nullptr, nullptr, loadFn, saveFn, isEnabled, nullptr};
}

inline OptionSpec makeToggle(const char* key, int saveOrder, ApplyFlags flags,
    ...
    bool (*isEnabled)(const GameOptions&) = nullptr)
{
    return {key, saveOrder, OptionSpec::Kind::Toggle, flags, ApplyFlags::None,
        nullptr, nullptr, getB, cyc, loadFn, saveFn, isEnabled, nullptr};
}

inline OptionSpec makeCycle(const char* key, int saveOrder, ApplyFlags flags,
    void (*cyc)(GameOptions&, int),
    void (*loadFn)(GameOptions&, std::string_view),
    void (*saveFn)(const GameOptions&, std::ostream&),
...
{
    return {key, saveOrder, OptionSpec::Kind::Cycle, flags, ApplyFlags::None,
        nullptr, nullptr, getB, cyc, loadFn, saveFn, isEnabled, needsResize};
}

inline OptionSpec makeHidden(const char* key, int saveOrder, ApplyFlags flags,
    bool (*getB)(const GameOptions&),
    void (*loadFn)(GameOptions&, std::string_view),
    void (*saveFn)(const GameOptions&, std::ostream&))
{
    return {key, saveOrder, OptionSpec::Kind::Hidden, flags, ApplyFlags::None,
        nullptr, nullptr, getB, nullptr, loadFn, saveFn, nullptr, nullptr};
}
