...
void GameOptions::save() {
    ...
    std::sort(specs.begin(), specs.end(), [](const OptionSpec* a, const OptionSpec* b) {
        return a->saveOrder < b->saveOrder;
    });
...
std::string GameOptions::getString(std::string_view persistKey) const {
    ...
    if ((*spec)->kind == OptionSpec::Kind::Toggle) {
        return resource::language::I18n::getTranslation(getBoolean(persistKey) ? "options.on" : "options.off");
    }
