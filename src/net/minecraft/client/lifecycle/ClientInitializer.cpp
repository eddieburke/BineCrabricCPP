...
void pauseBeforeExit() {
  ...
  MessageBoxA(nullptr, "The application has stopped. Press OK to exit.", "Minecraft Native",
...
void installHangWatchdog() {
  std::thread([]() {
    ...
    while(true) {
      ...
      // Main loop hasn't advanced in ~kStallThresholdChecks seconds: deadlock or
      // infinite loop, not a thrown exception, so unhandledExceptionFilter never
      // fires. Dump once per stall so a hang can be diagnosed post-mortem.
      bool expected = false;
...
void ClientInitializer::bootstrap(Minecraft& client) {
  ...
  client.options.load();
  client.worldStorageSource =
      std::make_unique<net::minecraft::RegionWorldStorageSource>(client.runDirectory_ / "saves");
  ...
  client.texturePacksStorage_ =
      std::make_unique<resource::pack::TexturePacks>(std::filesystem::path(MINECRAFT_NATIVE_RESOURCE_DIR),
                                                     client.runDirectory_, &client.options, &client.textureManager);
  ...
  if(!client.startupServerAddress_.empty()) {
    client.setScreen(std::make_unique<gui::screen::ConnectScreen>(&client, client.startupServerAddress_,
                                                                  client.startupServerPort));
  } else {
