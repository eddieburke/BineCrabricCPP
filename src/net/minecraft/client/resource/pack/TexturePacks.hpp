...
class TexturePacks {
public:
  TexturePacks(std::filesystem::path resourceRoot, std::filesystem::path gameDir,
               option::GameOptions* options = nullptr, texture::TextureManager* textureManager = nullptr)
      : resources_(std::move(resourceRoot)), defaultPack_(std::make_unique<BuiltInTexturePack>(resources_)),
        dir_(std::move(gameDir) / "texturepacks"), options_(options), textureManager_(textureManager) {
    if(options_ != nullptr) {
