...
std::vector<std::uint8_t> readZipEntryData(const std::vector<std::uint8_t>& archive, const ZipEntry& entry) {
 ...
 if(entry.compressionMethod == 8) {
  ...
  try {
   ...
   texturePackTrace("failed to inflate entry '" + entry.name + "' method=8 (raw deflate and zlib fallback both failed)");
   return {};
  }
 }
...
std::vector<std::uint8_t> ZippedTexturePack::getResource(std::string_view path) const {
 if(!archive_.empty()) {
  ...
  if(fallbackResources_ != nullptr) {
   texturePackTrace("resource '" + std::string(path) + "' not found in pack '" + name + "', falling back to parent pack");
