...
[[nodiscard]] JoinServerResult verifyLegacyJoinServer(
    ...
    const std::string& serverId)
{
    ...
    url << "http://www.minecraft.net/game/joinserver.jsp"
        << "?user=" << urlEncodeComponent(username)
...
[[nodiscard]] JoinServerResult verifyMicrosoftJoinServer(
    ...
    const std::string& serverId)
{
    ...
    std::string body = std::string(R"({"accessToken":")")
        + msauth::json::escape(session.mpPass)
        + R"(","selectedProfile":")"
        + msauth::json::escape(profileId)
        + R"(","serverId":")"
        + msauth::json::escape(serverId)
        + R"("})";
