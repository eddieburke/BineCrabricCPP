...
[[nodiscard]] JoinServerResult verifyLegacyJoinServer(const std::string& username, const std::string& sessionId,
...
JoinServerResult verifyJoinServer(const net::minecraft::client::util::Session& session, const std::string& serverId,
                                  const std::atomic_bool* canceled) {
  if(msauth::isAuthenticated(session)) {
