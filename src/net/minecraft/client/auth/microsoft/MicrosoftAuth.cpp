...
HttpResponse httpPostForm(const std::string& url,
...
std::string formatXstsError(std::int64_t xerr) {
 ...
 case 2148916229:
  return "This Microsoft account is linked to a family and your parent or guardian has not given you "
         "permission "
         "to play online.";
 case 2148916233:
 ...
 case 2148916236:
  return "This Microsoft account requires proof of age to play. Please sign in at login.live.com to provide "
         "proof "
         "of age.";
 case 2148916237:
  return "This Microsoft account has reached its playtime limit and has been blocked from logging in.";
 case 2148916238:
  return "This Microsoft account is underaged and is not linked to a family. See help.minecraft.net to set "
         "up your "
         "account.";
 default:
...
AuthResult authResultFromProfileJson(const std::string& clientId,
...
AuthResult fetchMinecraftProfile(const std::string& clientId,
                                 const std::string& accessToken,
                                 const std::atomic_bool* canceled = nullptr) {
 ...
 const HttpResponse profileResponse = httpGet(
     "https://api.minecraftservices.com/minecraft/profile", {{"Authorization", "Bearer " + accessToken}}, canceled);
 if(isCanceled(canceled)) {
  return canceledResult();
 }
...
std::optional<XboxToken> authorizeXboxToken(const std::string& userToken,
                                            const std::string& expectedUserHash,
                                            const std::string& relyingParty,
 ...
 const HttpResponse response = httpPostJson(
     "https://xsts.auth.xboxlive.com/xsts/authorize", body, {{"x-xbl-contract-version", "1"}}, canceled);
 if(isCanceled(canceled)) {
  return std::nullopt;
 }
 if(!response.ok()) {
...
AuthResult completeFromMsa(const std::string& clientId,
                           const std::string& msaAccessToken,
                           const std::atomic_bool* canceled,
 ...
 const HttpResponse xboxUserResponse = httpPostJson(
     "https://user.auth.xboxlive.com/user/authenticate", xboxAuthBody, {{"x-xbl-contract-version", "1"}}, canceled);
 if(isCanceled(canceled)) {
  return canceledResult();
 }
 ...
 const std::optional<XboxToken> minecraftApi =
     authorizeXboxToken(*xblToken, *userHash, "rp://api.minecraftservices.com/", canceled, authorizationError);
 if(isCanceled(canceled)) {
  return canceledResult();
 }
 ...
 const std::string loginBody =
     std::string(R"({"xtoken":")") + json::escape(identityToken) + R"(","platform":"PC_LAUNCHER"})";
 ...
 const HttpResponse xboxProfileResponse =
     httpGet(xboxProfileUrl,
             {{"Accept", "application/json"}, {"x-xbl-contract-version", "3"}, {"Authorization", xboxAuthorization}},
             canceled);
 if(isCanceled(canceled)) {
  return canceledResult();
 }
...
DevicePollResult pollDeviceCodeToken(const std::string& clientId,
...
std::optional<MsaTokenPair> refreshMsaAccessToken(const std::string& clientId,
...
AuthResult loginWithDeviceCode(const std::string& clientId,
...
AuthResult createMinecraftProfile(const MicrosoftAccount& account,
 ...
 const HttpResponse createResponse = httpPostJson("https://api.minecraftservices.com/minecraft/profile",
                                                  createBody,
