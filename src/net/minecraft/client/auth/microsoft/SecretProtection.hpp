...
inline std::optional<std::string> protectForCurrentUser(std::string_view plaintext) {
    ...
    if (!CryptProtectData(&inputBlob,
                          L"Minecraft Native refresh token",
                          &entropyBlob,
                          nullptr,
                          nullptr,
    ...
    if (!CryptBinaryToStringA(protectedBlob.pbData,
                              protectedBlob.cbData,
    ...
    const bool encodedOk = CryptBinaryToStringA(protectedBlob.pbData,
                                                protectedBlob.cbData,
                                                CRYPT_STRING_BASE64 | CRYPT_STRING_NOCRLF,
                                                encoded.data(),
                                                &encodedLength);
...
inline std::optional<std::string> unprotectForCurrentUser(std::string_view protectedText) {
    ...
    if (!CryptStringToBinaryA(protectedText.data(),
                              static_cast<DWORD>(protectedText.size()),
                              CRYPT_STRING_BASE64,
                              nullptr,
                              &binaryLength,
                              nullptr,
    ...
    if (!CryptStringToBinaryA(protectedText.data(),
                              static_cast<DWORD>(protectedText.size()),
                              CRYPT_STRING_BASE64,
                              reinterpret_cast<BYTE*>(protectedBytes.data()),
                              &binaryLength,
                              nullptr,
                              nullptr)) {
