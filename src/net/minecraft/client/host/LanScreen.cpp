...
void LanScreen::init() {
 ...
 openButton_ =
     &addActionButton(gui::layout::centerBtnX(width()), baseY + 128, "Start Server", [this] { openLan(); });
...
void LanScreen::refreshSettingLabels() {
 ...
 if(pvpButton_ != nullptr)
  pvpButton_->text = label("PvP", settings_.pvpEnabled);
 if(animalsButton_ != nullptr)
  animalsButton_->text = label("Animals", settings_.spawnAnimals);
 if(netherButton_ != nullptr)
  netherButton_->text = label("Nether", settings_.allowNether);
 if(modsButton_ != nullptr)
  modsButton_->text = label("Server mods", settings_.modsEnabled);
}
void LanScreen::tick() {
