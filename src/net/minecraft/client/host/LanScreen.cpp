...
void LanScreen::init() {
    enableTextInput();
    buttons_.clear();
    openButton_ = &addActionButton(
        gui::layout::centerBtnX(width()), gui::layout::formPrimaryBtnY(height()), "Open", [this] { openLan(); });
    addActionButton(gui::layout::centerBtnX(width()), gui::layout::formCancelBtnY(height()), "Cancel", [this] {
        backToGameMenu();
    });
...
void LanScreen::pollServerStart() {
    ...
    if (minecraft()->lanHostCoordinator().isAwaitingLoopback()) {
        navigateTo(std::make_unique<client::gui::screen::ConnectScreen>(
            minecraft(),
            "127.0.0.1",
            minecraft()->lanHostCoordinator().boundPort(),
...
void LanScreen::render(int mouseX, int mouseY, float tickDelta) {
    ...
    if (textRenderer() != nullptr) {
        drawCenteredTextWithShadow(
            *textRenderer(), "Open to LAN", width() / 2, gui::layout::formTitleY(height()), 0xFFFFFF);
        drawTextWithShadow(*textRenderer(),
                           "Choose a custom port for local network hosting.",
                           width() / 2 - 140,
                           height() / 4 + 2,
                           0xA0A0A0);
        ...
        if (!errorMessage_.empty()) {
            drawCenteredTextWithShadow(
                *textRenderer(), errorMessage_, width() / 2, gui::layout::formPrimaryBtnY(height()) - 20, 0xFF5555);
        } else if (startingServer_) {
            drawCenteredTextWithShadow(*textRenderer(),
                                       "Starting server...",
                                       width() / 2,
                                       gui::layout::formPrimaryBtnY(height()) - 20,
                                       0xFFFFFF);
        }
    }
