...
bool ServerProcessCoordinator::launch(std::string& errorOut) {
 ...
 const std::string ownerName = session::resolveJoinUsername(minecraft_->session);
 const std::wstring ownerNameWide(ownerName.begin(), ownerName.end());
 command << quoteArg(executable.wstring()) << L" nogui" << L" --storage-root " << quoteArg(storageRoot_.wstring())
         << L" --level-name " << quoteArg(std::filesystem::path(worldName_).wstring()) << L" --level-seed "
         << worldSeed_ << L" --server-ip " << quoteArg(L"") << L" --server-port " << settings_.port
         << L" --ready-file " << quoteArg(readyFile_.wstring()) << L" --owner " << quoteArg(ownerNameWide)
         << L" --online-mode false" << L" --spawn-animals "
         << (settings_.spawnAnimals ? L"true" : L"false") << L" --pvp "
         << (settings_.pvpEnabled ? L"true" : L"false") << L" --allow-flight "
         << (settings_.flightEnabled ? L"true" : L"false") << L" --allow-nether "
         << (settings_.allowNether ? L"true" : L"false") << L" --mods-enabled "
         << (settings_.modsEnabled ? L"true" : L"false");
