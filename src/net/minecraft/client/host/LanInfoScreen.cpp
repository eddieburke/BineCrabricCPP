...
void LanInfoScreen::render(int mouseX, int mouseY, float tickDelta) {
 ...
 if(textRenderer() != nullptr && minecraft() != nullptr) {
  ...
  drawCenteredTextWithShadow(
      *textRenderer(), "Dedicated Server", width() / 2, gui::layout::formTitleY(height()), 0xFFFFFF);
  drawTextWithShadow(*textRenderer(), "Primary network address", width() / 2 - 140, height() / 4, 0xA0A0A0);
  ...
  drawTextWithShadow(*textRenderer(),
                     info.loopbackEndpoint.empty() ? "127.0.0.1" : info.loopbackEndpoint,
                     width() / 2 - 140,
                     height() / 4 + 48,
                     0xFFFFFF);
