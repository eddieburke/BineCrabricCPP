...
public:
    ...
    {
        float f = maxParticleAge > 0 ? (static_cast<float>(particleAge) + tickDelta) / static_cast<float>(maxParticleAge) : 1.0f;
    ...
    {
        const float f = maxParticleAge > 0 ? (static_cast<float>(particleAge) + partialTicks) / static_cast<float>(maxParticleAge) : 0.0f;
