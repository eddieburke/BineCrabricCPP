...
public:
    LavaEmberParticle(World* world, double x, double y, double z)
    ...
    float getBrightnessAtEyes(float /*tickDelta*/) const override { return 1.0f; }
    ...
    {
        const float f = maxParticleAge > 0 ? (static_cast<float>(particleAge) + partialTicks) / static_cast<float>(maxParticleAge) : 1.0f;
    ...
    void tick() override
    {
        ...
        const float progress = maxParticleAge > 0 ? static_cast<float>(particleAge) / static_cast<float>(maxParticleAge) : 1.0f;
