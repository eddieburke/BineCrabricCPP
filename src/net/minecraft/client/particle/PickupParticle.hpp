...
public:
    PickupParticle(World* world, Entity* entity, Entity* collector, float offsetY)
        : Particle(world, entity != nullptr ? entity->x : 0.0, entity != nullptr ? entity->y : 0.0,
              entity != nullptr ? entity->z : 0.0, entity != nullptr ? entity->velocityX : 0.0,
              entity != nullptr ? entity->velocityY : 0.0, entity != nullptr ? entity->velocityZ : 0.0),
          entity_(entity),
    ...
    void render(render::Tessellator& /*tessellator*/, float partialTicks, float /*horizontalSize*/,
        float /*verticalSize*/, float /*depthSize*/, float /*widthOffset*/, float /*heightOffset*/) override
    {
        ...
        const double targetY = collector_->lastTickY + (collector_->y - collector_->lastTickY) * partialTicks + offsetY_;
    ...
    [[nodiscard]] int getGroup() const override { return 3; }
