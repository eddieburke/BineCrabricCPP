...
public:
    NoteParticle(World* world, double x, double y, double z, double noteColorModifier, double velocityY, double velocityZ, float scaleIn = 2.0f)
        : Particle(world, x, y, z, 0.0, 0.0, 0.0)
    {
        (void)velocityY; (void)velocityZ;
        this->velocityX *= 0.01;
    ...
    {
        float f = maxParticleAge > 0 ? (static_cast<float>(particleAge) + partialTicks) / static_cast<float>(maxParticleAge) * 32.0f : 0.0f;
