...
public:
    RainSplashParticle(World* world, double x, double y, double z)
    ...
    void tick() override
    {
        ...
        if ((material.isFluid() || material.isSolid())
            && y < static_cast<double>(static_cast<float>(MathHelper::floor(y) + 1)
                - block::LiquidBlock::getFluidHeightFromMeta(
                    world->getBlockMeta(MathHelper::floor(x), MathHelper::floor(y), MathHelper::floor(z))))) {
            markDead();
        }
    }
};
