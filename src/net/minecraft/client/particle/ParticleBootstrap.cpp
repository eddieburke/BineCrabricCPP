...
void registerVanillaParticles() {
  ...
  registry.registerFactory("reddust", [](const ParticleSpawnContext& c) {
    return std::make_unique<RedDustParticle>(c.world, c.x, c.y, c.z, static_cast<float>(c.velocityX),
                                             static_cast<float>(c.velocityY), static_cast<float>(c.velocityZ));
  });
...
static registry::RegisterPhase<VanillaParticleBootstrap> s_particles(mod::LifecyclePhase::ParticleRegistration, 0);
}
} // namespace net::minecraft::client::particle
