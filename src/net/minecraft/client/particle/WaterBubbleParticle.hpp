...
public:
    WaterBubbleParticle(World* world, double x, double y, double z, double velocityX, double velocityY, double velocityZ)
    ...
    void tick() override
    {
        ...
        velocityY *= 0.85;
        velocityZ *= 0.85;
        if (world != nullptr && &world->getMaterial(MathHelper::floor(x), MathHelper::floor(y), MathHelper::floor(z)) != &block::material::Material::WATER) {
            markDead();
        }
        if (maxParticleAge-- <= 0) {
            markDead();
