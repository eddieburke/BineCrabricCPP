...
public:
  ...
  [[nodiscard]] const gui::screen::Screen* currentScreen() const {
    return screenStack_.current();
  }
  [[nodiscard]] core::WorldSession& worldSession() noexcept {
    return worldSession_;
  }
  [[nodiscard]] const core::WorldSession& worldSession() const noexcept {
    return worldSession_;
  }
  [[nodiscard]] multiplayer::MultiplayerSession& multiplayerSession() noexcept {
    return multiplayerSession_;
  }
  [[nodiscard]] const multiplayer::MultiplayerSession& multiplayerSession() const noexcept {
    return multiplayerSession_;
  }
  [[nodiscard]] host::ServerProcessCoordinator& serverProcessCoordinator() noexcept {
    return *serverProcessCoordinator_;
  }
  [[nodiscard]] const host::ServerProcessCoordinator& serverProcessCoordinator() const noexcept {
    return *serverProcessCoordinator_;
  }
  // Java public fields.
