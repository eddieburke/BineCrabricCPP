...
[[nodiscard]] float listenerPan(float listenerX, float listenerZ, float lookX, float lookZ, float sourceX,
                                float sourceZ) {
...
#ifdef _WIN32
    if(!slot.loaded || !slot.spatial || slot.voice == nullptr || masteringVoice == nullptr || slot.inputChannels == 0) {
      return;
    }
...
void XAudio2Backend::setListener(float x, float y, float z, float lookX, float /*lookY*/, float lookZ, float /*upX*/,
                                 float /*upY*/, float /*upZ*/) {
  impl_->listenerX = x;
  impl_->listenerY = y;
  impl_->listenerZ = z;
