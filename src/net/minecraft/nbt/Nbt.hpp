...
// Tagged value tree (std::variant) plus binary I/O. Use NbtCompound / NbtList at API boundaries.
...
public:
    ...
    static Nbt readCompressed(std::istream& in)
    {
        ...
        std::size_t pos = 0;
        return nbt::detail::readNamedTag(decompressed, pos);
    }
...
private:
    ...
    void dumpImpl(std::ostream& out, int indent) const
    {
        ...
        case Type::List: {
            out << "[";
            const auto& list = std::get<List>(value_);
        ...
        case Type::Compound: {
            out << "{";
            const auto& compound = std::get<Compound>(value_);
            bool first = true;
