...
struct AchievementDef {
  ...
  [[nodiscard]] int statId() const noexcept {
    return kBase + index;
  }
...
struct Achievements {
  ...
  getTranslatedDescription(const AchievementDef& achievement,
                           const std::function<std::string(const std::string&)>& formatter = {});
  [[nodiscard]] static std::string getFormattedDescription(const AchievementDef& achievement, int inventoryKeyCode);
};
} // namespace net::minecraft::achievement
