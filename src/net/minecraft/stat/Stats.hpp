...
struct StatId {
    ...
    [[nodiscard]] bool operator==(const StatId& other) const noexcept { return id == other.id; }
    [[nodiscard]] std::string format(int value) const { return formatStatValue(formatter, value); }
};
...
struct Stats {
    ...
    [[nodiscard]] static int mineBlockStatId(int blockId) noexcept { return MINE_BLOCK_BASE + blockId; }
    [[nodiscard]] static int craftedItemStatId(int itemId) noexcept { return CRAFTED_BASE + itemId; }
    [[nodiscard]] static int usedItemStatId(int itemId) noexcept { return USED_BASE + itemId; }
    [[nodiscard]] static int brokenItemStatId(int itemId) noexcept { return BROKEN_BASE + itemId; }
