...
class PlayerStats {
   public:
    ...
    [[nodiscard]] int get(const StatId& stat) const {
        return getFrom(stats_, stat.id);
    }

    [[nodiscard]] int getById(int statId) const {
        return getFrom(stats_, statId);
    }

    [[nodiscard]] bool hasStat(int statId) const {
        return getFrom(stats_, statId) > 0;
    }

    [[nodiscard]] bool hasAchievement(const StatId& stat) const {
        return hasStat(stat.id);
    }
