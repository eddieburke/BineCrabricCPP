...
void FallingBlock::processFall(World* world, int x, int y, int z) {
  ...
  if(fallInstantly || !world->isRegionLoaded(fallX - regionRadius, fallY - regionRadius, fallZ - regionRadius,
                                             fallX + regionRadius, fallY + regionRadius, fallZ + regionRadius)) {
  ...
  auto* falling = new FallingBlockEntity(world, static_cast<double>(fallX) + 0.5, static_cast<double>(fallY) + 0.5,
                                         static_cast<double>(fallZ) + 0.5, id);
