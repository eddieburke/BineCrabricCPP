...
bool PistonBlock::push(World* world, int x, int y, int z, int dir)
{
    ...
    for (int step = 0; step < 13; ++step) {
        ...
        if (Block::BLOCKS[static_cast<std::size_t>(blockId)] != nullptr
            && Block::BLOCKS[static_cast<std::size_t>(blockId)]->getPistonBehavior() == 1) {
            Block::BLOCKS[static_cast<std::size_t>(blockId)]->dropStacks(world, px, py, pz, world->getBlockMeta(px, py, pz));
    ...
    while (px != x || py != y || pz != z) {
        ...
        if (sourceId == id && sourceX == x && sourceY == y && sourceZ == z) {
            ...
            world->setBlockWithoutNotifyingNeighbors(px, py, pz, Block::MOVING_PISTON != nullptr ? Block::MOVING_PISTON->id : 36, headMeta);
            world->setBlockEntity(px, py, pz,
                PistonExtensionBlock::createPistonBlockEntity(
                    Block::PISTON_HEAD != nullptr ? Block::PISTON_HEAD->id : 34, headMeta, dir, true, false));
        } else {
            world->setBlockWithoutNotifyingNeighbors(px, py, pz, Block::MOVING_PISTON != nullptr ? Block::MOVING_PISTON->id : 36, sourceMeta);
            world->setBlockEntity(px, py, pz,
                PistonExtensionBlock::createPistonBlockEntity(sourceId, sourceMeta, dir, true, false));
        }
        px = sourceX;
        py = sourceY;
        pz = sourceZ;
...
void PistonBlock::onBlockAction(World* world, int x, int y, int z, int data1, int data2)
{
    ...
    if (data1 == 0) {
        ...
        world->setBlockWithoutNotifyingNeighbors(x, y, z, Block::MOVING_PISTON != nullptr ? Block::MOVING_PISTON->id : 36, facing);
        ...
        if (sticky) {
            ...
            if (!pulled && farId > 0 && canMoveBlock(farId, world, farX, farY, farZ, false)
                ...
                world->setBlockWithoutNotifyingNeighbors(x, y, z,
                    Block::MOVING_PISTON != nullptr ? Block::MOVING_PISTON->id : 36, farMeta);
                world->setBlockEntity(x, y, z,
                    PistonExtensionBlock::createPistonBlockEntity(farId, farMeta, facing, false, false));
            } else if (!pulled) {
                deaf_ = false;
...
{
    const Box fullCube(static_cast<double>(x), static_cast<double>(y), static_cast<double>(z),
