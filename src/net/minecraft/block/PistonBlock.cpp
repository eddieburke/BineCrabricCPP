...
bool PistonBlock::push(World* world, int x, int y, int z, int dir) {
  ...
  for(int step = 0; step < 13; ++step) {
    ...
    if(Block::BLOCKS[static_cast<std::size_t>(blockId)] != nullptr &&
       Block::BLOCKS[static_cast<std::size_t>(blockId)]->getPistonBehavior() == 1) {
      Block::BLOCKS[static_cast<std::size_t>(blockId)]->dropStacks(world, px, py, pz,
                                                                   world->getBlockMeta(px, py, pz));
...
void PistonBlock::addIntersectingBoundingBox(World* /*world*/, int x, int y, int z, const net::minecraft::Box& box,
...
void PistonBlock::registerClass() {
  Block::STICKY_PISTON = (new PistonBlock(kBlockId, 106, true))
                             ...
                             ->ignoreMetaUpdates();
  Block::PISTON = (new PistonBlock(33, 107, false))
                      ->setHardness(0.5f)
                      ->setTranslationKey("pistonBase")
                      ->ignoreMetaUpdates();
}
void PistonBlock::registerRecipes(recipe::CraftingRecipeManager& recipeManager) {
  recipeManager.addShapedRecipe(ItemStack(Block::PISTON),
                                {std::string("TTT"), std::string("#X#"), std::string("#R#"), '#', Block::COBBLESTONE,
                                 'X', Item::byRawId(9), 'R', Item::byRawId(75), 'T', Block::PLANKS});
