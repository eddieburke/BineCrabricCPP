...
public:
  ...
  explicit PistonExtensionBlock(int id) : BlockWithEntity(id, material::Material::PISTON) {
    setHardness(-1.0f);
  }
  ...
  [[nodiscard]] std::optional<net::minecraft::Box> getPushedBlockCollisionShape(World* world, int x, int y, int z,
                                                                                int blockId, float sizeMultiplier,
                                                                                int facing) const;
  static std::unique_ptr<entity::BlockEntity> createPistonBlockEntity(int blockId, int blockMeta, int facing,
