...
bool MobSpawnerBlockEntity::isPlayerInRange() const {
  ...
  return world->getClosestPlayer(static_cast<double>(x) + 0.5, static_cast<double>(y) + 0.5,
  ...
  if(world != nullptr && !world->isRemote()) {
    ...
    for(int attempt = 0; attempt < spawnAttempts; ++attempt) {
      ...
      LivingEntity* living = world->spawnMob(spawnedEntityId_, [&](LivingEntity& mob) {
        const Box spawnBox{static_cast<double>(x), static_cast<double>(y), static_cast<double>(z),
                           static_cast<double>(x + 1), static_cast<double>(y + 1), static_cast<double>(z + 1)};
        const Box expandedBox{spawnBox.minX - 8.0, spawnBox.minY - 4.0, spawnBox.minZ - 8.0,
                              spawnBox.maxX + 8.0, spawnBox.maxY + 4.0, spawnBox.maxZ + 8.0};
        int nearbyCount = 0;
        ...
        mob.setPositionAndAnglesKeepPrevAngles(spawnX, spawnY, spawnZ, world->random().nextFloat() * 360.0f,
                                               0.0f);
        return mob.canSpawn();
      });
      if(tooManyNearby) {
