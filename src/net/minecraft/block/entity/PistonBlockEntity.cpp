...
void PistonBlockEntity::pushEntities(float collisionShapeSizeMultiplier, float entityMoveMultiplier)
{
    ...
    const std::optional<Box> box = pistonExtension->getPushedBlockCollisionShape(
        world, x, y, z, pushedBlockId_, multiplier, facing_);
    if (!box.has_value()) {
        return;
    }
    ...
    const float moveX = entityMoveMultiplier * static_cast<float>(block::PistonConstants::HEAD_OFFSET_X[static_cast<std::size_t>(facing_)]);
    const float moveY = entityMoveMultiplier * static_cast<float>(block::PistonConstants::HEAD_OFFSET_Y[static_cast<std::size_t>(facing_)]);
    const float moveZ = entityMoveMultiplier * static_cast<float>(block::PistonConstants::HEAD_OFFSET_Z[static_cast<std::size_t>(facing_)]);
