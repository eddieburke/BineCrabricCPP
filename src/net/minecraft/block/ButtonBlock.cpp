...
ButtonBlock::ButtonBlock(int blockId, int textureId) : Block(blockId, textureId, material::Material::PISTON_BREAKABLE) {
...
bool ButtonBlock::canPlaceAt(World* world, int x, int y, int z, int side) const {
 ...
 return side >= 2 && sideSuffocates(world, x, y, z, side);
}
...
void ButtonBlock::setupRenderBoundingBox() {
 ...
 setBoundingBox(
     0.5f - halfWidth, 0.5f - halfHeight, 0.5f - halfDepth, 0.5f + halfWidth, 0.5f + halfHeight, 0.5f + halfDepth);
}
...
void ButtonBlock::onBlockBreakStart(World* world, int x, int y, int z, net::minecraft::PlayerEntity* player) {
...
void ButtonBlock::onTick(World* world, int x, int y, int z, JavaRandom& /*random*/) {
...
bool ButtonBlock::canTransferPowerInDirection(World* world, int x, int y, int z, int direction) const {
 if(world == nullptr || (world->getBlockMeta(x, y, z) & 8) == 0) {
  return false;
 }
 const int face = world->getBlockMeta(x, y, z) & 7;
...
void ButtonBlock::registerClass() {
 Block::BUTTON = (new ButtonBlock(kBlockId, Block::BLOCKS[1]->textureId))
                     ->setHardness(0.5f)
