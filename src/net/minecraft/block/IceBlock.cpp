...
bool IceBlock::isSideVisibleForBounds(
    const BlockView* blockView, int x, int y, int z, int side, const net::minecraft::Box& bounds) const {
 return TranslucentBlock::isSideVisibleForBounds(blockView, x, y, z, 1 - side, bounds);
}
void IceBlock::afterBreak(World* world, net::minecraft::PlayerEntity* player, int x, int y, int z, int meta) {
...
void IceBlock::registerClass() {
 Block::ICE = (new IceBlock(kBlockId, 67))
                  ->setHardness(0.5f)
                  ->setOpacity(3)
