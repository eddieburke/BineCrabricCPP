...
class RailNode {
public:
    RailNode(World* worldIn, int xIn, int yIn, int zIn)
        : world(worldIn), x(xIn), y(yIn), z(zIn)
    {
...
private:
    ...
    {
        return RailBlock::isRail(world, cx, cy, cz)
...
{
    if (alwaysStraight ? Block::POWERED_RAIL != nullptr && id == Block::POWERED_RAIL->id && (meta & 8) == 0 : meta >= 6) {
        return textureId - 16;
    }
    return textureId;
}
...
void RailBlock::neighborUpdate(World* world, int x, int y, int z, int neighborId)
{
    ...
    if (Block::POWERED_RAIL != nullptr && id == Block::POWERED_RAIL->id) {
        ...
        powered = powered
            || isPoweredByConnectedRails(world, x, y, z, meta, true, 0)
...
bool RailBlock::isPoweredByRail(
    World* world, int x, int y, int z, bool towardsNegative, int depth, int shape)
{
