...
bool RailBlock::isPoweredByConnectedRails(World* world, int x, int y, int z, int meta, bool towardsNegative, int depth) {
...
std::optional<net::minecraft::HitResult> RailBlock::raycast(World* world, int x, int y, int z,
...
void RailBlock::registerRecipes(recipe::CraftingRecipeManager& recipeManager) {
  ...
  recipeManager.addShapedRecipe(ItemStack(Block::POWERED_RAIL, 6),
                                {std::string("X X"), std::string("X#X"), std::string("XRX"), 'X', Item::byRawId(10),
                                 'R', Item::byRawId(75), '#', Item::byRawId(24)});
}
namespace {
} // namespace
