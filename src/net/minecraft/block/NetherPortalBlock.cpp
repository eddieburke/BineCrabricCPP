...
std::optional<Box> NetherPortalBlock::getCollisionShape(
...
bool NetherPortalBlock::isSideVisible(const BlockView* blockView, int x, int y, int z, int side) const
{
    if (blockView == nullptr) {
        return false;
    }
    ...
    const bool westEdge =
        blockView->getBlockId(x - 1, y, z) == id && blockView->getBlockId(x - 2, y, z) != id;
    const bool eastEdge =
        blockView->getBlockId(x + 1, y, z) == id && blockView->getBlockId(x + 2, y, z) != id;
    const bool northEdge =
        blockView->getBlockId(x, y, z - 1) == id && blockView->getBlockId(x, y, z - 2) != id;
    const bool southEdge =
        blockView->getBlockId(x, y, z + 1) == id && blockView->getBlockId(x, y, z + 2) != id;
...
void NetherPortalBlock::randomDisplayTick(
