...
bool PistonExtensionBlock::canPlaceAt(
    World* /*world*/, int /*x*/, int /*y*/, int /*z*/, int /*side*/) const
{
    return false;
}

std::optional<net::minecraft::Box> PistonExtensionBlock::getCollisionShape(
    World* world, int x, int y, int z) const
{
    ...
    return getPushedBlockCollisionShape(
        world, x, y, z, piston->getPushedBlockId(), progress, piston->getFacing());
}
...
bool PistonExtensionBlock::onUse(
...
void PistonExtensionBlock::dropStacks(
    World* world, int x, int y, int z, int /*meta*/, float /*luck*/)
{
...
void PistonExtensionBlock::neighborUpdate(
    World* world, int x, int y, int z, int /*neighborId*/)
{
    if (world == nullptr || world->isRemote() || world->getBlockEntity(x, y, z) != nullptr) {
        return;
    }
    ...
    int facing) const
{
    ...
    const float offsetX = static_cast<float>(PistonConstants::HEAD_OFFSET_X[static_cast<std::size_t>(facing)]) * sizeMultiplier;
    const float offsetY = static_cast<float>(PistonConstants::HEAD_OFFSET_Y[static_cast<std::size_t>(facing)]) * sizeMultiplier;
    const float offsetZ = static_cast<float>(PistonConstants::HEAD_OFFSET_Z[static_cast<std::size_t>(facing)]) * sizeMultiplier;
