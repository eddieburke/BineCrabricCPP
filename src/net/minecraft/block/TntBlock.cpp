...
void TntBlock::onMetadataChange(World* world, int x, int y, int z, int meta) {
  ...
  if((meta & 1) == 0) {
    ...
    auto* tnt = new TntEntity(world, static_cast<float>(x) + 0.5f, static_cast<float>(y) + 0.5f,
...
void TntBlock::onDestroyedByExplosion(World* world, int x, int y, int z) {
  ...
  auto* tnt =
      new TntEntity(world, static_cast<float>(x) + 0.5f, static_cast<float>(y) + 0.5f, static_cast<float>(z) + 0.5f);
...
void TntBlock::registerClass() {
  Block::TNT =
      (new TntBlock(kBlockId, 8))->setHardness(0.0f)->setSoundGroup(&kDirtSound)->setTranslationKey("tnt");
}
void TntBlock::registerRecipes(recipe::CraftingRecipeManager& recipeManager) {
  recipeManager.addShapedRecipe(ItemStack(Block::TNT), {std::string("X#X"), std::string("#X#"), std::string("X#X"),
                                                        'X', Item::byRawId(33), '#', Block::SAND});
}
