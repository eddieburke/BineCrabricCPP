...
RedstoneOreBlock::RedstoneOreBlock(int id, int textureId, bool litIn)
    : Block(id, textureId, material::Material::STONE) {
  lit = litIn;
  if(lit) {
...
void RedstoneOreBlock::onBlockBreakStart(World* world, int x, int y, int z, net::minecraft::PlayerEntity* player) {
  light(world, x, y, z);
...
void RedstoneOreBlock::randomDisplayTick(World* world, int x, int y, int z, JavaRandom& /*random*/) {
...
void RedstoneOreBlock::registerClass() {
  Block::REDSTONE_ORE = (new RedstoneOreBlock(73, 51, false))
                            ->setHardness(3.0f)
                            ->setResistance(5.0f)
                            ->setTranslationKey("oreRedstone")
                            ->ignoreMetaUpdates();
  Block::LIT_REDSTONE_ORE = (new RedstoneOreBlock(kBlockId, 51, true))
                                ->setLuminance(0.625f)
                                ->setHardness(3.0f)
                                ->setResistance(5.0f)
                                ->setTranslationKey("oreRedstone")
