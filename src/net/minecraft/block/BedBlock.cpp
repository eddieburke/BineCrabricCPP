...
int BedBlock::getTexture(int side, int meta) const {
 ...
 const int mappedSide =
     util::math::Facings::BED_FACINGS[static_cast<std::size_t>(direction)][static_cast<std::size_t>(side)];
 if(isHeadOfBed(meta)) {
  if(mappedSide == 2) {
...
void BedBlock::registerClass() {
 Block::BED = (new BedBlock(kBlockId))
                  ->setHardness(0.2f)
                  ->setTranslationKey("bed")
