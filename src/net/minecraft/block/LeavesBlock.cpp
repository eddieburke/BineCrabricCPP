...
void LeavesBlock::onBreak(World* world, int x, int y, int z) {
  ...
  if(!world->isRegionLoaded(x - loadedRadius,
                            y - loadedRadius,
                            z - loadedRadius,
                            x + loadedRadius,
                            y + loadedRadius,
...
void LeavesBlock::afterBreak(World* world, net::minecraft::PlayerEntity* player, int x, int y, int z, int meta) {
...
void LeavesBlock::registerClass() {
  Block::LEAVES = (new LeavesBlock(kBlockId, 52))
                      ->setHardness(0.2f)
                      ->setOpacity(1)
                      ->setSoundGroup(&kDirtSound)
                      ->setTranslationKey("leaves")
