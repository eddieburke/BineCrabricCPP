...
std::optional<net::minecraft::Box> SnowyBlock::getCollisionShape(World* world, int x, int y, int z) const {
...
void SnowyBlock::afterBreak(World* world, net::minecraft::PlayerEntity* player, int x, int y, int z, int meta) {
...
bool SnowyBlock::isSideVisibleForBounds(
...
void SnowyBlock::registerClass() {
  Block::SNOW =
      (new SnowyBlock(kBlockId, 66))->setHardness(0.1f)->setSoundGroup(&kClothSound)->setTranslationKey("snow");
}
