...
bool LeverBlock::onUse(World* world, int x, int y, int z, net::minecraft::PlayerEntity* /*player*/) {
  ...
  world->playSound(static_cast<double>(x) + 0.5, static_cast<double>(y) + 0.5, static_cast<double>(z) + 0.5,
                   "random.click", 0.3f, powered > 0 ? 0.6f : 0.5f);
...
bool LeverBlock::canTransferPowerInDirection(World* world, int x, int y, int z, int direction) const {
  if(world == nullptr || (world->getBlockMeta(x, y, z) & 8) == 0) {
    return false;
  }
  const int face = world->getBlockMeta(x, y, z) & 7;
...
bool LeverBlock::isEmittingRedstonePowerInDirection(const BlockView* blockView, int x, int y, int z,
                                                    int /*direction*/) const {
  return blockView != nullptr && (blockView->getBlockMeta(x, y, z) & 8) > 0;
}
void LeverBlock::registerClass() {
  Block::LEVER = (new LeverBlock(kBlockId, 96))
                     ->setHardness(0.5f)
                     ->setSoundGroup(&kWoodSound)
                     ->setTranslationKey("lever")
                     ->ignoreMetaUpdates();
}
void LeverBlock::registerRecipes(recipe::CraftingRecipeManager& recipeManager) {
  recipeManager.addShapedRecipe(
      ItemStack(Block::LEVER), {std::string("X"), std::string("#"), '#', Block::COBBLESTONE, 'X', Item::byRawId(24)});
}
