...
int LockedChestBlock::getTextureId(const BlockView* blockView, int x, int y, int z, int side) const {
  ...
  if(blockView != nullptr) {
    ...
    if(northId >= 0 && northId < Block::BLOCK_COUNT && Block::BLOCKS_OPAQUE[static_cast<std::size_t>(northId)] &&
       (southId < 0 || southId >= Block::BLOCK_COUNT ||
        !Block::BLOCKS_OPAQUE[static_cast<std::size_t>(southId)])) {
      facing = FACE_WEST;
    } else if(southId >= 0 && southId < Block::BLOCK_COUNT &&
              Block::BLOCKS_OPAQUE[static_cast<std::size_t>(southId)] &&
              (northId < 0 || northId >= Block::BLOCK_COUNT ||
               !Block::BLOCKS_OPAQUE[static_cast<std::size_t>(northId)])) {
      facing = FACE_EAST;
    } else if(westId >= 0 && westId < Block::BLOCK_COUNT &&
              Block::BLOCKS_OPAQUE[static_cast<std::size_t>(westId)] &&
              (eastId < 0 || eastId >= Block::BLOCK_COUNT ||
               !Block::BLOCKS_OPAQUE[static_cast<std::size_t>(eastId)])) {
      facing = FACE_SOUTH;
    } else if(eastId >= 0 && eastId < Block::BLOCK_COUNT &&
              Block::BLOCKS_OPAQUE[static_cast<std::size_t>(eastId)] &&
              (westId < 0 || westId >= Block::BLOCK_COUNT ||
               !Block::BLOCKS_OPAQUE[static_cast<std::size_t>(westId)])) {
      facing = FACE_NORTH;
    }
  }
...
void LockedChestBlock::registerClass() {
  Block::LOCKED_CHEST = (new LockedChestBlock(kBlockId))
                            ->setHardness(0.0f)
                            ->setLuminance(1.0f)
                            ->setSoundGroup(&kWoodSound)
                            ->setTranslationKey("lockedchest")
