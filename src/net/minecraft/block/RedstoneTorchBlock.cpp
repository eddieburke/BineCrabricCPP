...
void RedstoneTorchBlock::onTick(World* world, int x, int y, int z, JavaRandom& random) {
  ...
  if(lit) {
    if(unpower && Block::REDSTONE_TORCH != nullptr) {
      world->setBlock(x, y, z, Block::REDSTONE_TORCH->id,
                      static_cast<std::uint8_t>(world->getBlockMeta(x, y, z)));
      if(isBurnedOut(world, x, y, z, true)) {
        world->playSound(static_cast<double>(x) + 0.5, static_cast<double>(y) + 0.5,
                         static_cast<double>(z) + 0.5, "random.fizz", 0.5f,
                         2.6f + (world->random().nextFloat() - world->random().nextFloat()) * 0.8f);
        for(int i = 0; i < 5; ++i) {
          world->addParticle("smoke", static_cast<double>(x) + random.nextDouble() * 0.6 + 0.2,
                             static_cast<double>(y) + random.nextDouble() * 0.6 + 0.2,
                             static_cast<double>(z) + random.nextDouble() * 0.6 + 0.2, 0.0, 0.0, 0.0);
        }
      }
    }
  } else if(!unpower && !isBurnedOut(world, x, y, z, false) && Block::LIT_REDSTONE_TORCH != nullptr) {
    world->setBlock(x, y, z, Block::LIT_REDSTONE_TORCH->id,
                    static_cast<std::uint8_t>(world->getBlockMeta(x, y, z)));
  }
}
...
bool RedstoneTorchBlock::isEmittingRedstonePowerInDirection(const BlockView* blockView, int x, int y, int z,
                                                            int direction) const {
