...
void RedstoneTorchBlock::onTick(World* world, int x, int y, int z, JavaRandom& random)
{
    ...
    if (lit) {
        if (unpower && Block::REDSTONE_TORCH != nullptr) {
            world->setBlock(x, y, z, Block::REDSTONE_TORCH->id, static_cast<std::uint8_t>(world->getBlockMeta(x, y, z)));
        ...
        world->setBlock(x, y, z, Block::LIT_REDSTONE_TORCH->id, static_cast<std::uint8_t>(world->getBlockMeta(x, y, z)));
    }
}
...
bool RedstoneTorchBlock::canTransferPowerInDirection(
...
void RedstoneTorchBlock::randomDisplayTick(
