...
std::optional<net::minecraft::Box> Block::getCollisionShape(World* /*world*/, int x, int y, int z) const {
  return net::minecraft::Box{static_cast<double>(x) + minX, static_cast<double>(y) + minY,
...
void Block::dropStack(World* world, int x, int y, int z, const net::minecraft::ItemStack& stack) {
  ...
  auto* itemEntity = new ItemEntity(world, static_cast<double>(x) + offsetX, static_cast<double>(y) + offsetY,
                                    static_cast<double>(z) + offsetZ, stack);
...
std::optional<net::minecraft::HitResult> Block::raycastLocalBounds(double localMinX, double localMinY, double localMinZ,
                                                                   double localMaxX, double localMaxY, double localMaxZ,
                                                                   int x, int y, int z, Vec3d startPos, Vec3d endPos) {
...
std::optional<net::minecraft::HitResult> Block::raycast(World* world, int x, int y, int z, Vec3d startPos,
                                                        Vec3d endPos) const {
...
static registry::RegisterPhase<BlockRegistryFinalizePass> s_finalize(mod::LifecyclePhase::BlockRegistryFinalize, 0);
static registry::RegisterPhase<GenericBlockItemPass> s_genericBlockItems(mod::LifecyclePhase::BlockItemRegistration,
