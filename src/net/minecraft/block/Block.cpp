...
Block::Block(int id, int textureId, Material& material)
    : Block(id, material)
{
...
{
    return {static_cast<double>(x) + minX, static_cast<double>(y) + minY, static_cast<double>(z) + minZ,
            static_cast<double>(x) + maxX, static_cast<double>(y) + maxY, static_cast<double>(z) + maxZ};
}

std::optional<net::minecraft::Box> Block::getCollisionShape(World* /*world*/, int x, int y, int z) const
{
    return net::minecraft::Box{static_cast<double>(x) + minX, static_cast<double>(y) + minY, static_cast<double>(z) + minZ,
               static_cast<double>(x) + maxX, static_cast<double>(y) + maxY, static_cast<double>(z) + maxZ};
}

void Block::addIntersectingBoundingBox(World* world, int x, int y, int z, const Box& box, std::vector<Box>& boxes) const
{
...
void Block::dropStack(World* world, int x, int y, int z, const net::minecraft::ItemStack& stack)
{
    ...
    auto* itemEntity = new ItemEntity(
        world, static_cast<double>(x) + offsetX, static_cast<double>(y) + offsetY, static_cast<double>(z) + offsetZ, stack);
...
bool containsInYzPlane(
    const std::optional<Vec3d>& v, double minY, double maxY, double minZ, double maxZ)
{
    if (!v) {
        return false;
    }
...
bool containsInXzPlane(
    const std::optional<Vec3d>& v, double minX, double maxX, double minZ, double maxZ)
{
    if (!v) {
        return false;
    }
...
bool containsInXyPlane(
    const std::optional<Vec3d>& v, double minX, double maxX, double minY, double maxY)
{
    if (!v) {
        return false;
    }
...
std::optional<net::minecraft::HitResult> Block::raycastLocalBounds(
    double localMinX,
    double localMinY,
    double localMinZ,
    double localMaxX,
    ...
    Vec3d endPos)
{
    ...
    for (int i = 0; i < 6; ++i) {
        ...
        if (bestIndex == 0
            || distSq(startPos, **candidates[i]) < distSq(startPos, **candidates[bestIndex - 1])) {
            bestIndex = i + 1;
        }
    }
...
static registry::RegisterPhase<BlockRegistryFinalizePass> s_finalize(
    mod::LifecyclePhase::BlockRegistryFinalize, 0);
static registry::RegisterPhase<GenericBlockItemPass> s_genericBlockItems(
    mod::LifecyclePhase::BlockItemRegistration, registry::kGenericBlockItemOrder);
