...
void FurnaceBlock::randomDisplayTick(World* world, int x, int y, int z, JavaRandom& random) {
...
void FurnaceBlock::registerClass() {
  Block::FURNACE = (new FurnaceBlock(kBlockId, false))
                       ->setHardness(3.5f)
                       ->setTranslationKey("furnace")
                       ->ignoreMetaUpdates();
  Block::LIT_FURNACE = (new FurnaceBlock(62, true))
                           ->setHardness(3.5f)
                           ->setLuminance(0.875f)
                           ->setTranslationKey("furnace")
                           ->ignoreMetaUpdates();
...
void FurnaceBlock::registerRecipes(recipe::CraftingRecipeManager& recipeManager) {
  recipeManager.addShapedRecipe(ItemStack(Block::FURNACE), {std::string("###"), std::string("# #"),
                                                            std::string("###"), '#', Block::COBBLESTONE});
}
namespace {
} // namespace
