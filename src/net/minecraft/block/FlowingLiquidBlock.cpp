...
void FlowingLiquidBlock::spreadTo(World* world, int x, int y, int z, int depth) {
  ...
  if(existingId > 0) {
    if(&material == &material::Material::LAVA) {
      ...
      Block::BLOCKS[static_cast<std::size_t>(existingId)]->dropStacks(world, x, y, z,
                                                                      world->getBlockMeta(x, y, z));
    }
  }
...
bool* FlowingLiquidBlock::getSpread(World* world, int x, int y, int z) {
  for(int direction = 0; direction < 4; ++direction) {
    ...
    distanceToGap[direction] =
        !isLiquidBreaking(world, nx, ny - 1, nz) ? 0 : getDistanceToGap(world, nx, ny, nz, 1, direction);
  }
...
void FlowingLiquidBlock::registerClass() {
  ...
  Block::FLOWING_WATER = (new FlowingLiquidBlock(kBlockId, mat::Material::WATER))
                             ->setHardness(100.0f)
                             ->setOpacity(3)
                             ->setTranslationKey("water")
                             ->disableTrackingStatistics()
                             ->ignoreMetaUpdates();
  Block::FLOWING_LAVA = (new FlowingLiquidBlock(10, mat::Material::LAVA))
                            ->setHardness(0.0f)
                            ->setLuminance(1.0f)
                            ->setOpacity(255)
                            ->setTranslationKey("lava")
                            ->disableTrackingStatistics()
                            ->ignoreMetaUpdates();
}
