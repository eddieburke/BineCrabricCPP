...
PressurePlateBlock::PressurePlateBlock(int blockId, int textureId, PressurePlateActivationRule activationRuleIn,
                                       Material& material)
...
void PressurePlateBlock::updatePlateState(World* world, int x, int y, int z) {
  ...
  const net::minecraft::Box box(static_cast<double>(x) + inset, static_cast<double>(y),
                                static_cast<double>(z) + inset, static_cast<double>(x + 1) - inset,
                                static_cast<double>(y) + 0.25, static_cast<double>(z + 1) - inset);
...
void PressurePlateBlock::onTick(World* world, int x, int y, int z, JavaRandom& /*random*/) {
  if(world == nullptr || world->isRemote() || world->getBlockMeta(x, y, z) == 0) {
    return;
  }
  updatePlateState(world, x, y, z);
}
void PressurePlateBlock::onEntityCollision(World* world, int x, int y, int z, net::minecraft::Entity* /*entity*/) {
...
bool PressurePlateBlock::isEmittingRedstonePowerInDirection(const BlockView* blockView, int x, int y, int z,
                                                            int /*direction*/) const {
  return blockView != nullptr && blockView->getBlockMeta(x, y, z) > 0;
}
bool PressurePlateBlock::canTransferPowerInDirection(World* world, int x, int y, int z, int direction) const {
  return world != nullptr && world->getBlockMeta(x, y, z) != 0 && direction == 1;
}
void PressurePlateBlock::registerClass() {
  namespace mat = material;
  Block::STONE_PRESSURE_PLATE = (new PressurePlateBlock(70, Block::BLOCKS[1]->textureId,
                                                        PressurePlateActivationRule::MOBS, mat::Material::STONE))
                                    ->setHardness(0.5f)
                                    ->setTranslationKey("pressurePlate")
                                    ->ignoreMetaUpdates();
  Block::WOODEN_PRESSURE_PLATE =
      (new PressurePlateBlock(kBlockId, Block::BLOCKS[5]->textureId, PressurePlateActivationRule::EVERYTHING,
                              mat::Material::WOOD))
          ->setHardness(0.5f)
