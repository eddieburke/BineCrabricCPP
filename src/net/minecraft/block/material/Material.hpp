...
class Material {
public:
    explicit Material(const MapColor& mapColor)
        : mapColor(mapColor)
    {
    }
    ...
    [[nodiscard]] virtual bool isFluid() const { return false; }
    [[nodiscard]] virtual bool isSolid() const { return true; }
    [[nodiscard]] virtual bool blocksVision() const { return true; }
    [[nodiscard]] virtual bool blocksMovement() const { return true; }

    [[nodiscard]] bool isBurnable() const { return burnable_; }
    [[nodiscard]] bool isReplaceable() const { return replaceable_; }
    [[nodiscard]] bool suffocates() const { return !transparent_ && blocksMovement(); }
    [[nodiscard]] bool isHandHarvestable() const { return handHarvestable_; }
    [[nodiscard]] int getPistonBehavior() const { return pistonBehavior_; }
