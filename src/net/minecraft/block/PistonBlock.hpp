...
public:
  ...
  [[nodiscard]] int getTopTexture() const {
    return sticky ? PistonConstants::TEXTURE_STICKY_TOP : PistonConstants::TEXTURE_TOP;
  }
  ...
  void addIntersectingBoundingBox(World* world, int x, int y, int z, const net::minecraft::Box& box,
                                  std::vector<Box>& boxes) const override;
  static int getFacing(int meta) {
    return meta & 7;
  }
  static bool isExtended(int meta) {
    return (meta & 8) != 0;
  }
