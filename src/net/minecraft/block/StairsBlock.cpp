...
StairsBlock::StairsBlock(int id, Block& base) : Block(id, base.textureId, base.material) {
  baseBlock = &base;
...
void StairsBlock::addIntersectingBoundingBox(World* world, int x, int y, int z, const net::minecraft::Box& box,
...
int StairsBlock::getTextureId(const BlockView* blockView, int x, int y, int z, int side) const {
  return baseBlock != nullptr ? baseBlock->getTextureId(blockView, x, y, z, side)
                              : Block::getTextureId(blockView, x, y, z, side);
}
...
void StairsBlock::applyVelocity(World* world, int x, int y, int z, net::minecraft::Entity* entity,
...
void StairsBlock::registerClass() {
  ...
  Block::COBBLESTONE_STAIRS =
      (new StairsBlock(kBlockId, *Block::BLOCKS[4]))->setTranslationKey("stairsStone")->ignoreMetaUpdates();
}
void StairsBlock::registerRecipes(recipe::CraftingRecipeManager& recipeManager) {
  ...
  recipeManager.addShapedRecipe(
      ItemStack(Block::COBBLESTONE_STAIRS, 4),
