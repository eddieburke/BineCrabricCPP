...
void FireBlock::trySpreadingFire(
    World* world, int x, int y, int z, int spreadFactor, JavaRandom& random, int currentAge) const {
    if (world == nullptr) {
        return;
    }
...
void FireBlock::randomDisplayTick(World* world, int x, int y, int z, JavaRandom& random) {
    ...
    if (random.nextInt(24) == 0) {
        world->playSound(static_cast<float>(x) + 0.5f,
