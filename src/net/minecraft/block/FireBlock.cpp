...
bool FireBlock::isFlammableBlock(const BlockView* blockView, int x, int y, int z) {
  ...
  return static_cast<const FireBlock*>(Block::FIRE)
             ->burnChances_[static_cast<std::size_t>(blockView->getBlockId(x, y, z))] > 0;
}
...
void FireBlock::trySpreadingFire(World* world, int x, int y, int z, int spreadFactor, JavaRandom& random,
...
void FireBlock::randomDisplayTick(World* world, int x, int y, int z, JavaRandom& random) {
  if(world == nullptr) {
    return;
  }
  if(random.nextInt(24) == 0) {
    world->playSound(static_cast<float>(x) + 0.5f, static_cast<float>(y) + 0.5f, static_cast<float>(z) + 0.5f,
  ...
  if(isFlammable(world, x - 1, y, z)) {
    for(int i = 0; i < 2; ++i) {
      world->addParticle("largesmoke", static_cast<float>(x) + random.nextFloat() * 0.1f,
                         static_cast<float>(y) + random.nextFloat(), static_cast<float>(z) + random.nextFloat(),
                         0.0, 0.0, 0.0);
    }
  }
  if(isFlammable(world, x + 1, y, z)) {
    for(int i = 0; i < 2; ++i) {
      world->addParticle("largesmoke", static_cast<float>(x + 1) - random.nextFloat() * 0.1f,
                         static_cast<float>(y) + random.nextFloat(), static_cast<float>(z) + random.nextFloat(),
                         0.0, 0.0, 0.0);
    }
  }
  if(isFlammable(world, x, y, z - 1)) {
    for(int i = 0; i < 2; ++i) {
      world->addParticle("largesmoke", static_cast<float>(x) + random.nextFloat(),
                         static_cast<float>(y) + random.nextFloat(),
                         static_cast<float>(z) + random.nextFloat() * 0.1f, 0.0, 0.0, 0.0);
    }
  }
  if(isFlammable(world, x, y, z + 1)) {
    for(int i = 0; i < 2; ++i) {
      world->addParticle("largesmoke", static_cast<float>(x) + random.nextFloat(),
                         static_cast<float>(y) + random.nextFloat(),
                         static_cast<float>(z + 1) - random.nextFloat() * 0.1f, 0.0, 0.0, 0.0);
    }
  }
  if(isFlammable(world, x, y + 1, z)) {
    for(int i = 0; i < 2; ++i) {
      world->addParticle("largesmoke", static_cast<float>(x) + random.nextFloat(),
                         ...
                         static_cast<float>(z) + random.nextFloat(), 0.0, 0.0, 0.0);
    }
  }
}
void FireBlock::registerClass() {
  Block::FIRE = (new FireBlock(kBlockId, 31))
                    ->setHardness(0.0f)
                    ->setLuminance(1.0f)
