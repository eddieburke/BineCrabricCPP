...
void LiquidBlock::applyVelocity(World* world, int x, int y, int z, net::minecraft::Entity* /*entity*/,
...
void LiquidBlock::fizz(World* world, int x, int y, int z) {
  ...
  for(int i = 0; i < 8; ++i) {
    world->addParticle("largesmoke", static_cast<double>(x) + world->random().nextDouble(),
                       static_cast<double>(y) + 1.2, static_cast<double>(z) + world->random().nextDouble(), 0.0,
                       0.0, 0.0);
  }
}
...
void LiquidBlock::randomDisplayTick(World* world, int x, int y, int z, JavaRandom& random) {
  if(world == nullptr) {
    return;
  }
  if(&material == &material::Material::WATER) {
    ...
    if(random.nextInt(64) == 0 && meta > 0 && meta < 8) {
      world->playSound(static_cast<float>(x) + 0.5f, static_cast<float>(y) + 0.5f, static_cast<float>(z) + 0.5f,
                       "liquid.water", random.nextFloat() * 0.25f + 0.75f, random.nextFloat() * 1.0f + 0.5f);
    }
  }
