...
bool LiquidBlock::isSideVisibleForBounds(
...
void LiquidBlock::applyVelocity(
...
void LiquidBlock::fizz(World* world, int x, int y, int z) {
  ...
  for(int i = 0; i < 8; ++i) {
    world->addParticle("largesmoke",
                       static_cast<double>(x) + world->random().nextDouble(),
                       static_cast<double>(y) + 1.2,
                       static_cast<double>(z) + world->random().nextDouble(),
                       0.0,
                       0.0,
                       0.0);
  }
