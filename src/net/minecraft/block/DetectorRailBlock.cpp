...
bool DetectorRailBlock::canTransferPowerInDirection(World* world, int x, int y, int z, int direction) const {
...
void DetectorRailBlock::registerClass() {
  Block::DETECTOR_RAIL = (new DetectorRailBlock(kBlockId, 195))
                             ->setHardness(0.7f)
                             ->setSoundGroup(&kMetalSound)
                             ->setTranslationKey("detectorRail")
                             ->ignoreMetaUpdates();
}
void DetectorRailBlock::registerRecipes(recipe::CraftingRecipeManager& recipeManager) {
  recipeManager.addShapedRecipe(ItemStack(Block::DETECTOR_RAIL, 6),
                                {std::string("X X"), std::string("X#X"), std::string("XRX"), 'X', Item::byRawId(9),
                                 'R', Item::byRawId(75), '#', Block::STONE_PRESSURE_PLATE});
}
