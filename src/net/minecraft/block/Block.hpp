...
class Block {
 ...
 Block* setLightColor(float red, float green, float blue);
 ...
 // Whether entity blob shadows may be drawn on this block. Vanilla limits
 // shadows to full cubes; mod blocks with custom shapes can opt in.
 [[nodiscard]] virtual bool receivesEntityShadow() const {
  return isFullCube();
 }
 ...
 // Virtual so subclasses that need custom face-culling (e.g. liquids merging
 // with neighbors of the same fluid) are honored on the render path, which
 // calls this overload directly rather than the non-bounds isSideVisible.
 [[nodiscard]] virtual bool isSideVisibleForBounds(const net::minecraft::BlockView* blockView,
                                                   int x,
                                                   int y,
                                                   int z,
                                                   int side,
