...
RedstoneWireBlock::RedstoneWireBlock(int id, int textureId) : Block(id, textureId, material::Material::PISTON_BREAKABLE) {
...
bool RedstoneWireBlock::isEmittingRedstonePowerInDirection(const BlockView* blockView, int x, int y, int z,
                                                           int direction) const {
...
void RedstoneWireBlock::randomDisplayTick(World* world, int x, int y, int z, JavaRandom& random) {
  ...
  world->addParticle("reddust", px, py, pz, static_cast<double>(redIn), static_cast<double>(greenIn),
                     static_cast<double>(blueIn));
}
