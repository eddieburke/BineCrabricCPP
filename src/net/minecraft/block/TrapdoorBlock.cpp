...
void playDoorToggleSound(World* world, int x, int y, int z) {
  ...
  world->playSound(static_cast<double>(x) + 0.5, static_cast<double>(y) + 0.5, static_cast<double>(z) + 0.5, sound,
...
std::optional<net::minecraft::HitResult> TrapdoorBlock::raycast(World* world, int x, int y, int z,
...
void TrapdoorBlock::registerClass() {
  ...
  Block::TRAPDOOR = (new TrapdoorBlock(kBlockId, mat::Material::WOOD))
                        ->setHardness(3.0f)
                        ->setSoundGroup(&kWoodSound)
                        ->setTranslationKey("trapdoor")
