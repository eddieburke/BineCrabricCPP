...
void playMining(World* world, double x, double y, double z, Block* block) {
  ...
  const BlockSoundGroup* group = block->getSoundGroup();
  world->playSound(x, y, z, block->getMiningSound(), (group->getVolume() + 1.0f) / 8.0f,
                   group->getPitch() * 0.5f);
}
void playBreak(World* world, double x, double y, double z, Block* block) {
  ...
  const BlockSoundGroup* group = block->getSoundGroup();
  world->playSound(x, y, z, block->getBreakSound(), (group->getVolume() + 1.0f) / 2.0f,
                   group->getPitch() * 0.8f);
}
void playPlace(World* world, double x, double y, double z, Block* block) {
  ...
  const BlockSoundGroup* group = block->getSoundGroup();
  world->playSound(x, y, z, block->getStepSound(), (group->getVolume() + 1.0f) * 0.5f,
                   group->getPitch() * 0.8f);
}
