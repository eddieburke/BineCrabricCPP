...
void World::manageChunkUpdatesAndEvents() {
  ...
  for(const ChunkPos& chunkPos : activeChunks_) {
    ...
    if(ambientSoundCounter_ == 0) {
      ...
      if(blockId == 0 && getBrightness(worldX, localY, worldZ) <= random_.nextInt(8) && getBrightness(LightType::Sky, worldX, localY, worldZ) <= 0) {
    ...
    if(random_.nextInt(16) == 0) {
      ...
      const int worldZ = localZ + chunkOriginZ;
      const int worldY = getTopSolidBlockY(worldX, worldZ);
      if(getBiome(worldX, worldZ).canSnow() && worldY >= 0 && worldY < Chunk::height && chunk.getLight(LightType::Block, localX, worldY, localZ) < 10) {
        const int belowId = chunk.getBlockId(localX, worldY - 1, localZ);
        const int currentId = chunk.getBlockId(localX, worldY, localZ);
        if(isRaining() && currentId == 0 && Block::SNOW != nullptr && Block::SNOW->canPlaceAt(this, worldX, worldY, worldZ) && belowId != 0 && Block::BLOCKS[static_cast<std::size_t>(belowId)] != nullptr && Block::BLOCKS[static_cast<std::size_t>(belowId)]->material.blocksMovement()) {
