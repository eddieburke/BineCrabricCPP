...
ChunkSource* World::createChunkCache() {
    ...
    cache->initAsync(getSeed(), [this](std::uint64_t seed) { return dimension->createChunkGeneratorFromSeed(seed); });
...
void World::handleChunkDataUpdate(
    ...
    for (int chunkX = minChunkX; chunkX <= maxChunkX; ++chunkX) {
        ...
        for (int chunkZ = minChunkZ; chunkZ <= maxChunkZ; ++chunkZ) {
            ...
            if (auto* multiplayerCache = dynamic_cast<client::world::chunk::MultiplayerChunkCache*>(chunkCache_.get());
