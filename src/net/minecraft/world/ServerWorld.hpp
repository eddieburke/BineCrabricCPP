...
public:
  ServerWorld(server::MinecraftServer* server, WorldStorage* storage, const std::string& name, int dimensionId,
              std::uint64_t seed);
  ...
  std::vector<block::entity::BlockEntity*> getBlockEntities(int minX, int minY, int minZ, int maxX, int maxY,
                                                            int maxZ);
