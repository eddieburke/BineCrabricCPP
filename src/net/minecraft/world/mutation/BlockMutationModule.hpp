...
// Narrow dependency bundle for block mutation. World& is the primary port today;
// blockUpdateEvent / setBlockDirty delegate to World until WorldEvents (M3) owns fan-out.
struct BlockMutationContext {
    World& world;

    explicit BlockMutationContext(World& worldIn) noexcept
        : world(worldIn)
    {
    }
};
...
public:
    explicit BlockMutationModule(BlockMutationContext& context) noexcept
        : context_(context)
    {
    }

    [[nodiscard]] BlockMutationContext& context() noexcept { return context_; }
    [[nodiscard]] const BlockMutationContext& context() const noexcept { return context_; }
