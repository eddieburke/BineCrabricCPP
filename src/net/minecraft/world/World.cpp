...
void World::save(bool blocking) {
  ...
  if(dimensionData_ != nullptr) {
    ...
    if(blocking) {
      ...
      asyncSaveFuture_ = std::async(std::launch::async,
...
void World::updateSpawnPosition() {
  ...
  mod::WorldSpawnSearchEvent spawnEvent{this, properties_.getSpawnX(), properties_.getSpawnY(),
                                        properties_.getSpawnZ(), false};
  mod::hooks().publish(spawnEvent);
  if(spawnEvent.resolved) {
    properties_.setSpawn(spawnEvent.x, spawnEvent.y, spawnEvent.z);
    setSpawnPos(Vec3i{spawnEvent.x, spawnEvent.y, spawnEvent.z});
...
void World::finishLightingUpdates() {
  // Lighting is fully asynchronous: never block world load on it. Forward any regions the worker has already produced; the rest stream in over the next frames via doLightingUpdates() and the renderer re-meshes them as they land.
...
void World::scheduleBlockUpdate(int x, int y, int z, int id, int tickRate) {
  ...
  if(id > 0) {
    const long long scheduledTime = static_cast<long long>(tickRate) + static_cast<long long>(hasStorageBackedProperties_ ? properties_.getTime() : time_);
