...
void World::notifyEntityAdded(Entity* entity) {
  ...
  if(entity != nullptr && mod::hooks().hasListeners<mod::EntitySpawnEvent>()) {
...
void World::notifyEntityRemoved(Entity* entity) {
  ...
  if(entity != nullptr && mod::hooks().hasListeners<mod::EntityRemoveEvent>()) {
...
void World::tickEntities() {
  for(std::size_t i = 0; i < globalEntities.size(); ++i) {
    ...
    bool canceled = false;
    if(mod::hooks().hasListeners<mod::EntityTickEvent>()) {
