...
entity::LivingEntity* World::spawnMob(const std::string& entityType, double x, double y, double z, float yaw,
                                      float pitch) {
...
void World::updateEntity(Entity* entity, bool requireLoaded, int depth) {
  ...
  if(requireLoaded && !isRegionLoaded(floorX - regionRadius, 0, floorZ - regionRadius, floorX + regionRadius,
                                      Chunk::height, floorZ + regionRadius)) {
    return;
  }
