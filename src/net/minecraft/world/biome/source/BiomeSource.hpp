...
public:
  explicit BiomeSource(std::uint64_t seed)
      : seed_(seed), temperatureRandom_(seed * 9871ULL), downfallRandom_(seed * 39811ULL),
        weirdnessRandom_(seed * 543321ULL), temperatureSampler_(temperatureRandom_, 4),
        downfallSampler_(downfallRandom_, 4), weirdnessSampler_(weirdnessRandom_, 2) {
  }
