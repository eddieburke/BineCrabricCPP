...
#include "net/minecraft/world/biome/SpawnListEntry.hpp"
...
class Biome {
 public:
 Biome();
 Biome& setName(std::string value) {
  name = std::move(value);
  return *this;
 }
 Biome& setGrassColor(int value) {
  grassColor = value;
  return *this;
 }
 Biome& setFoliageColor(int value) {
  foliageColor = value;
  return *this;
 }
 Biome& enableSnow() {
 ...
 [[nodiscard]] const std::vector<SpawnListEntry>& getSpawnableEntities(int creatureType) const;
 [[nodiscard]] bool canSnow() const noexcept {
  return hasSnow;
 }
 [[nodiscard]] bool canRain() const noexcept {
  return hasRain && !hasSnow;
 }
 [[nodiscard]] std::string_view wireName() const noexcept {
  return wireName_;
 }
 static void init();
 ...
 protected:
 std::vector<SpawnListEntry> spawnableMonsters_;
 std::vector<SpawnListEntry> spawnablePassive_;
 std::vector<SpawnListEntry> spawnableWaterCreatures_;
