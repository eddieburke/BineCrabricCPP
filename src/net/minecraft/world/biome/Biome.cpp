...
static registry::RegisterPhase<BiomeBootstrap> s_reg(mod::LifecyclePhase::Init, 0);
} // namespace
Biome::Biome() {
...
const std::vector<SpawnListEntry>& Biome::getSpawnableEntities(int creatureType) const {
 if(creatureType == 0) {
 ...
 static const std::vector<SpawnListEntry> empty;
 return empty;
}
void Biome::init() {
 if(gInitialized) {
