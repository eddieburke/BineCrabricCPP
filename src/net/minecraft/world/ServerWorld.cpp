...
ServerWorld::ServerWorld(server::MinecraftServer* server,
...
ChunkSource* ServerWorld::createChunkCache() {
  ...
  auto cache = std::make_unique<server::world::chunk::ServerChunkCache>(
      this, std::move(chunkStorage), chunkGeneratorSource_.get());
  chunkCache = cache.get();
  setChunkCache(std::move(cache));
  return chunkCache;
}
std::vector<block::entity::BlockEntity*> ServerWorld::getBlockEntities(
...
void ServerWorld::playNoteBlockActionAt(int x, int y, int z, int soundType, int pitch) {
  ...
  if(server_ != nullptr && dimension != nullptr) {
    ...
    server_->playerManager.sendToAround(
        static_cast<double>(x), static_cast<double>(y), static_cast<double>(z), 64.0, dimension->id, packet);
  }
}
