...
[[nodiscard]] bool shouldSkipSpawnBucket(const SpawnRules& rules, bool spawnAnimals, bool spawnMonsters,
                                         std::size_t chunkCount, const std::array<int, 3>& bucketCounts) {
...
Vec3i NaturalSpawner::getRandomPosInChunk(World* world, int chunkX, int chunkZ) {
  ...
  return Vec3i{chunkX + world->random().nextInt(16), world->random().nextInt(128),
  ...
  for(PlayerEntity* playerEntity : players) {
    ...
    for(int attempt = 0; attempt < 20 && !spawnedNearPlayer; ++attempt) {
      ...
      LivingEntity* living = world->spawnMob(monsterId, [&](LivingEntity& mob) {
        ...
        wakePos = block::BedBlock::findWakeUpPosition(world, MathHelper::floor(playerEntity->x),
