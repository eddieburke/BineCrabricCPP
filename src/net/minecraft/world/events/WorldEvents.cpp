...
void WorldEvents::removeEventListener(GameEventListener* listener)
{
    eventListeners_.erase(
        std::remove(eventListeners_.begin(), eventListeners_.end(), listener),
        eventListeners_.end());
}
...
void WorldEvents::blockUpdate(int x, int y, int z, int blockId)
{
    blockUpdateEvent(x, y, z);
    world_.notifyNeighbors(x, y, z, blockId);
}
