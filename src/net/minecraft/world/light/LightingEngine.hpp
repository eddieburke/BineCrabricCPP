...
class Chunk;
// Fully asynchronous lighting engine: propagation runs on one background thread; the main thread only enqueues via push() (never waits), drains finished regions via drainDirtyRegions() (the renderer re-meshes them), and registers/unregisters chunks (non-blocking). No public method blocks on the worker, so lighting never stalls a frame or gates world load. Chunk lifetime uses the shared render-pin: the worker pins each chunk it touches, so an evicting owner waits its turn through the existing non-blocking deferred render-eviction path.
