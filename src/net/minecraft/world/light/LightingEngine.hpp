...
// Background lighting thread: propagates block/sky light off the main thread.
// Main thread enqueues boxes via push(); drainDirtyRegions() forwards results to the renderer.
...
public:
    ...
    [[nodiscard]] bool busy() const noexcept { return pendingCount_.load(std::memory_order_relaxed) != 0; }
...
private:
    ...
    void enqueueSpread(LightType type, int x, int y, int z, int level);
