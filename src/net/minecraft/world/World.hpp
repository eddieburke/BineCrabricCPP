...
class World : public IEntityWorld {
 public:
 World(std::string name, std::uint64_t seed, std::unordered_map<std::string, std::string> creationOptions = {});
 World(WorldStorage* dimensionData,
       const std::string& name,
       std::int64_t seed,
       bool deferSpawnInit = false,
       std::unordered_map<std::string, std::string> creationOptions = {});
 // Faithful to Java World(World world, Dimension dimension) â€” dimension switch.
 ...
 [[nodiscard]] bool isNewWorld() const noexcept {
  return newWorld;
 }
 // Whether world events (block-place fan-out, neighbor updates) are processed.
 [[nodiscard]] bool isEventProcessingEnabled() const noexcept {
  return eventProcessingEnabled;
 }
 void setEventProcessingEnabled(bool value) noexcept {
  eventProcessingEnabled = value;
 }
 [[nodiscard]] bool isLuaModGenerationEnabled() const noexcept {
  return luaModGenerationEnabled_;
 }
 void setLuaModGenerationEnabled(bool value) noexcept {
  luaModGenerationEnabled_ = value;
 }
 ...
 void setInstantBlockUpdate(bool value) noexcept {
  instantBlockUpdateEnabled = value;
 }
 // Java exposes world.random publicly; Block/Item behavior reads from it.
 [[nodiscard]] JavaRandom& random() noexcept override {
  return random_;
 }
 ...
 void synchronizeTimeAndUpdates(std::uint64_t time) noexcept;
 ...
 [[nodiscard]] bool updateMovementInFluid(const Box& entityBoundingBox,
 ...
 [[nodiscard]] std::optional<HitResult> raycast(const Vec3d& start,
                                                const Vec3d& end,
 ...
 void allowSpawning(bool allowHostile, bool allowPeaceful) {
 ...
 [[nodiscard]] entity::LivingEntity* spawnMob(
     const std::string& entityType, double x, double y, double z, float yaw = 0.0f, float pitch = 0.0f);
 [[nodiscard]] bool spawnMob(entity::LivingEntity* mob);
 bool attemptSaving(int step);
 void updateEntity(Entity* entity) {
  updateEntity(entity, true, 0);
 }
 void updateEntity(Entity* entity, bool requireLoaded) {
  updateEntity(entity, requireLoaded, 0);
 }
 ...
 [[nodiscard]] std::vector<Box> getBlockCollisions(const Box& box) const;
 [[nodiscard]] bool isAnyBlockInBox(const Box& box) const;
 ...
 [[nodiscard]] bool canInteract(PlayerEntity* /*user*/, int /*x*/, int /*y*/, int /*z*/) const {
  return true;
 }
 virtual bool isSavingDisabled() const { return false; }
 ...
 void worldEvent(int event, int x, int y, int z, int data);
 ...
 StateT* getOrCreateState(const std::string& id) {
  // Register StateT's factory so an existing save on disk is reconstructed as the right type
  // (instead of being silently dropped and replaced with a fresh, empty state).
  PersistentStateRegistry::instance().registerType<StateT>();
 ...
 [[nodiscard]] WorldWeather& weather() noexcept {
  return weather_;
 }
 [[nodiscard]] const WorldWeather& weather() const noexcept {
  return weather_;
 }
 ...
 // Mode flags â€” exposed through accessors above; not for direct external poke.
 bool newWorld = false;
 bool eventProcessingEnabled = false;
 bool luaModGenerationEnabled_ = false;
 bool instantBlockUpdateEnabled = false;
 // Internal spawn/time/sleep state â€” World-only.
