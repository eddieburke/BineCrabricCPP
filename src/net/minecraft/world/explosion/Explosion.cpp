...
void Explosion::explode() {
  ...
  for(entity::Entity* entity : entities) {
    ...
    entity->damage(source, static_cast<int>((blastExposure * blastExposure + blastExposure) / 2.0 * 8.0 *
...
void Explosion::playExplosionSound(bool addParticles) {
  ...
  world_->playSound(x, y, z, "random.explode", 4.0f,
  ...
  for(auto it = blocks.rbegin(); it != blocks.rend(); ++it) {
    ...
    if(addParticles) {
      ...
      world_->addParticle("explode", (px + x) / 2.0, (py + y) / 2.0, (pz + z) / 2.0, dirX * speed, dirY * speed,
                          dirZ * speed);
