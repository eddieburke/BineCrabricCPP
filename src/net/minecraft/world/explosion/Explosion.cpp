...
void Explosion::explode()
{
    ...
    for (int i = 0; i < sampleCount; ++i) {
        for (int j = 0; j < sampleCount; ++j) {
            for (int k = 0; k < sampleCount; ++k) {
                if (i != 0 && i != sampleCount - 1 && j != 0 && j != sampleCount - 1 && k != 0 && k != sampleCount - 1) {
                    continue;
                }
    ...
    const std::vector<Entity*> entities = world_->getEntities(
        source, Box{static_cast<double>(minX), static_cast<double>(minY), static_cast<double>(minZ),
                    static_cast<double>(maxX), static_cast<double>(maxY), static_cast<double>(maxZ)});
    for (entity::Entity* entity : entities) {
        ...
        entity->damage(source, static_cast<int>((blastExposure * blastExposure + blastExposure) / 2.0 * 8.0 * static_cast<double>(power) + 1.0));
    ...
    if (fire) {
        ...
        for (auto it = blocks.rbegin(); it != blocks.rend(); ++it) {
            ...
            if (blockId != 0 || belowId == 0
                || !block::Block::BLOCKS_OPAQUE[static_cast<std::size_t>(belowId)]) {
                continue;
            }
