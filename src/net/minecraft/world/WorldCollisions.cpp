...
bool World::isMaterialInBox(const Box& boundingBox, block::material::Material& material) const {
  return forEachBlockInBounds(
      *this, blockScanBounds(boundingBox),
      [&](int /*x*/, int /*y*/, int /*z*/, Block& block) { return &block.material == &material; });
}
...
bool World::updateMovementInFluid(const Box& entityBoundingBox, block::material::Material& fluidMaterial,
                                  Entity* entity) {
...
bool World::isBoxSubmergedInFluid(const Box& box) const {
  return forEachBlockInBounds(
      *this, blockScanBoundsWithNegativePadding(box),
      [&](int /*x*/, int /*y*/, int /*z*/, Block& block) { return block.material.isFluid(); });
}
void World::extinguishFire(PlayerEntity* /*player*/, int x, int y, int z, int direction) {
  ...
  if(Block::FIRE != nullptr && getBlockId(x, y, z) == Block::FIRE->id) {
    ...
    playSound(static_cast<double>(x) + 0.5, static_cast<double>(y) + 0.5, static_cast<double>(z) + 0.5,
              "random.fizz", 0.5f, 2.6f + (rng.nextFloat() - rng.nextFloat()) * 0.8f);
    setBlock(x, y, z, 0);
  }
}
std::optional<HitResult> World::raycast(const Vec3d& start, const Vec3d& end, bool ignoreLiquids,
