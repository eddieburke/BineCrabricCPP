...
protected:
                                ...
                                for (int i = maxY - 1; i >= minY; --i) {
                                    ...
                                    if (dy > -0.7 && dx * dx + dy * dy + dz * dz < 1.0) {
                                        ...
                                        if (by == Block::STONE->id || by == Block::DIRT->id || by == Block::GRASS_BLOCK->id) {
                                            if (i < 10) {
                                                setRawBlock(chunk, lx, i + 1, lz, Block::FLOWING_LAVA->id);
                                            } else {
                                                setRawBlock(chunk, lx, i + 1, lz, 0);
                                                if (grassAbove && rawBlock(chunk, lx, i, lz) == Block::DIRT->id) {
                                                    setRawBlock(chunk, lx, i, lz, Block::GRASS_BLOCK->id);
                                                }
                                            }
                                        }
                                    }
    ...
    {
        int n = random.nextInt(random.nextInt(random.nextInt(40) + 1) + 1);
