...
SkyChunkGenerator::SkyChunkGenerator(World* world, std::uint64_t seed)
    : random_(seed), seed_(seed), world_(world), minLimitPerlinNoise_(random_, 16),
      maxLimitPerlinNoise_(random_, 16), perlinNoise1_(random_, 8), perlinNoise2_(random_, 4),
      perlinNoise3_(random_, 4), floatingIslandScale_(random_, 10), floatingIslandNoise_(random_, 16),
      forestNoise_(random_, 8), biomeSource_(seed) {
}
...
Chunk& SkyChunkGenerator::getChunk(int chunkX, int chunkZ) {
  ...
  world::gen::ChunkGenerationContext context{world_, this, &chunk, biomeSource, &random_,
  ...
  world::gen::runVanillaStage(world::gen::ChunkStage::Surface, context,
...
void SkyChunkGenerator::decorate(ChunkSource* source, int chunkX, int chunkZ) {
  ...
  world::gen::ChunkGenerationContext context{
      world_, source, chunk, biomeSource, &random_, worldSeed, chunkX, chunkZ,
      world_ != nullptr && world_->isLuaModGenerationEnabled(), false};
  world::gen::runVanillaStage(world::gen::ChunkStage::Features, context, [&] {
    world::gen::feature::decorateSkyChunk(world_, random_, *biomeSource, forestNoise_, chunkX, chunkZ,
                                          decorateTemperatures_);
  });
...
void SkyChunkGenerator::buildTerrain(int chunkX, int chunkZ, Chunk& chunk, const std::vector<double>& /*temperatures*/) {
...
std::vector<double>& SkyChunkGenerator::generateHeightMap(std::vector<double>& heightMap, int x, int y, int z,
  ...
  perlinNoiseBuffer_ = perlinNoise1_.create(perlinNoiseBuffer_, x, y, z, sizeX, sizeY, sizeZ, baseScale / 80.0,
                                            verticalScale / 160.0, baseScale / 80.0);
  minLimitPerlinNoiseBuffer_ = minLimitPerlinNoise_.create(minLimitPerlinNoiseBuffer_, x, y, z, sizeX, sizeY, sizeZ,
                                                           baseScale, verticalScale, baseScale);
  maxLimitPerlinNoiseBuffer_ = maxLimitPerlinNoise_.create(maxLimitPerlinNoiseBuffer_, x, y, z, sizeX, sizeY, sizeZ,
                                                           baseScale, verticalScale, baseScale);
  int index = 0;
