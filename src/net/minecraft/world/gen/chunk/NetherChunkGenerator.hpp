...
class NetherChunkGenerator : public ChunkSource {
 ...
 Chunk& getChunk(int chunkX, int chunkZ) override {
  ...
  world::gen::runVanillaStage(
      world::gen::ChunkStage::Terrain, context, [&] { buildTerrain(chunkX, chunkZ, chunk); });
  world::gen::runVanillaStage(
      world::gen::ChunkStage::Surface, context, [&] { buildSurfaces(chunkX, chunkZ, chunk); });
  world::gen::runVanillaStage(
 ...
 void decorate(ChunkSource* source, int chunkX, int chunkZ) override {
  ...
  Chunk* chunk =
      world_ != nullptr && world_->hasChunk(chunkX, chunkZ) ? &world_->getChunk(chunkX, chunkZ) : nullptr;
  world::gen::ChunkGenerationContext context{world_,
                                             source,
                                             chunk,
                                             nullptr,
  ...
  world::gen::runVanillaStage(world::gen::ChunkStage::Features, context, [&] {
   ...
   for(int i = 0; i < 8; ++i) {
    ...
    NetherLavaSpringFeature(Block::FLOWING_LAVA->id)
        .generate(world_, random_, featureX, featureY, featureZ);
   }
 ...
 std::vector<double>& generateHeightMap(
  ...
  perlinNoiseBuffer_ = perlinNoise1_.create(
      perlinNoiseBuffer_, x, y, z, sizeX, sizeY, sizeZ, baseScale / 80.0, verticalScale / 60.0, baseScale / 80.0);
