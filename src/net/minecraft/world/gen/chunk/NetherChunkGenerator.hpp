...
public:
  explicit NetherChunkGenerator(World* world, std::uint64_t seed)
      : random_(seed), seed_(seed), minLimitPerlinNoise_(random_, 16), maxLimitPerlinNoise_(random_, 16),
        perlinNoise1_(random_, 8), perlinNoise2_(random_, 4), perlinNoise3_(random_, 4), scaleNoise_(random_, 10),
        depthNoise_(random_, 16), world_(world) {
  }
  ...
  Chunk& getChunk(int chunkX, int chunkZ) override {
    ...
    world::gen::ChunkGenerationContext context{world_, this, &chunk, nullptr, &random_,
                                               world_ != nullptr ? world_->getSeed() : seed_, chunkX, chunkZ,
                                               world_ != nullptr && world_->isLuaModGenerationEnabled(), false};
    world::gen::runVanillaStage(world::gen::ChunkStage::Terrain, context, [&] { buildTerrain(chunkX, chunkZ, chunk); });
    world::gen::runVanillaStage(world::gen::ChunkStage::Surface, context, [&] { buildSurfaces(chunkX, chunkZ, chunk); });
  ...
  void decorate(ChunkSource* source, int chunkX, int chunkZ) override {
    ...
    world::gen::ChunkGenerationContext context{world_, source, chunk, nullptr, &random_,
                                               world_ != nullptr ? world_->getSeed() : seed_, chunkX, chunkZ,
                                               world_ != nullptr && world_->isLuaModGenerationEnabled(), false};
...
private:
  ...
  void buildSurfaces(int chunkX, int chunkZ, Chunk& chunk) {
    ...
    depthBuffer_ = perlinNoise3_.create(depthBuffer_, chunkX * 16, chunkZ * 16, 0.0, 16, 16, 1, scale * 2.0,
                                        scale * 2.0, scale * 2.0);
    for(int z = 0; z < 16; ++z) {
      for(int x = 0; x < 16; ++x) {
  ...
  std::vector<double>& generateHeightMap(std::vector<double>& heightMap, int x, int y, int z, int sizeX, int sizeY,
                                         int sizeZ) {
    ...
    perlinNoiseBuffer_ = perlinNoise1_.create(perlinNoiseBuffer_, x, y, z, sizeX, sizeY, sizeZ, baseScale / 80.0,
                                              verticalScale / 60.0, baseScale / 80.0);
    minLimitPerlinNoiseBuffer_ = minLimitPerlinNoise_.create(minLimitPerlinNoiseBuffer_, x, y, z, sizeX, sizeY,
                                                             sizeZ, baseScale, verticalScale, baseScale);
    maxLimitPerlinNoiseBuffer_ = maxLimitPerlinNoise_.create(maxLimitPerlinNoiseBuffer_, x, y, z, sizeX, sizeY,
                                                             sizeZ, baseScale, verticalScale, baseScale);
    std::vector<double> verticalCurve(static_cast<std::size_t>(sizeY));
    for(int iy = 0; iy < sizeY; ++iy) {
      verticalCurve[static_cast<std::size_t>(iy)] =
          std::cos(static_cast<double>(iy) * 3.141592653589793 * 6.0 / static_cast<double>(sizeY)) * 2.0;
      ...
      if(distanceFromEdge < 4.0) {
        ...
        verticalCurve[static_cast<std::size_t>(iy)] -=
            distanceFromEdge * distanceFromEdge * distanceFromEdge * 10.0;
      }
    }
    int index = 0;
