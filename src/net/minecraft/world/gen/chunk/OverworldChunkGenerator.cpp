...
Chunk OverworldChunkGenerator::loadChunk(ChunkSource* source, int chunkX, int chunkZ) {
  ...
  world::gen::ChunkGenerationContext context{world_, source, &chunk, biomeSource, &random_,
                                             world_ != nullptr ? world_->getSeed() : seed_, chunkX, chunkZ,
                                             world_ != nullptr ? world_->isLuaModGenerationEnabled()
                                                               : modGenerationEnabled_,
                                             true};
...
void OverworldChunkGenerator::decorate(ChunkSource* source, int chunkX, int chunkZ) {
  ...
  world::gen::ChunkGenerationContext context{
      ...
      world_->isLuaModGenerationEnabled(), true};
  world::gen::runVanillaStage(world::gen::ChunkStage::Features, context, [&] {
    world::gen::feature::decorateOverworldChunk(world_, random_, *biomeSource, forestNoise_, chunkX, chunkZ,
                                                decorateTemperatures_);
  });
  block::FallingBlock::fallInstantly = false;
}
std::vector<double>& OverworldChunkGenerator::generateHeightMap(std::vector<double>& heightMap, int x, int y, int z,
  ...
  perlinNoiseBuffer_ = perlinNoise1_.create(perlinNoiseBuffer_, x, y, z, sizeX, sizeY, sizeZ, baseScale / 80.0,
                                            baseScale / 160.0, baseScale / 80.0);
