...
public:
  ...
  static void setRawBlock(Chunk& chunk, int localX, int y, int localZ, int blockId) {
    chunk.blocks[static_cast<std::size_t>((localX << 11) | (localZ << 7) | y)] =
        static_cast<std::uint8_t>(blockId & 0xFF);
  }

protected:
  virtual void place(World* /*world*/, int /*startChunkX*/, int /*startChunkZ*/, int /*chunkX*/, int /*chunkZ*/,
                     Chunk& /*chunk*/) {
  }
  int range = 8;
  JavaRandom random;
