...
class LargeOakTreeFeature : public Feature {
   public:
    ...
    void makeBranches() {
        ...
        while (heightFromOrigin >= 0) {
            ...
            for (int i = 0; i < branchCount; ++i) {
                const double length = branchLengthScale *
                                      (static_cast<double>(shape) * (static_cast<double>(random.nextFloat()) + 0.328));
                const double angle = static_cast<double>(random.nextFloat()) * 2.0 * 3.14159;
                const int branchX =
                    MathHelper::floor(length * std::sin(angle) + static_cast<double>(origin[0]) + centerOffset);
                const int branchZ =
                    MathHelper::floor(length * std::cos(angle) + static_cast<double>(origin[2]) + centerOffset);
    ...
    void placeBranch(const std::array<int, 3>& from, const std::array<int, 3>& to, int log) {
        ...
        const double ratioA =
            static_cast<double>(delta[static_cast<std::size_t>(minorAxisA)]) / static_cast<double>(delta[majorAxis]);
        const double ratioB =
            static_cast<double>(delta[static_cast<std::size_t>(minorAxisB)]) / static_cast<double>(delta[majorAxis]);
        std::array<int, 3> pos = {0, 0, 0};
        const int endStep = delta[majorAxis] + step;
        for (int i = 0; i != endStep; i += step) {
            pos[static_cast<std::size_t>(majorAxis)] =
                MathHelper::floor(static_cast<double>(from[majorAxis] + i) + 0.5);
            pos[static_cast<std::size_t>(minorAxisA)] =
                MathHelper::floor(static_cast<double>(from[static_cast<std::size_t>(minorAxisA)]) +
                                  static_cast<double>(i) * ratioA + 0.5);
            pos[static_cast<std::size_t>(minorAxisB)] =
                MathHelper::floor(static_cast<double>(from[static_cast<std::size_t>(minorAxisB)]) +
    ...
    [[nodiscard]] int tryBranch(const std::array<int, 3>& from, const std::array<int, 3>& to) {
        ...
        for (; i != endStep; i += step) {
            ...
            pos[static_cast<std::size_t>(minorAxisA)] = MathHelper::floor(
                static_cast<double>(from[static_cast<std::size_t>(minorAxisA)]) + static_cast<double>(i) * ratioA);
            pos[static_cast<std::size_t>(minorAxisB)] = MathHelper::floor(
                static_cast<double>(from[static_cast<std::size_t>(minorAxisB)]) + static_cast<double>(i) * ratioB);
            const int blockId = world->getBlockId(pos[0], pos[1], pos[2]);
            if (blockId != 0 && blockId != tree_feature::id(Block::LEAVES)) {
                break;
            }
