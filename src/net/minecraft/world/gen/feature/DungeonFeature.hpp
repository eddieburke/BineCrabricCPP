...
class DungeonFeature : public Feature {
 ...
 bool generate(World* world, JavaRandom& random, int x, int y, int z) override {
  ...
  for(int dx = x - roomWidth - 1; dx <= x + roomWidth + 1; ++dx) {
   for(int dy = y - 1; dy <= y + roomHeight + 1; ++dy) {
    for(int dz = z - roomDepth - 1; dz <= z + roomDepth + 1; ++dz) {
     ...
     if(dx != x - roomWidth - 1 && dx != x + roomWidth + 1 && dz != z - roomDepth - 1 &&
        dz != z + roomDepth + 1) {
      continue;
     }
  ...
  for(int attempt = 0; attempt < 2; ++attempt) {
   for(int tries = 0; tries < 3; ++tries) {
    ...
    auto* chestBlockEntity =
        dynamic_cast<block::entity::ChestBlockEntity*>(world->getBlockEntity(chestX, chestY, chestZ));
    if(chestBlockEntity != nullptr) {
     for(int slot = 0; slot < 8; ++slot) {
      ...
      chestBlockEntity->setStack(
          static_cast<std::size_t>(random.nextInt(static_cast<int>(chestBlockEntity->size()))),
          std::move(itemStack));
     }
    }
    goto chestPlaced;
 ...
 chestPlaced:
  ...
  if(auto* mobSpawnerBlockEntity =
         dynamic_cast<block::entity::MobSpawnerBlockEntity*>(world->getBlockEntity(x, y, z))) {
