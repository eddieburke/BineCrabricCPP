      ...
      try {
        ...
        if(input) {
          ...
          if(root.contains("data")) {
            if(std::unique_ptr<PersistentState> state = PersistentStateRegistry::instance().create(stateClass, id)) {
...
void SavedDataStorage::set(const std::string& id, std::unique_ptr<PersistentState> state) {
  ...
  if(const auto existing = impl_->loadedStatesById_.find(id); existing != impl_->loadedStatesById_.end()) {
    impl_->loadedStates_.erase(std::remove(impl_->loadedStates_.begin(), impl_->loadedStates_.end(), existing->second.get()),
