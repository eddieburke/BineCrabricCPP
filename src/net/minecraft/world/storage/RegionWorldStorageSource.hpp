...
public:
  ...
  [[nodiscard]] std::vector<WorldSaveInfo> getAll() override {
    ...
    for(const fs::directory_entry& entry : fs::directory_iterator(dir)) {
      ...
      saves.emplace_back(saveName, displayName, propsCopy.setLastPlayed(), propsCopy.getSizeOnDisk(),
                         sameVersion);
    }
    return saves;
  }
  ...
  [[nodiscard]] bool needsConversion(const std::string& saveName) const override {
    const std::optional<WorldProperties> props =
        const_cast<RegionWorldStorageSource*>(this)->getWorldProperties(saveName);
