...
public:
    EmptyChunk(World* world = nullptr, int x = 0, int z = 0)
        : Chunk(world, x, z)
    {
        empty = true;
    }

    EmptyChunk(World* world, const std::vector<std::uint8_t>& blocks, int x, int z)
        : Chunk(world, blocks, x, z)
    {
        empty = true;
    }
    ...
    int loadFromPacket(const std::vector<std::uint8_t>&, int minX, int minY, int minZ, int maxX, int maxY, int maxZ, int offset)
    {
