...
void appendNamedByteArray(std::vector<std::uint8_t>& out, const std::string& key, const std::vector<std::uint8_t>& bytes)
{
    appendNamedByteArray(out, key, bytes.data(), bytes.size());
}

void appendNamedListHeader(std::vector<std::uint8_t>& out, const std::string& key, Nbt::Type itemType, std::int32_t size)
{
