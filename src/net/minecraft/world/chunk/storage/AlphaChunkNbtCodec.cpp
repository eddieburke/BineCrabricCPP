...
void appendNamedByteArray(std::vector<std::uint8_t>& out, const std::string& key, const std::uint8_t* data,
                          std::size_t size) {
...
void appendNamedByteArray(std::vector<std::uint8_t>& out, const std::string& key,
                          const std::vector<std::uint8_t>& bytes) {
  appendNamedByteArray(out, key, bytes.data(), bytes.size());
}
void appendNamedListHeader(std::vector<std::uint8_t>& out, const std::string& key, Nbt::Type itemType,
                           std::int32_t size) {
...
void writeLevelFields(std::vector<std::uint8_t>& out, Chunk& chunk, World* world) {
  ...
  appendNamedListHeader(out, "Entities", entities.size() == 0 ? Nbt::Type::Byte : Nbt::Type::Compound,
  ...
  appendNamedListHeader(out, "TileEntities", tileEntities.size() == 0 ? Nbt::Type::Byte : Nbt::Type::Compound,
...
Chunk AlphaChunkNbtCodec::loadChunkFromNbt(World* world, NbtCompound& nbt) {
  ...
  if(std::vector<std::uint8_t>* heightMapBytes = takeByteArray(nbt, "HeightMap")) {
    ...
    if(!heightMapBytes->empty()) {
      std::copy_n(heightMapBytes->begin(), std::min(heightMapBytes->size(), chunk.heightmap.size()),
