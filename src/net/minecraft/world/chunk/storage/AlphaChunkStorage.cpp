...
void AlphaChunkStorage::saveChunk(World* world, Chunk& chunk) {
 ...
 // Alpha is a legacy/import format: the runtime saves natively as McRegion, and old worlds are
 // repacked to region on load (RegionWorldStorageSource::convert). This write path exists only
 // to satisfy the ChunkStorage interface for the never-instantiated "Old Format" source, so it
 // stays a plain temp+rename — no crash-safe fsync machinery needed.
 try {
