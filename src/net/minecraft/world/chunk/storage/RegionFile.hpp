...
class RegionFile {
public:
    explicit RegionFile(fs::path file)
        : file_(std::move(file))
    {
...
private:
    ...
    void writeChunkData(std::uint32_t sectorOffset, const std::vector<std::uint8_t>& compressed, std::uint8_t compressionType)
    {
        stream_.clear();
        ...
        if (!compressed.empty()) {
            stream_.write(reinterpret_cast<const char*>(compressed.data()), static_cast<std::streamsize>(compressed.size()));
        }
        stream_.flush();
    }
