  ...
  if(storage_ != nullptr && world_ != nullptr) {
   try {
    ...
    Log::LOGGER.log(LogLevel::Warning,
      "ChunkCache: chunk load failed for (" + std::to_string(chunkX) + ", " + std::to_string(chunkZ) + "), falling back to generation");
   }
  }
...
void ChunkCache::integrateFinishedLoads(int budget) {
 for(auto it = pendingLoads_.begin(); it != pendingLoads_.end() && budget > 0;) {
...
void ChunkCache::prefetchChunksNear(int centerChunkX, int centerChunkZ) {
 ...
 const int maxLoadsPerCall = asyncCapable ? 16 : 8;
 int loaded = 0;
...
std::string ChunkCache::getDebugInfo() const {
 return "ChunkCache: " + std::to_string(chunksByPos_.size()) +
