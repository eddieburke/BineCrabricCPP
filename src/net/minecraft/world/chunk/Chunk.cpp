...
void Chunk::populateHeightMap()
{
    ...
    for (int localX = 0; localX < 16; ++localX) {
        for (int localZ = 0; localZ < 16; ++localZ) {
            ...
            int yPos = 127;
            do {
                light -= Block::BLOCKS_LIGHT_OPACITY[static_cast<std::size_t>(blocks[static_cast<std::size_t>(base + yPos)] & 0xFFU)];
                if (light > 0) {
...
void Chunk::updateHeightMap(int localX, int yPos, int localZ)
{
    ...
    const int base = (localX << 11) | (localZ << 7);
    while (newHeight > 0
           && Block::BLOCKS_LIGHT_OPACITY[static_cast<std::size_t>(blocks[static_cast<std::size_t>(base + newHeight - 1)] & 0xFFU)]
                  == 0) {
        --newHeight;
    }
    ...
    if (newHeight < minHeightmapValue) {
        ...
        for (int localZIndex = 0; localZIndex < 16; ++localZIndex) {
            for (int localXIndex = 0; localXIndex < 16; ++localXIndex) {
                const int heightValue = static_cast<int>(heightmap[static_cast<std::size_t>((localZIndex << 4) | localXIndex)] & 0xFFU);
    ...
    while (newHeight > 0 && Block::BLOCKS_LIGHT_OPACITY[static_cast<std::size_t>(getBlockId(localX, newHeight - 1, localZ))] == 0) {
        --newHeight;
    }
