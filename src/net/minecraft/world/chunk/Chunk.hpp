...
public:
  ...
  Chunk(World* world, int x, int z)
      : world(world), blocks(volume, 0), meta(static_cast<int>(volume)), skyLight(static_cast<int>(volume)),
        blockLight(static_cast<int>(volume)), x(x), z(z) {
    heightmap.fill(0);
  }
  ...
  Chunk(Chunk&& other) noexcept
      : world(other.world), blocks(std::move(other.blocks)), loaded(other.loaded), meta(std::move(other.meta)),
        skyLight(std::move(other.skyLight)), blockLight(std::move(other.blockLight)), heightmap(other.heightmap),
        minHeightmapValue(other.minHeightmapValue), x(other.x), z(other.z),
        blockEntities(std::move(other.blockEntities)), entities(std::move(other.entities)),
        terrainPopulated(other.terrainPopulated), dirty(other.dirty), empty(other.empty),
        lastSaveHadEntities(other.lastSaveHadEntities), lastSaveTime(other.lastSaveTime) {
    other.world = nullptr;
    other.loaded = false;
  }
  ...
  int loadFromPacket(const std::vector<std::uint8_t>& bytes, int minX, int minY, int minZ, int maxX, int maxY,
                     int maxZ, int offset) {
    ...
    for(int localX = minX; localX < maxX; ++localX) {
      for(int localZ = minZ; localZ < maxZ; ++localZ) {
        const std::size_t dest = index(localX, minY, localZ) >> 1U;
        const int count = (maxY - minY) / 2;
        std::copy_n(bytes.begin() + offset, count,
                    blockLight.bytes.begin() + static_cast<std::ptrdiff_t>(dest));
        offset += count;
      }
    }
    for(int localX = minX; localX < maxX; ++localX) {
  ...
  [[nodiscard]] int toPacket(std::vector<std::uint8_t>& bytes, int minX, int minY, int minZ, int maxX, int maxY,
                             int maxZ, int offset) const {
    ...
    for(int localX = minX; localX < maxX; ++localX) {
      for(int localZ = minZ; localZ < maxZ; ++localZ) {
        const std::size_t source = index(localX, minY, localZ) >> 1U;
        const int count = sizeY / 2;
        std::copy_n(blockLight.bytes.begin() + static_cast<std::ptrdiff_t>(source), count,
                    bytes.begin() + offset);
        offset += count;
      }
    }
    for(int localX = minX; localX < maxX; ++localX) {
      for(int localZ = minZ; localZ < maxZ; ++localZ) {
        const std::size_t source = index(localX, minY, localZ) >> 1U;
        const int count = sizeY / 2;
        std::copy_n(skyLight.bytes.begin() + static_cast<std::ptrdiff_t>(source), count,
                    bytes.begin() + offset);
        offset += count;
      }
    }
