...
public:
    ShapelessRecipe(ItemStack output, std::vector<ItemStack> input)
        : output(output), input(std::move(input)) {}
    ...
    [[nodiscard]] int getSize() const override { return static_cast<int>(this->input.size()); }
