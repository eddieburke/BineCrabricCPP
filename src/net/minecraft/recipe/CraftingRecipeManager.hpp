...
class RecipeArg {
public:
    enum class Kind { Rows, Row, Key, Item, Block, Stack };
    ...
    [[nodiscard]] const std::string& row() const { return row_; }
    [[nodiscard]] char key() const { return key_; }
    [[nodiscard]] net::minecraft::Item* item() const { return item_; }
    [[nodiscard]] net::minecraft::block::Block* block() const { return block_; }
    [[nodiscard]] const ItemStack& stack() const { return stack_; }
...
public:
    ...
    [[nodiscard]] const std::vector<std::unique_ptr<CraftingRecipe>>& getRecipes() const { return recipes; }
