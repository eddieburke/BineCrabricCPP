...
local function read_int(field, fallback, lo, hi)
  local raw = minecraft.screen.field_text(field)
  local value = tonumber(raw)
  if value == nil then return clamp_int(fallback, lo, hi) end
  return clamp_int(value, lo, hi)
end

local function read_num(field, fallback, lo, hi)
  local raw = minecraft.screen.field_text(field)
  local value = tonumber(raw)
  if value == nil then return clamp(fallback, lo, hi) end
  return clamp(value, lo, hi)
end

local function read_str(field, fallback)
  local raw = minecraft.screen.field_text(field)
  if type(raw) ~= "string" or #raw == 0 then return fallback end
  return raw
end
...
local function toggle_at(settings, x, y)
  ...
  for index, toggle_y in ipairs(ui.toggle_ys) do
    if minecraft.util.in_rect(x, y, ui.toggle_x, toggle_y, ui.toggle_w, ui.toggle_h) then
      ...
      if index == 4 then settings.drive_sun = not settings.drive_sun end
      return true
    end
  end
  return false
  ...
  minecraft.on("screen_event", {
    ...
    if event.phase == "init" then
      ...
      minecraft.screen.add_button(event.width - 70, event.height - 26, 62, 20, "Back", function()
        leave_settings(settings, save_fn)
      end)
