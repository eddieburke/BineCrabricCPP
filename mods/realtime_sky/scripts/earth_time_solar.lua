...
local function parse_fixed_offset_minutes(time_zone_id)
  local sign, hours, minutes = tostring(time_zone_id):match("^GMT([%+%-])(%d%d?):(%d%d)$")
  if sign == nil then
    sign, hours = tostring(time_zone_id):match("^GMT([%+%-])(%d%d?)$")
    minutes = "0"
  end
  assert(sign ~= nil, "realtime_sky: time zone must use GMT±H or GMT±H:MM")
...
function earth_time_solar.time_zone_info(settings)
  return {
