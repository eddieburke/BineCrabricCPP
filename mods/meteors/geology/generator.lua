...
function geology.buildModel(kind, variant, lod, thermal)
  ...
  local key = table.concat({ config.model_version, kind, variant, lod, subdivisions, thermal and "hot" or "body" }, "_")
...
function geology.modelLOD(distance)
  local setting = function(name, fallback)
    local value = minecraft.settings.get(config.settings[name].key)
    if value == nil then return fallback end
    return value
