...
function rendering.renderMeteor(meteor, camera, geologyModule)
  local vertices = {}
  local distance = length3(meteor.x - camera.x, meteor.y - camera.y, meteor.z - camera.z)
  local lod = geologyModule.modelLOD(distance)
  
  -- Get or build model
  local modelHandle = geologyModule.buildModel(meteor.kind, meteor.variant, lod, meteor.thermal)
