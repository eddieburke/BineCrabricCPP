...
bool R(vec3 v,float y,Ray f,bool x,inout float i,inout vec3 z) { return TraceBlockShape(v, y, f, x, i, z); }
 ...
 vec3 R(float v,float f,float y,vec3 i)
 {
   ...
   m.y=y*sin(v);
   m.z=f;
   vec3 s=abs(i.y)<.999?vec3(0,0,1):vec3(1,0,0),x=normalize(cross(i,vec3(0.,1.,1.))),r=cross(x,i);
   return x*m.x+r*m.y+i*m.z;
 }
 ...
 {
   float f=G(y),s=2*3.14159*v.x,x=pow(v.y,1.f/(f+1.f)),i=sqrt(1-x*x);
   return R(s,x,i,z);
 }
 float a(vec2 v)
 {
 ...
 vec4 G(sampler2D v,float f,bool y,float i,float s,float z,float x)
 {
   ...
   vec4 c=GetViewPosition(texcoord.xy,m.depth),t=gbufferModelViewInverse*vec4(c.xyz,1.),d=gbufferModelViewInverse*vec4(c.xyz,0.);
   vec3 w=normalize(c.xyz),o=normalize(d.xyz),A=normalize((gbufferModelViewInverse*vec4(m.normal,0.)).xyz);
   float p=GetDepthLinear(texcoord.xy),l=dot(-w,m.normal.xyz),e=1.-m.smoothness,M=e*e,b=R(m.smoothness,m.metalness);
   ...
   vec2 F=normalize(cross(m.normal,w).xy),k=a(F,1.5708);
   float K=1.-pow(1.-saturate(l),1.);
   F*=mix(.1675,.5,K);
   k*=mix(mix(.7,.7,M),.5,K);
   vec3 N=reflect(-w,m.normal);
   int B=0;
   for(int I=-1;
I<=1;
I++)
...
V++)
         {
           ...
           vec3 C=GetNormals(E);
           float j=GetDepthLinear(E),Z=pow(saturate(dot(N,reflect(-w,C))),105./M),ab=exp(-(abs(j-p)*S)),ac=exp(-(h(O.xyz,W.xyz)*T)),ad=exp(-abs(J-a(E))*L),ae=Z*ab*ac*ad;
 ...
 void main()
 {
   ...
   f=G(colortex6,10.,true,180.,40.,.1,.1);
