...
vec3 r(vec2 v) { return ShadowUVToVolumeCoord(v); }
...
l+=1)
     {
       {
         vec2 D=vec2(l+M.x)/vec2(viewWidth,viewHeight)*a*c,g=i.xy+D.xy;
         ...
         vec3 S=GetNormals(g);
         float J=GetDepth(g),u=ExpToLinearDepth(J),L=pow(saturate(dot(h,S)),R),P=exp(-(abs(u-t)*A)),Q=0.;
 ...
 {
   float v;
   vec4 y=G(colortex6,texcoord.xy,true,1.,.011,vec2(0.,1.),v);
   gl_FragData[0]=y;
 }
